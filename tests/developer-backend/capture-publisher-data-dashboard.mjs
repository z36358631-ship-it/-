...
await page.locator('.publisher-metric-tooltip.is-open').waitFor({ state:'hidden' });

const dayHit = page.locator('[data-detail-point][data-detail-date="2026-08-27"] [data-detail-day-hit]');
const dayHitBox = await dayHit.boundingBox();
if (!dayHitBox) throw new Error('Daily hover region is not visible');
await page.mouse.move(dayHitBox.x + dayHitBox.width / 2,dayHitBox.y + 3);
await page.locator('[data-detail-hover-tooltip]').waitFor({ state:'visible' });
await page.screenshot({ path:path.join(outputDir, '05-dashboard-day-hover.png'), fullPage:false });
await page.mouse.move(5,5);
await page.locator('[data-detail-hover-tooltip]').waitFor({ state:'hidden' });
...
await page.screenshot({ path:path.join(outputDir, '06-dashboard-users.png'), fullPage:false });
...
const flowSteps = [
  ...
  ['02-dashboard-overview.png','02 查看曝光转化','默认展示近 30 天六项指标和有效曝光按日详情'],
  ['03-dashboard-calendar.png','03 筛选与理解指标','按时间、来源与地区筛选，悬浮问号查看口径'],
  ['06-dashboard-users.png','04 查看用户数据','点击活跃、新增、留存或时长，查看图表或逐日表格'],
