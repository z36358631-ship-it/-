...
test('企业认证内容配置使用独立文章列表且不提供排序', async () => {
  ...
  try {
    ...
    assert.match(await page.locator('[data-runtime-result]').innerText(), /已保存并生效/);
  } finally {
    await context.close();
  }
});
...
test('帮助中心独立页面支持导航目录和目录子文档双语 CRUD 与排序约束', async () => {
  ...
  try {
    ...
    assert.deepEqual(await page.locator('.content-article-list thead th').allTextContents(), ['导航名称／唯一编号', '多语言', '子文档', '更新时间', '操作']);
    ...
    assert.deepEqual(await page.locator('.content-article-list thead th').allTextContents(), ['文档标题／唯一编号', '所属导航', '多语言', '更新时间', '操作']);
