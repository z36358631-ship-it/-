...
- [ ] 按 `/to-prd` 八章主体加第九章输出。
- [ ] C 端与 B 端分别使用一张大表，截图均放在“图示”列。
- [ ] 产品规则只在详细设计写一次，边界、验收和评审仅保留差异项。
- [ ] 回填固定 SHA 的 jsDelivr 图片地址。

### Task 4: 发布验收

- [ ] 校验 Markdown 图片数等于固定 SHA 地址数等于远程通过数。
- [ ] 逐图校验 HTTP 200、`Content-Type: image/png`、文件大小大于0。
