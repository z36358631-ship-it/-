# 【Prd】《盖世游戏 Mac》macOS 26 全屏启动台需求 V2.3
...
| 2026.07.22 | V2.3 | 产品 | <mark><span style="color:#1677ff"><strong>文件夹补齐右上角关闭和应用移出按钮；拖放建夹或移入后停留当前页；程序坞引导补齐图标、标题、副标题、确认和关闭，并按macOS卡片风格展示</strong></span></mark> | <mark>当前开发与验收版本，替代V2.2正文口径</mark> |
...
- <mark><span style="color:#1677ff"><strong>V2.3：全部应用直接展示、名称排序、横向分页、文件夹归类、搜索、长按编辑、移到废纸篓、原生应用启动、设置、应用来源、全局唤起、独立程序坞入口及macOS风格引导。</strong></span></mark>
...
| 模块名称 | 图示 | 展示&交互说明 |
|---|---|---|
| F001 侧边栏入口 | Demo“客户端与入口” | ① macOS 26及以上在侧边栏显示九宫格图标，低版本隐藏。<br>② 入口仅显示图标，不显示固定名称；悬停提示“启动台”。<br>③ <mark><span style="color:#1677ff"><strong>点击后直接展示当前显示器的全部应用网格，不打开普通内容页，也不经过扫描页。</strong></span></mark><br>④ 退出后恢复进入前的盖世页面和侧边栏选中状态。 |
| F002 应用发现 | Demo“全部应用/应用来源” | ① 从 `/Applications`、`/System/Applications`、`/System/Applications/Utilities`、`~/Applications` 发现应用。<br>② 只展示可启动 `.app`，过滤 Helper、Updater、Renderer、Crash Handler、Uninstaller和后台服务。<br>③ Bundle ID相同按标准目录和可启动性去重；同名但Bundle ID不同的应用保留。<br>④ <mark><span style="color:#1677ff"><strong>用户进入后直接看到全部应用；后台发现和增量更新不展示骨架、进度或扫描文案。</strong></span></mark><br>⑤ 监听应用安装、卸载和路径变化；监听缺失时下次进入校正。 |
| F003 网格、分页与搜索 | Demo“全部应用启动台/搜索与无结果” | ① 进入后直接展示全部应用，不显示“游戏/全部应用”页签。<br>② 使用应用原始图标和本地化名称；每页容量根据显示器空间计算。<br>③ 支持鼠标滚轮、触控板横滑、方向键和底部分页圆点切页。<br>④ 底部显示总页数和当前页；退出再进入时恢复上次页码。<br>⑤ 搜索覆盖所有页面和文件夹内应用，按名称包含匹配，不区分大小写和首尾空格。<br>⑥ 搜索结果展开为应用图标；<mark><span style="color:#1677ff"><strong>无结果只显示“未找到应用”和当前搜索词，不显示“清空搜索”按钮。</strong></span></mark><br>⑦ 有搜索词时Esc先清空，无搜索词时Esc退出。 |
| F004 文件夹 | Demo“文件夹” | ① 拖到另一应用上创建文件夹；默认名称“文件夹”。<br>② 拖到已有文件夹时移入文件夹；拖到空白处、其他位置或页面边缘不改变顺序。<br>③ <mark><span style="color:#1677ff"><strong>创建或移入成功后停留在当前启动台页，不自动打开文件夹；用户再次点击文件夹后才打开。</strong></span></mark><br>④ 不支持页内自由排序和跨页搬动；系统“实用工具”文件夹固定在首位，其余顶层应用与用户文件夹按当前客户端语言的本地化名称升序混排。<br>⑤ `/System/Applications/Utilities`中的应用首次发现后默认归入“实用工具”。<br>⑥ <mark><span style="color:#1677ff"><strong>不显示独立改名按钮；点击文件夹名称直接进入输入状态。</strong></span></mark>名称去除首尾空格后为1—20个字符；Enter或失焦保存，Esc取消。<br>⑦ <mark><span style="color:#1677ff"><strong>文件夹封面以3×3展示完整应用图标，不裁成色块；文件夹内应用保持完整图标比例。</strong></span></mark><br>⑧ <mark><span style="color:#1677ff"><strong>文件夹层右上角显示关闭按钮，每个应用显示“移出”；点击“移出”后应用返回顶层并按名称排序。点击右上角关闭、外部模糊区域或按Esc均可关闭文件夹。</strong></span></mark><br>⑨ 文件夹只剩一个应用时自动解散；拖放期间应用被卸载时取消操作并刷新。 |
| F005 启动与激活 | Demo“调用本机应用/启动失败” | ① 单击应用即发起启动，同一时间只允许一个请求。<br>② <mark><span style="color:#1677ff"><strong>调用 `NSWorkspace.openApplication` 打开未运行应用，通过 `NSRunningApplication` 激活已运行应用。</strong></span></mark><br>③ 启动中显示进度并阻止重复点击。<br>④ completion返回成功后目标应用置前，启动台退到后层并保留页码、文件夹状态。<br>⑤ <mark><span style="color:#1677ff"><strong>Demo只提示已调用macOS，不在内部展示目标应用页面，也没有返回按钮。</strong></span></mark><br>⑥ 点击“盖世游戏”图标时激活盖世窗口。<br>⑦ 启动失败、系统拒绝或超时后留在原页，展示原因和重试。 |
...
| F007 设置与来源 | Demo“设置与应用来源” | ① <mark><span style="color:#1677ff"><strong>右上角更多菜单依次包含“设置”“添加到程序坞/已添加到程序坞”和“退出”。</strong></span></mark><br>② 设置页同时保留“添加到程序坞”；未添加时可操作，已添加时显示“已添加”，检测到用户移除后恢复按钮。<br>③ 设置同时展示快捷键、F4、触控板、触发角、权限状态和应用来源。<br>④ 点击“去授权”打开对应系统设置，回到盖世后刷新状态。<br>⑤ 自定义目录通过系统目录选择器添加；同一路径不可重复添加。<br>⑥ 标准目录不可删除；自定义目录可移除，移除不卸载应用。<br>⑦ 目录无权限或已失效时提供“重新授权”和“移除”。<br>⑧ 所有设置变更立即保存并生效。 |
| F008 多显示器与状态 | Demo“异常&边界” | ① 侧边栏进入时使用盖世窗口所在显示器；全局入口使用鼠标所在显示器。<br>② 同一时间只显示一个启动台；跨显示器触发时迁移现有窗口。<br>③ 显示器断开时迁移到主显示器并重新计算分页。<br>④ <mark><span style="color:#1677ff"><strong>直接展示、后台更新、无应用、搜索无结果、编辑、删除确认、删除拦截、启动中、启动失败、目录无权限、入口无权限和设备不支持均有明确状态。</strong></span></mark> |
| F011 长按编辑与删除 | Demo“长按编辑模式/删除二次确认/运行中删除拦截” | ① <mark><span style="color:#1677ff"><strong>静止长按650毫秒进入编辑，图标轻微错峰抖动；按下后位移超过6点则取消长按并进入文件夹拖放。</strong></span></mark><br>② 可删除的第三方应用左上角显示“−”；系统、受保护或无删除权限应用不显示。<br>③ 运行中的第三方应用仍显示“−”，点击时即时检查运行状态。<br>④ 未运行时提示“确认要删除‘XX’吗？应用将被移到废纸篓。”，辅助说明“仅移动应用包，配置、存档和缓存会保留”，按钮为“取消”“删除”。<br>⑤ 已运行时提示“不能将项目‘XX’移到废纸篓，因为它已打开。”，只有“好”。<br>⑥ 用户确认时再次检查运行状态，并在移动完成前监听对应应用启动；检测到启动则中止，若文件已进入废纸篓则恢复原路径。<br>⑦ 运行态复核通过后移动 `.app` 到废纸篓并刷新分页。<br>⑧ 权限、SIP、只读卷、外置卷或恢复失败时保留图标和编辑模式，并说明原因；恢复失败上报高优先级错误并触发删除功能熔断。 |
| F012 程序坞入口 | Demo“全部应用启动台/设置与应用来源/客户端与入口” | ① <mark><span style="color:#1677ff"><strong>首次进入且尚未处理时，在搜索栏同行、更多按钮右侧展示一次引导。</strong></span></mark><br>② <mark><span style="color:#1677ff"><strong>引导采用macOS半透明深色卡片：左侧为盖世同款图标，中部展示标题“添加启动台到程序坞？”与副标题“点击图标可直接打开全屏启动台”，右侧为蓝色“确认”按钮和圆形关闭按钮。</strong></span></mark><br>③ 在用户处理前保持展示，不因翻页、搜索或文件夹自动消失；确认成功或关闭后不再自动展示，设置页和更多菜单仍保留添加入口。<br>④ 独立图标点击后直接请求盖世打开全屏启动台；盖世未运行时先拉起盖世，再打开启动台。<br>⑤ 启动器不展示独立窗口，完成请求后退出；添加失败时保留入口并提示原因。<br>⑥ <mark><span style="color:#1677ff"><strong>“确认后一键固定”属于技术准入项：排期前必须验证符合平台规则且稳定的实现方式；若只能修改Dock配置或重启Dock，不得上线伪成功，需返回产品重新确认手动保留方案。</strong></span></mark> |
...
| 搜索无结果 | “未找到应用”和当前搜索词，不显示操作按钮 | 修改关键词或按Esc清空 |
...
| 删除确认 | “确认要删除‘XX’吗？应用将被移到废纸篓。” | 取消或删除 |
| 删除运行中应用 | “不能将项目‘XX’移到废纸篓，因为它已打开。” | 点击“好”返回编辑模式 |
| 删除失败 | 保留应用和编辑模式，展示系统原因 | 关闭提示后继续操作 |
| 程序坞首次引导 | 搜索栏同行展示同款图标、标题、副标题、蓝色确认和圆形关闭按钮 | 确认添加或关闭引导 |
...
- <mark><span style="color:#1677ff"><strong>按下应用后，指针在650毫秒内移动超过6点则取消长按计时；未超过阈值且达到650毫秒才进入编辑模式。</strong></span></mark>
...
- <mark><span style="color:#1677ff"><strong>同一身份存在多个副本时，删除只处理当前展示的最高优先级副本；完成后重新发现应用，若还有可启动副本，则次级副本按名称重新出现。每个副本必须分别确认，不批量删除。</strong></span></mark>
...
- 滚轮、触控板、键盘和圆点均可切页。
- 搜索覆盖所有页面及文件夹内应用；按Esc后恢复原页和布局；<mark><span style="color:#1677ff"><strong>无结果页没有“清空搜索”按钮。</strong></span></mark>
- 顶层应用和文件夹内应用按本地化名称升序排列，不支持自由调整顺序或跨页搬动。
- 拖到应用或已有文件夹可创建或移入文件夹；<mark><span style="color:#1677ff"><strong>点击文件夹名称可直接编辑，不出现独立改名按钮；3×3缩略图和文件夹内图标完整可见。</strong></span></mark>
- 新安装应用按名称插入；卸载应用后删除并重新生成分页。

### 8.4 编辑与删除

- 静止长按应用650毫秒后进入编辑模式，应用轻微抖动；位移超过6点时不进入编辑模式。
...
- 首次进入且尚未处理时，引导显示在搜索栏同行、更多按钮右侧，使用盖世App同款图标。
- 引导包含“确认”和关闭按钮；用户处理前保持展示，处理后不再自动出现。
- 确认成功后设置页显示“已添加”；关闭引导后设置页仍可执行“添加到程序坞”。
- 用户从程序坞移除图标后，设置页恢复可添加状态，一次性引导不重复出现。
- 每次打开设置、客户端重新激活或程序坞添加完成后检查一次固定状态，1秒内更新“已添加/添加到程序坞”；重复点击添加先检查现状，已存在时只刷新为“已添加”，不得插入重复图标。
...
- <mark><span style="color:#1677ff"><strong>V2.3已补充文件夹右上角关闭、应用移出、建夹或移入后停留当前页；程序坞引导补齐图标、标题、副标题、确认和关闭，更多菜单与设置均保留添加入口。</strong></span></mark>
