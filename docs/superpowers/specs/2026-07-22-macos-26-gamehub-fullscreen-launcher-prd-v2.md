# 【Prd】《盖世游戏 Mac》macOS 26 全屏启动台需求 V2.2
...
| 2026.07.22 | V2.2 | 产品 | <mark><span style="color:#1677ff"><strong>删除用户可见扫描过程；新增长按抖动、系统式删除、文件夹名称直接编辑、NSWorkspace原生打开和独立程序坞入口</strong></span></mark> | <mark>当前开发与验收版本，替代V2.1正文口径</mark> |
...
- <mark><span style="color:#1677ff"><strong>V2.2：全部应用直接展示、名称排序、横向分页、文件夹归类、搜索、长按编辑、移到废纸篓、原生应用启动、设置、应用来源、全局唤起及独立程序坞入口。</strong></span></mark>
...
| F004 文件夹 | Demo“文件夹” | ① 拖到另一应用上创建文件夹；默认名称“文件夹”。<br>② 拖到已有文件夹时移入文件夹；拖到空白处、其他位置或页面边缘不改变顺序。<br>③ 不支持页内自由排序和跨页搬动；系统“实用工具”文件夹固定在首位，其余顶层应用与用户文件夹按当前客户端语言的本地化名称升序混排。<br>④ `/System/Applications/Utilities`中的应用首次发现后默认归入“实用工具”。<br>⑤ <mark><span style="color:#1677ff"><strong>不显示独立改名按钮；点击文件夹名称直接进入输入状态。</strong></span></mark>名称去除首尾空格后为1—20个字符；Enter或失焦保存，Esc取消。<br>⑥ <mark><span style="color:#1677ff"><strong>文件夹封面以3×3展示完整应用图标，不裁成色块；文件夹内应用保持完整图标比例。</strong></span></mark><br>⑦ 文件夹内应用按本地化名称升序排列；支持通过“移出”操作返回顶层。<br>⑧ 文件夹只剩一个应用时自动解散；拖放期间应用被卸载时取消操作并刷新。 |
...
| F007 设置与来源 | Demo“设置与应用来源” | ① 右上角更多菜单只包含“设置”和“退出”。<br>② <mark><span style="color:#1677ff"><strong>设置增加“添加到程序坞”；未添加时可操作，已添加时显示“已添加”，检测到用户移除后恢复按钮。</strong></span></mark><br>③ 设置同时展示快捷键、F4、触控板、触发角、权限状态和应用来源。<br>④ 点击“去授权”打开对应系统设置，回到盖世后刷新状态。<br>⑤ 自定义目录通过系统目录选择器添加；同一路径不可重复添加。<br>⑥ 标准目录不可删除；自定义目录可移除，移除不卸载应用。<br>⑦ 目录无权限或已失效时提供“重新授权”和“移除”。<br>⑧ 所有设置变更立即保存并生效。 |
...
| F012 程序坞入口 | Demo“全部应用启动台/设置与应用来源/客户端与入口” | ① <mark><span style="color:#1677ff"><strong>首次进入且尚未处理时，在搜索栏同行、更多按钮右侧展示一次引导。</strong></span></mark><br>② 引导使用与盖世App相同的图标，包含“确认”和关闭按钮；在用户处理前保持展示，不因翻页、搜索或文件夹自动消失。<br>③ 确认成功或关闭后不再自动展示；设置页始终保留添加入口。<br>④ 独立图标点击后直接请求盖世打开全屏启动台；盖世未运行时先拉起盖世，再打开启动台。<br>⑤ 启动器不展示独立窗口，完成请求后退出；添加失败时保留入口并提示原因。<br>⑥ <mark><span style="color:#1677ff"><strong>“确认后一键固定”属于技术准入项：排期前必须验证符合平台规则且稳定的实现方式；若只能修改Dock配置或重启Dock，不得上线伪成功，需返回产品重新确认手动保留方案。</strong></span></mark> |
...
| 程序坞首次引导 | 搜索栏同行展示同款图标、确认和关闭 | 确认添加或关闭引导 |
...
| `launcher_folder_action` | 文件夹操作 | 创建、名称编辑、移入、移出或解散 | `action`, `app_count` |
