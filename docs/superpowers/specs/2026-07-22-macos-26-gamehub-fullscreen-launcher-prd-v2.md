# 【Prd】《盖世游戏 Mac》macOS 26 全屏启动台需求 V2.4
...
| 2026.07.22 | V2.4 | 产品 | <mark><span style="color:#1677ff"><strong>建夹固定替换目标槽位，后续应用前移；文件夹封面图标改为等比方形；程序坞引导删除左侧图标和副标题；更多菜单删除程序坞项，设置内改用开关并默认关闭后台驻留</strong></span></mark> | <mark>当前开发与验收版本，替代V2.3正文口径</mark> |
...
- <mark><span style="color:#1677ff"><strong>V2.4：全部应用直接展示、初始名称排序、横向分页、文件夹原槽位归类、搜索、长按编辑、移到废纸篓、原生应用启动、设置、应用来源、全局唤起、独立程序坞入口及精简macOS风格引导。</strong></span></mark>
...
| F004 文件夹 | Demo“文件夹” | ① 拖到另一应用上创建文件夹；默认名称“文件夹”。<br>② <mark><span style="color:#1677ff"><strong>新文件夹直接替换被放置的目标应用槽位；拖动源从原槽位移除，两个应用合并为一个文件夹项，目标位置之后的应用依次前移一格。</strong></span></mark><br>③ 拖到已有文件夹时移入文件夹，文件夹位置保持不变；拖到空白处、其他位置或页面边缘不改变顺序。<br>④ 创建或移入成功后停留在当前启动台页，不自动打开文件夹；用户再次点击文件夹后才打开。<br>⑤ 不支持页内自由排序和跨页搬动；首次布局按本地化名称升序生成，创建、移入、移出或改名后保留当前槽位，不按文件夹名称重新排序。<br>⑥ `/System/Applications/Utilities`中的应用首次发现后默认归入“实用工具”。<br>⑦ 不显示独立改名按钮；点击文件夹名称直接进入输入状态。名称去除首尾空格后为1—20个字符；Enter或失焦保存，Esc取消。<br>⑧ <mark><span style="color:#1677ff"><strong>文件夹封面以3×3展示14×14等比方形应用缩略图，不拉伸成长条、不裁成色块；文件夹内应用保持完整图标比例。</strong></span></mark><br>⑨ 文件夹层右上角显示关闭按钮，每个应用显示“移出”；点击“移出”后应用放在文件夹后一位。点击右上角关闭、外部模糊区域或按Esc均可关闭文件夹。<br>⑩ 文件夹只剩一个应用时自动解散，剩余应用占原文件夹槽位，移出应用紧随其后；拖放期间应用被卸载时取消操作并刷新。 |
...
| F007 设置与来源 | Demo“设置与应用来源” | ① <mark><span style="color:#1677ff"><strong>右上角更多菜单只包含“设置”和“退出”，不重复展示“添加到程序坞”。</strong></span></mark><br>② “保持盖世后台驻留”默认关闭，用户主动打开后才在关闭主窗口时保持后台驻留。<br>③ 设置页“添加到程序坞”使用与后台驻留相同的开关：打开执行添加，关闭执行移除；检测到用户从程序坞手动移除后，开关在1秒内同步为关闭。<br>④ 设置同时展示快捷键、F4、触控板、触发角、权限状态和应用来源。<br>⑤ 点击“去授权”打开对应系统设置，回到盖世后刷新状态。<br>⑥ 自定义目录通过系统目录选择器添加；同一路径不可重复添加。<br>⑦ 标准目录不可删除；自定义目录可移除，移除不卸载应用。<br>⑧ 目录无权限或已失效时提供“重新授权”和“移除”。<br>⑨ 所有设置变更立即保存并生效。 |
...
| F011 长按编辑与删除 | Demo“长按编辑模式/删除二次确认/运行中删除拦截” | ① <mark><span style="color:#1677ff"><strong>静止长按650毫秒进入编辑，图标轻微错峰抖动；按下后位移超过6点则取消长按并进入文件夹拖放。</strong></span></mark><br>② 可删除的第三方应用左上角显示“−”；系统、受保护或无删除权限应用不显示。<br>③ 运行中的第三方应用仍显示“−”，点击时即时检查运行状态。<br>④ <mark><span style="color:#1677ff"><strong>未运行时使用收窄弹窗：主提示仅为“确认要删除‘XX’吗？”，下方辅助说明为“应用将被移到废纸篓。”，不展示配置、存档和缓存说明；按钮为“取消”“删除”。</strong></span></mark><br>⑤ 已运行时提示“不能将项目‘XX’移到废纸篓，因为它已打开。”，只有“好”。<br>⑥ 用户确认时再次检查运行状态，并在移动完成前监听对应应用启动；检测到启动则中止，若文件已进入废纸篓则恢复原路径。<br>⑦ 运行态复核通过后移动 `.app` 到废纸篓并刷新分页。<br>⑧ 权限、SIP、只读卷、外置卷或恢复失败时保留图标和编辑模式，并说明原因；恢复失败上报高优先级错误并触发删除功能熔断。 |
| F012 程序坞入口 | Demo“全部应用启动台/设置与应用来源/客户端与入口” | ① <mark><span style="color:#1677ff"><strong>首次进入且尚未处理时，在搜索栏同行、更多按钮右侧展示一次引导。</strong></span></mark><br>② <mark><span style="color:#1677ff"><strong>引导采用macOS半透明深色卡片，只展示标题“添加启动台到程序坞？”、蓝色“确认”按钮和圆形关闭按钮；不展示左侧图标，不展示“点击图标可直接打开全屏启动台”等副标题。</strong></span></mark><br>③ 在用户处理前保持展示，不因翻页、搜索或文件夹自动消失；确认成功或关闭后不再自动展示，后续仅可在设置页通过开关添加或移除。<br>④ 独立图标点击后直接请求盖世打开全屏启动台；盖世未运行时先拉起盖世，再打开启动台。<br>⑤ 启动器不展示独立窗口，完成请求后退出；添加失败时保留入口并提示原因。<br>⑥ <mark><span style="color:#1677ff"><strong>“确认后一键固定”属于技术准入项：排期前必须验证符合平台规则且稳定的实现方式；若只能修改Dock配置或重启Dock，不得上线伪成功，需返回产品重新确认手动保留方案。</strong></span></mark> |
...
| BR-04 | 首次布局按当前客户端语言的本地化名称升序生成，系统“实用工具”文件夹固定在首位；用户创建文件夹后，以槽位顺序为准持久化，不因文件夹改名或成员变化重新排序。同名项在首次布局时按稳定应用主键升序。 |
| BR-05 | 新安装应用在未归类项中按名称寻找插入位置，但不得改变已存在用户文件夹槽位；卸载应用后删除对应项，后续应用自然前移。 |
| BR-06 | 分辨率变化只改变页容量，不改变已持久化槽位顺序和文件夹归属。 |
...
- 新安装应用在未归类项中按名称插入，不改变用户文件夹槽位；卸载应用后删除，后续项目自然前移并重新生成分页。
...
- 文件夹拖放期间发生应用卸载、扫描更新、分辨率变化或显示器断开时，网格冻结；拖放结束后按当前槽位重建分页，不按文件夹名称重排，且当前页不越界。
...
- 已补充初始名称排序、用户槽位持久化、分页容量变化、空状态、加载状态和搜索无结果。
...
- V2.3已补充文件夹右上角关闭、应用移出、建夹或移入后停留当前页。
- <mark><span style="color:#1677ff"><strong>V2.4已将文件夹固定在目标槽位并修正等比缩略图；程序坞引导仅保留标题、确认和关闭；更多菜单删除程序坞项，设置中使用开关，后台驻留默认关闭。</strong></span></mark>
