...
async function main() {
  ...
  const thirdReviewSourceChecks = [
    ...
    ['会员三项权益介绍', templateSource.includes('renderMembershipValue') && templateSource.includes('MEMBERSHIP_BENEFITS') && templateSource.includes('会员库内畅玩') && templateSource.includes('游戏持续更新') && templateSource.includes('个人云存档同步') && !templateSource.includes('PC引擎与手柄适配')],
    ['首页 Banner 首租与在租信息', templateSource.includes('hero-rental-price') && templateSource.includes("id: 'trial'") && templateSource.includes("durationLabel: '2小时'") && templateSource.includes('price: 1.9') && templateSource.includes('hero-rental-demand') && templateSource.includes('99+ 在租')],
    ['售后区分退款与换号', templateSource.includes('AFTER_SALES_REQUEST_TYPES') && templateSource.includes('AFTER_SALES_REASONS') && templateSource.includes("['no-reason', '3天无理由']")],
  ];
  ...
  const finalReviewSourceChecks = [
    ['详情F操作模型', templateSource.includes('resolveDetailActions') && templateSource.includes("label: '更多'") && templateSource.includes('立即购买')],
    ...
    ['会员游戏卡仅名称与版本', templateSource.includes('renderMemberGameGrid') && templateSource.includes('member-game-version')],
    ['首期只售权益方案', templateSource.includes("saleMode: GAME_SALE_MODES.ENTITLEMENT") && templateSource.includes("defaultSku: 'permanent'") && templateSource.includes("label: '首次体验'") && templateSource.includes("label: '单游戏永久'")],
  ...
  const sixthReviewSourceChecks = [
    ...
    ['详情四槽动作', templateSource.includes("label: '更多'") && templateSource.includes("icon: 'more'") && templateSource.includes("icon: 'quick-play'") && templateSource.includes('detail-action-icon')],
    ...
    ['会员套餐默认周卡且无推荐角标', templateSource.includes("memberPlan: 'weekly'") && !templateSource.includes('membership-plan-badge')],
    ['双端登录成功进入游戏详情', templateSource.includes("showToast('登录成功，已进入游戏详情')") && !templateSource.includes('成功后返回游戏库')],
  ];
  ...
  const eighthReviewSourceChecks = [
    ...
    ['确认订单底部支付栏', templateSource.includes('需支付 ¥${amount}') && templateSource.includes("'立即购买'")],
    ...
    ['一键上号成功进入游戏详情', templateSource.includes("showToast('登录成功，已进入游戏详情')") && templateSource.includes("navigate('detail', { rememberSource: false, replaceTask: true })") && !templateSource.includes('一键上号失败')],
    ['Steam登录前移除令牌占位提示', templateSource.includes("if (!state.steamForm.requiresGuard) return ''") && templateSource.includes('获取并输入验证码') && !templateSource.includes('提交账号密码后获取令牌')],
  ];
  ...
  const playPcRentalSourceChecks = [
    ...
    ['发现列表三态契约', templateSource.includes('data-discovery-game-card') && templateSource.includes('data-discovery-rental-state') && templateSource.includes('first-rental') && templateSource.includes('rented') && templateSource.includes('playable')],
    ['仅PC游戏展示', templateSource.includes("state.playTab === 'pc'")],
    ['列表无独立操作热区', !templateSource.includes('data-action="play-card-action"')],
  ...
  try {
    ...
    await runRefactorGate('CATALOG_INTEGRITY', async () => {
      ...
      const catalog = await page.evaluate(() => {
        ...
        const checkoutProductEdition = document.querySelector('.checkout-product-edition')?.textContent.trim();
      ...
      checkCatalog(catalog.checkoutEditionControls === 0 && catalog.checkoutProductName === '影之刃零' && catalog.checkoutProductEdition === '标准版', `确认订单商品名与标准版副标题未分层：${JSON.stringify({ controls: catalog.checkoutEditionControls, name: catalog.checkoutProductName, edition: catalog.checkoutProductEdition })}`);
      checkCatalog(catalog.freeActions.map(({ label }) => label).join('|') === '更多|秒玩|获取游戏' && !catalog.freeActions.some(({ id }) => id === 'rent'), `免费获取场景没有正确隐藏租号：${JSON.stringify(catalog.freeActions)}`);
      checkCatalog(catalog.cloudBadges.length === 0 && catalog.unsupportedBadges === 0, `会员游戏卡仍显示云存档标签：${JSON.stringify(catalog.cloudBadges)}`);
      checkCatalog(catalog.cloudBadges.length === 0, '横屏会员游戏预览仍显示云存档标签');
      checkCatalog(catalog.firstRentalText === '¥1.9首租' && catalog.regularRentalText === '¥198.0租号', `首页首租资格失效后未回退当前最低可购权益：${JSON.stringify({ first: catalog.firstRentalText, regular: catalog.regularRentalText })}`);
    ...
    await runRefactorGate('DISCOVERY_DISPLAY_MODEL', async () => {
      ...
      checkDiscovery(
        discovery.models.rentalPrice.displayType === 'rental-price'
          && discovery.models.rentalPrice.displayText === '¥198.0 · 租号',
        `无权益时租号价格文案错误：${JSON.stringify(discovery.models.rentalPrice)}`,
      );
      checkDiscovery(
        discovery.models.rentalPrice.rawAmount === 198
          && discovery.models.rentalPrice.formattedAmount === '198.0',
      ...
      checkDiscovery(
        discovery.ineligibleFirstPrice.displayType === 'rental-price'
          && discovery.ineligibleFirstPrice.rawAmount === 68
          && discovery.ineligibleFirstPrice.displayText === '¥68.0 · 租号',
        `首次体验资格失效后未回退单游戏永久最低价：${JSON.stringify(discovery.ineligibleFirstPrice)}`,
      );
      checkDiscovery(
    ...
    await runRefactorGate('PLAY_PC_RENTAL', async () => {
      ...
      const portraitStates = await page.evaluate(() => ({
        ...
        screen: window.__appRentalDemo.snapshot().screen,
        types: [...document.querySelectorAll('[data-discovery-rental-state]')].map((node) => node.dataset.discoveryRentalState).sort(),
        texts: [...document.querySelectorAll('[data-discovery-rental-state]')].map((node) => node.innerText.split(/\n+/).map((line) => line.trim()).filter(Boolean)),
        playText: document.querySelector('#appRentalDemo')?.innerText || '',
        cardCount: document.querySelectorAll('[data-discovery-game-card]').length,
        nestedActionCount: [...document.querySelectorAll('[data-discovery-game-card]')]
          .reduce((count, card) => count + card.querySelectorAll('button, [role="button"], [data-action]').length, 0),
      }));
      assert(portraitStates.screen === 'play' && portraitStates.playTab === 'pc', `PC游戏截图态错误：${JSON.stringify(portraitStates)}`);
      assert(JSON.stringify([...new Set(portraitStates.types)].sort()) === JSON.stringify(['first-rental', 'playable', 'rented']), `竖屏三类列表状态错误：${JSON.stringify(portraitStates)}`);
      assert(portraitStates.texts.some((lines) => JSON.stringify(lines) === JSON.stringify(['¥1.9 首租', '99+ 在租'])) && portraitStates.texts.some((lines) => JSON.stringify(lines) === JSON.stringify(['已租号', '99+ 在租'])) && portraitStates.texts.some((lines) => JSON.stringify(lines) === JSON.stringify(['可畅玩', '99+ 在租'])), `竖屏三态两行文案错误：${JSON.stringify(portraitStates.texts)}`);
      assert(!portraitStates.playText.includes('剩余') && !portraitStates.playText.includes('账号分配') && !/(登录游戏|下载游戏|启动游戏|租号开玩|立即购买)/.test(portraitStates.playText), `PC游戏发现卡暴露了禁用信息或操作：${portraitStates.playText}`);
      assert(portraitStates.cardCount === 4 && portraitStates.nestedActionCount === 0, `PC游戏卡数量或独立操作热区错误：${JSON.stringify(portraitStates)}`);
      ...
      await page.locator('[data-discovery-game-card][data-game-id="hogwarts"]').click();
      ...
      const landscapeGeometry = await page.evaluate(() => {
        ...
        const cards = [...document.querySelectorAll('.landscape-play [data-discovery-game-card]')];
        return cards.map((card) => {
          const cardRect = card.getBoundingClientRect();
          const summaryRect = card.querySelector('[data-discovery-rental-state]')?.getBoundingClientRect();
          ...
          return {
            ...
            nestedActionCount: card.querySelectorAll('button, [role="button"], [data-action]').length,
          };
        });
      });
      assert(landscapeGeometry.length === 4 && landscapeGeometry.every((item) => item.cardInsideDevice && item.summaryInsideCard && item.nameSummarySeparated && item.nestedActionCount === 0), `横屏卡片越界、文字重叠或仍有独立操作：${JSON.stringify(landscapeGeometry)}`);
      process.stdout.write('PLAY_PC_RENTAL 14/14 PASS\n');
    });
    ...
    await runRefactorGate('CDKEY_VISUAL_CONVERGENCE', async () => {
      ...
      await page.evaluate(() => window.__appRentalDemo.selectRentalSku('trial'));
      ...
      checkVisual(
        detailVisual.every(({ primaryBackground, primaryHasBlue, forbiddenBusinessCopy }) => (
          ...
          && detailConfirmed.snapshot.order?.durationLabel === '长期有效'
          && detailConfirmed.snapshot.order?.rawAmount === 198
          && detailConfirmed.snapshot.selectedSku === 'permanent'
          && detailConfirmed.saleMode === 'entitlement'
          && detailConfirmed.text.includes('影之刃零')
          && detailConfirmed.text.includes('标准版')
          ...
          && ['trial', 'permanent', 'membership'].every((kind) => detailConfirmed.skuKinds.includes(kind))
          && detailSelected.snapshot.order?.durationLabel === '2小时'
          && detailSelected.snapshot.order?.rawAmount === 1.9
          && detailSelected.text.includes('¥1.90')
      ...
      const checkoutFields = await page.evaluate(() => {
        ...
        return {
          text,
          fields: ['影之刃零', '标准版', '权益方案', '游戏原价', '订单金额', '支付方式', '需支付', '立即购买'].every((field) => text.includes(field)),
          removed: ['当前报价', '租赁信息', '租号服务协议', '支付有效期', '扫码支付'].every((field) => !text.includes(field)),
      ...
      checkVisual(
        checkoutVisual.every(({ primaryBackground, primaryHasBlue, forbiddenBusinessCopy }) => (
          ...
          && checkoutFields.fields && checkoutFields.removed && checkoutFields.editionControls === 0 && checkoutFields.twoDecimalAmount
    ...
    await runRefactorGate('FINAL_CHECKOUT_AMOUNT_LAYOUT', async () => {
      ...
      const readPortraitAmounts = () => page.evaluate(() => {
        ...
        const domOrder = [packagePanel, summary, paymentPanel];
        return {
      ...
      for (const [index, stateView] of portraitStates.entries()) {
        ...
        checkCheckout(stateView.domOrdered, `竖屏状态${index + 1} 未按权益方案→金额→支付方式排列`);
      ...
      checkCheckout(new Set(selectedAmounts).size === 2 && selectedAmounts.includes(198) && selectedAmounts.includes(1.9), `单游戏永久与首次体验金额未随权益选择变化：${JSON.stringify(selectedAmounts)}`);
      ...
      const landscapeCheckout = await page.evaluate(() => {
        ...
        return {
          ...
          leftHasAmounts: Boolean(left?.querySelector('[data-checkout-amount-summary]')),
      ...
      checkCheckout(
        ...
        `横屏右栏未按权益方案→金额→支付方式→支付栏排列：${JSON.stringify(landscapeCheckout)}`,
      );
    ...
    await runRefactorGate('FINAL_RENTAL_BENEFITS', async () => {
      ...
      assert(result.timeRental.visible && result.timeRental.title === '租号权益' && result.timeRental.summary === '' && JSON.stringify(result.timeRental.items) === JSON.stringify(expected), `时租未恢复上一版五项租号权益：${JSON.stringify(result.timeRental)}`);
    ...
    await runRefactorGate('FINAL_TRIAL_PAYMENT', async () => {
      ...
      const result = await page.evaluate((paymentTime) => {
        const api = window.__appRentalDemo;
        const missing = ['simulateTrialPayment', 'prepareAccountForLaunch']
          .filter((name) => typeof api[name] !== 'function');
        if (missing.length) return { missing };
        const trial = api.simulateTrialPayment(paymentTime);
        const beforePrepare = api.snapshot().accountAllocationCount;
        const firstPrepare = api.prepareAccountForLaunch({ orderId: trial.id, launchRequestId: 'TRIAL-LAUNCH-001' });
        const afterFirstPrepare = api.snapshot().accountAllocationCount;
        const duplicatePrepare = api.prepareAccountForLaunch({ orderId: trial.id, launchRequestId: 'TRIAL-LAUNCH-001' });
        const afterDuplicatePrepare = api.snapshot().accountAllocationCount;
        api.setSelectedGame('spiritfarer', { shouldRender: false });
        const spiritfarerContext = api.getCheckoutEligibilityContext('spiritfarer');
        const spiritfarerSkus = api.eligibleCheckoutSkus('spiritfarer', spiritfarerContext).map(({ id }) => id);
        api.setSelectedGame('shadow-blade-zero', { shouldRender: false });
        const shadowContext = api.getCheckoutEligibilityContext('shadow-blade-zero');
        const shadowSkus = api.eligibleCheckoutSkus('shadow-blade-zero', shadowContext)
          .map(({ id, price, firstOnly }) => ({ id, price, firstOnly: Boolean(firstOnly) }));
        const shadowDisplay = api.getDiscoveryDisplay('shadow-blade-zero');
        const visibleText = document.querySelector('#appRentalDemo')?.innerText || '';
        return {
          trial,
          firstPrepare,
          duplicatePrepare,
      ...
      assert(
        trial.firstRentalEligible === false
          && trial.platformTrialEligible === false
          && result.spiritfarerContext.firstRentalEligible === false
          && !result.spiritfarerSkus.includes('trial')
          && result.shadowContext.firstRentalEligible === false
          && result.shadowSkus.every(({ firstOnly }) => !firstOnly)
          && result.shadowSkus.some(({ id, price }) => id === 'permanent' && Number(price) === 198)
          && result.shadowDisplay.displayText === '¥198.0 · 租号',
        `首次体验支付成功后未消耗平台级终身资格或其他游戏仍展示首体验价：${JSON.stringify(result)}`,
      );
      assert(
        result.firstPrepare.ok === true
    ...
    await runRefactorGate('FINAL_MEMBER_INTRO', async () => {
      ...
      checkMemberIntro(firstVisit.title === '关于会员' && firstVisit.items.length === 3, `关于会员标题或条目数量错误：${JSON.stringify(firstVisit)}`);
      checkMemberIntro(firstVisit.items[1]?.includes('不支持主动3天无理由退款') && firstVisit.items[1]?.includes('履约异常可申请人工售后'), `关于会员退款与售后说明错误：${firstVisit.fullText}`);
      checkMemberIntro(!firstVisit.fullText.includes('远程协助'), `关于会员仍保留已移除的远程协助说明：${firstVisit.fullText}`);
      ...
      process.stdout.write('FINAL_MEMBER_INTRO 9/9 PASS\n');
    });
      ...
      for (const orientation of ['portrait', 'landscape']) {
        for (const contractItem of FULL_PAGE_MATRIX) {
          ...
          const result = await page.evaluate(({ orientation: nextOrientation, contractItem: expected }) => {
            ...
            const rootNode = document.querySelector('#appRentalDemo');
            const renderedPageId = expected.renderedPageId || expected.pageId;
            const markerNode = rootNode?.dataset.pageId === renderedPageId
              ? rootNode
              : rootNode?.querySelector(`[data-page-id="${renderedPageId}"]`);
          ...
          const passed = !result.apiMissing
            ...
            && result.actualPageId === (contractItem.renderedPageId || contractItem.pageId)
    ...
    await runRefactorGate('APP_LIBRARY_CONVERGENCE', async () => {
      ...
      for (const orientation of ['portrait', 'landscape']) {
        await reloadDemo();
        const pcLibrary = await page.evaluate(({ nextOrientation, forbiddenCopies }) => {
          const api = window.__appRentalDemo;
          api.setOrientation(nextOrientation);
          const opened = api.openCaptureState('library');
          const tabs = [...document.querySelectorAll('[data-library-top-tab]')];
          const entitlementCards = [...document.querySelectorAll('[data-library-entitlement-card]')];
          return {
            ...
            entitlementCards: entitlementCards.map((node) => ({
              ...
              validityText: node.querySelector('.library-validity')?.textContent.trim() || '',
        ...
        check(pcLibrary.screen === 'library' && pcLibrary.libraryTab === 'pc', `${orientation} 游戏库截图态未默认进入PC游戏：${JSON.stringify(pcLibrary)}`);
        check(JSON.stringify(pcLibrary.tabs) === JSON.stringify(expectedTopTabs) && pcLibrary.selected === 'pc' && pcLibrary.singleLine, `${orientation} 游戏库顶层Tab顺序、选中态或单行布局错误：${JSON.stringify(pcLibrary.tabs)}`);
        check(pcLibrary.entitlementCards.length >= 2 && new Set(pcLibrary.entitlementCards.map(({ type }) => type)).size === 2, `${orientation} 游戏库未同时穷举首次体验与单游戏永久权益卡：${JSON.stringify(pcLibrary.entitlementCards)}`);
        const trialCard = pcLibrary.entitlementCards.find(({ type }) => type === 'trial');
        const permanentCard = pcLibrary.entitlementCards.find(({ type }) => type === 'permanent');
        check(
          trialCard?.gameId
            && trialCard.text.includes('标准版 · 已租号')
            && /^剩余(?:\d+天\d+小时|\d+小时\d+分)$/.test(trialCard.validityText)
            && trialCard.text.indexOf('标准版 · 已租号') < trialCard.text.indexOf(trialCard.validityText)
            && trialCard.validityCount === 1,
          `${orientation} 首次体验卡的版本／权益／有效期层级错误：${JSON.stringify(trialCard)}`,
        );
        check(
          permanentCard?.gameId
            && permanentCard.text.includes('标准版 · 单游戏永久')
            && permanentCard.validityCount === 0
            && !permanentCard.text.includes('剩余'),
          `${orientation} 单游戏永久卡错误显示倒计时或缺少权益：${JSON.stringify(permanentCard)}`,
        );
        ...
        const memberLibrary = await page.evaluate((forbiddenCopies) => {
          ...
          return {
            ...
            entitlementCardCount: document.querySelectorAll('.member-game-card[data-library-entitlement-card], .member-game-card .library-validity').length,
            actionCount: cards.reduce((count, node) => count + node.querySelectorAll('button, [role="button"], [data-action]').length, 0),
        ...
        for (const entryState of ['membership', 'membership-success', 'play']) {
          const entryResult = await page.evaluate((pageId) => {
            ...
            const entry = document.querySelector('[data-action="open-member-library"], [data-screen="member-library"]');
            entry?.click();
            const snapshot = api.snapshot();
            return {
              found: Boolean(entry),
        ...
        for (const surface of ['play', 'ranking']) {
          await page.evaluate((pageId) => window.__appRentalDemo.openCaptureState(pageId), surface);
          const discovery = await page.evaluate(({ expectedStates, forbiddenCopies }) => {
            const cards = [...document.querySelectorAll('[data-discovery-game-card]')];
            const states = cards.map((node) => {
              const stateNode = node.querySelector('[data-discovery-rental-state]');
              return {
                value: stateNode?.dataset.discoveryRentalState || '',
                lines: (stateNode?.innerText || '').split(/\n+/).map((line) => line.trim()).filter(Boolean),
                nestedActions: [...node.querySelectorAll('button, [role="button"], [data-action]')].filter((child) => child !== node).length,
              };
            });
            return {
              cardCount: cards.length,
              ...
              allCardsClickable: cards.every((node) => node.matches('button, a, [role="button"], [data-action]')),
              forbiddenActions: cards.filter((node) => /登录游戏|下载游戏|启动游戏|租号开玩|立即购买/.test(node.innerText)).length,
              forbiddenVisible: forbiddenCopies.filter((copy) => document.querySelector('#appRentalDemo')?.innerText.includes(copy)),
            };
          }, { expectedStates: expectedDiscoveryStates, forbiddenCopies: forbiddenVisibleStates });
          check(discovery.cardCount >= 3 && JSON.stringify(discovery.stateSet) === JSON.stringify(Object.keys(expectedDiscoveryStates).sort()), `${orientation} ${surface} 未穷举三种发现列表状态：${JSON.stringify(discovery)}`);
          check(discovery.labelsMatch, `${orientation} ${surface} 三态两行文案错误：${JSON.stringify(discovery.states)}`);
          check(discovery.states.every(({ nestedActions }) => nestedActions === 0) && discovery.forbiddenActions === 0 && discovery.allCardsClickable, `${orientation} ${surface} 列表仍含独立操作或无法整卡点击：${JSON.stringify(discovery)}`);
          check(discovery.forbiddenVisible.length === 0, `${orientation} ${surface} 暴露后台账号过程：${discovery.forbiddenVisible.join('、')}`);

          const beforeClick = await page.evaluate(() => {
            const snapshot = window.__appRentalDemo.snapshot();
            return {
              order: snapshot.order,
              rentalUsage: snapshot.rentalUsage,
              accountAllocationCount: snapshot.accountAllocationCount,
            };
          });
          await page.locator('[data-discovery-game-card]').first().click();
          ...
          check(
            afterClick.screen === 'detail'
              && JSON.stringify(afterClick.order) === JSON.stringify(beforeClick.order)
              && JSON.stringify(afterClick.rentalUsage) === JSON.stringify(beforeClick.rentalUsage)
              && afterClick.accountAllocationCount === beforeClick.accountAllocationCount,
            `${orientation} ${surface} 整卡点击未仅进入详情或产生了交易／账号副作用：${JSON.stringify({ beforeClick, afterClick })}`,
          );
        }
      }

      await reloadDemo();
      const defaultPc = await page.evaluate(() => {
        window.__appRentalDemo.navigate('library');
        const snapshot = window.__appRentalDemo.snapshot();
        return { screen: snapshot.screen, libraryTab: snapshot.libraryTab };
      });
      check(defaultPc.screen === 'library' && defaultPc.libraryTab === 'pc', `未开会员用户首次进入游戏库未默认PC游戏：${JSON.stringify(defaultPc)}`);

      await reloadDemo();
      const defaultMember = await page.evaluate(() => {
        window.__appRentalDemo.setMembershipStatus('weekly-active');
        window.__appRentalDemo.navigate('library');
        const snapshot = window.__appRentalDemo.snapshot();
        return { screen: snapshot.screen, libraryTab: snapshot.libraryTab };
      });
      ...
      const rememberedTab = await page.evaluate(() => {
        const api = window.__appRentalDemo;
        api.setLibraryTab('retro', { remember: true });
        api.navigate('home');
      ...
      check(rememberedTab.screen === 'library' && rememberedTab.libraryTab === 'retro', `用户主动选择的游戏库Tab未在返回和旋转后保留：${JSON.stringify(rememberedTab)}`);

      await page.evaluate(() => {
        window.__appRentalDemo.openMemberLibrary({ remember: true });
        const input = document.querySelector('[data-member-library-search-input]');
        input.value = 'Spiritfarer';
        input.dispatchEvent(new Event('input', { bubbles: true }));
      });
      await page.waitForTimeout(220);
      const memberSearchRetention = await page.evaluate(() => {
        const api = window.__appRentalDemo;
        api.setOrientation('landscape');
        api.setOrientation('portrait');
        const snapshot = api.snapshot();
        return {
          screen: snapshot.screen,
          libraryTab: snapshot.libraryTab,
          query: snapshot.memberLibrarySearch,
          input: document.querySelector('[data-member-library-search-input]')?.value || '',
          resultNames: [...document.querySelectorAll('.member-game-card > strong')].map((node) => node.textContent.trim()),
        };
      });
      check(
        ...
        `会员游戏搜索词或结果未在旋转后保留：${JSON.stringify(memberSearchRetention)}`,
      );

      const durationFormats = await page.evaluate(() => {
        const format = window.__appRentalDemo.formatLibraryRemaining;
        if (typeof format !== 'function') return null;
        const now = 1_800_000_000_000;
        return {
          within24h: format(now + 102 * 60_000, now),
          beyond24h: format(now + 26 * 60 * 60_000, now),
        };
      });
      check(
        durationFormats?.within24h === '剩余1小时42分' && durationFormats?.beyond24h === '剩余1天2小时',
        `游戏库有效期小时／天小时格式错误：${JSON.stringify(durationFormats)}`,
      );
      process.stdout.write(`APP_LIBRARY_CONVERGENCE ${checks}/${checks} PASS\n`);
    });
    ...
    await runRefactorGate('ORDER_CENTER_V2', async () => {
      ...
      const initial = await page.evaluate(() => {
        ...
        return {
          ...
          tabRects: tabNodes.map((node) => {
            const rect = node.getBoundingClientRect();
            return { left: Math.round(rect.left), top: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height) };
          }),
          tabsSingleLine: tabNodes.every((node) => node.scrollHeight <= node.clientHeight + 1 && getComputedStyle(node).whiteSpace === 'nowrap'),
      ...
      await page.waitForFunction(() => document.activeElement?.matches('[data-order-search-input]'));
      const expandedSearch = await page.evaluate(() => ({
        ...
        tabRects: [...document.querySelectorAll('[data-order-tab], .order-tabs [role="tab"]')].map((node) => {
          const rect = node.getBoundingClientRect();
          return { left: Math.round(rect.left), top: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height) };
        }),
        tabsSingleLine: [...document.querySelectorAll('[data-order-tab], .order-tabs [role="tab"]')].every((node) => node.scrollHeight <= node.clientHeight + 1 && getComputedStyle(node).whiteSpace === 'nowrap'),
        searchRightOfUsable: (() => {
          const search = document.querySelector('[data-order-search]')?.getBoundingClientRect();
          const usable = document.querySelector('[data-order-tab="usable"], .order-tabs [data-value="usable"]')?.getBoundingClientRect();
          return Boolean(search && usable && search.left >= usable.right - 1);
        })(),
      }));
      assert(
        expandedSearch.open
          && expandedSearch.aria.includes('当前')
          && expandedSearch.focused
          && expandedSearch.tabsSingleLine
          && expandedSearch.searchRightOfUsable
          && JSON.stringify(expandedSearch.tabRects) === JSON.stringify(initial.tabRects),
        `订单搜索展开导致Tab换行、位移或覆盖：${JSON.stringify({ initial: initial.tabRects, expanded: expandedSearch })}`,
      );
    ...
    await runRefactorGate('CROSS_GAME_CHECKOUT', async () => {
      ...
      const checkoutContexts = await page.evaluate(() => {
        const readCheckout = () => {
          ...
          return {
            ...
            coverLabel: document.querySelector('.checkout-cover[data-wireframe-media], .checkout-cover [data-wireframe-media]')?.getAttribute('aria-label') || '',
            wireframeCount: document.querySelectorAll('.checkout-cover[data-wireframe-media], .checkout-cover [data-wireframe-media]').length,
            imageCount: document.querySelectorAll('.checkout-cover img').length,
          };
        };
        window.__appRentalDemo.setScenario('not-member-library');
        window.__appRentalDemo.setOrientation('portrait');
        window.__appRentalDemo.setSelectedGame('elden-ring');
        window.__appRentalDemo.selectRentalSku('permanent');
      ...
      assert(
        checkoutContexts.first.gameId === 'elden-ring'
          && checkoutContexts.first.coverLabel === '艾尔登法环 游戏封面'
          && checkoutContexts.first.wireframeCount === 1
          && checkoutContexts.first.imageCount === 0
          ...
          && checkoutContexts.second.coverLabel === '影之刃零 游戏封面'
          && checkoutContexts.second.wireframeCount === 1
          && checkoutContexts.second.imageCount === 0
    ...
    await runRefactorGate('GAME_TRANSACTION_COMPLETION', async () => {
      ...
      const transaction = await page.evaluate(() => {
        ...
        const payment = {
          ...
          entitlementReady: /权益已生效|进入游戏库|开始畅玩/.test(document.querySelector('#appRentalDemo').innerText),
      ...
      assert(
        Boolean(transaction.paid)
          ...
          && transaction.payment.entitlementReady
          && !transaction.payment.forbiddenIntermediateAction,
        `游戏支付后未直接进入权益生效态：${JSON.stringify(transaction)}`,
      );
    });
    ...
    await runRefactorGate('MEMBERSHIP_COMPLETION', async () => {
      ...
      const membership = await page.evaluate(() => {
        ...
        return {
          ...
          validityCopy: /有效期至\s*\d{4}\.\d{2}\.\d{2}/.test(document.querySelector('#appRentalDemo').innerText),
        };
      });
      assert(
        membership.created === 'pending'
          && membership.hasCompletionApi
          && Boolean(membership.completed)
    ...
    await runRefactorGate('ROTATION_SCROLL_REGISTRY', async () => {
      const results = [];
      for (const [screen, requestedTop] of [['play', 35], ['library', 145], ['membership', 108]]) {
        await reloadDemo();
        ...
        const after = await page.evaluate(() => {
          const candidates = [...document.querySelectorAll(
            '[data-scroll-region="primary"], .landscape-membership-body, .device.landscape .landscape-content, .device.landscape .task-scroll-region',
          )];
        ...
        results.push({
          ...
          roundTrip,
          expected,
          retained: before.max === 0
            ? roundTrip.top === 0
            : before.top > 0 && Math.abs(after.top - expected) <= 2 && Math.abs(roundTrip.top - before.top) <= 2,
        });
      }
    ...
    await runRefactorGate('ADMIN_DUAL_CLIENT', async () => {
      ...
      try {
        ...
        assert(
          macPlans.includes('周卡')
            && macPlans.includes('月卡')
            && macPlans.includes('季卡')
            && !macPlans.includes('年度会员')
            && !macPlans.includes('永久会员'),
          `Mac 会员套餐未统一为周卡/月卡/季卡：${macPlans}`,
        );
        ...
        try {
          ...
          const linkedAdminPages = await unifiedPage.locator('.admin-nav[data-mode="admin"][data-page]').evaluateAll((nodes) => nodes.map((node) => node.dataset.page));
          assert(
            JSON.stringify(linkedAdminPages) === JSON.stringify(['products', 'accounts', 'admin-orders', 'stats']),
            `Mac 后台可用页面集合异常：${JSON.stringify(linkedAdminPages)}`,
          );
          const unifiedStates = [];
          for (const pageId of linkedAdminPages) {
          ...
          assert(
            unifiedStates.every(({ pageId, tabCount, activeClient }) => linkedAdminPages.includes(pageId)
              && tabCount === 0
              && activeClient === 'all'),
            `Mac 原后台链接未保持原页面或错误注入端别 Tab：${JSON.stringify(unifiedStates)}`,
          );
          ...
          const unifiedProductControls = await unifiedPage.evaluate(() => ({
            query: document.querySelectorAll('.admin-toolbar input.a-input[placeholder*="搜索"]').length,
            create: document.querySelectorAll('.admin-header [data-action="copy-product"]').length,
            edit: document.querySelectorAll('[data-action="toggle-product"]').length,
    ...
    const landscape = await page.evaluate(() => {
      ...
      const metrics = cards.map((node) => {
        const rect = node.getBoundingClientRect();
        const wireframe = node.querySelector('[data-wireframe-media]');
        return {
          top: Math.round(rect.top),
          left: Math.round(rect.left),
      ...
      return {
        ...
        secondRowWireframe: secondRow.some(({ visible, wireframe }) => visible && wireframe),
      };
    });
    ...
    const proportionalWireframes = await page.evaluate(() => [...document.querySelectorAll('.landscape-library [data-wireframe-media]')].map((node) => {
      ...
      return {
        width: rect.width,
        height: rect.height,
        contained: Boolean(parentRect && rect.left >= parentRect.left - 1 && rect.right <= parentRect.right + 1 && rect.top >= parentRect.top - 1 && rect.bottom <= parentRect.bottom + 1),
      };
    }));
    assert(
      proportionalWireframes.length >= 6
        && proportionalWireframes.every(({ width, height, contained }) => width > 0 && height > 0 && contained),
      '横屏线框媒体尺寸无效或溢出容器',
    );
    ...
    for (const screen of ['home', 'play', 'library', 'profile']) {
      ...
      landscapePages[screen] = await page.evaluate((value) => ({
        ...
        primaryCount: document.querySelectorAll('[data-primary-action="true"]').length,
        wireframes: [...document.querySelectorAll('[data-wireframe-media]')]
    ...
    assert(Object.values(landscapePages).every(({ wireframes, visibleImages }) => wireframes >= 1 && visibleImages === 0), '横屏页面未完整使用线框媒体');
    ...
    await page.evaluate(() => {
      ...
      window.__appRentalDemo.navigate('library');
      window.__appRentalDemo.setLibraryTab('retro');
    ...
    assert(
      rotationState.screen === 'profile'
        ...
        && rotationState.libraryTab === 'retro',
      '横竖屏切换丢失共享状态',
    );
    ...
    const entitlementCases = [
      ...
      ['not-member-library', 'permanent', ['membership']],
    ...
    const skuCases = [
      ['first-phase-shadow', 'member-library-trial', 'shadow-blade-zero', ['影之刃零', '标准版', '首次体验 · 2小时', '单游戏永久', '开会员畅玩'], ['按小时', '日租', '周租', '增强版', '豪华版'], ['trial', 'permanent', 'membership']],
      ['entitlement-trial', 'member-library-trial', 'spiritfarer', ['首次体验 · 2小时', '单游戏永久', '开会员畅玩'], ['按小时', '日租', '周租'], ['trial', 'permanent', 'membership']],
    ...
    for (const [caseName, scenario, gameId, present, absent, expectedKinds] of skuCases) {
      await page.evaluate(({ scenario, gameId }) => {
        ...
        api.setDiscoveryContext(gameId, { firstRentalEligible: scenario === 'member-library-trial' });
        api.setSelectedGame(gameId);
        api.navigate('detail');
    ...
    const portraitDetail = await page.evaluate(() => {
      ...
      return {
        ...
        hero: Boolean(document.querySelector('.portrait-detail-hero [data-wireframe-media]')),
        primaryCount: document.querySelectorAll('[data-primary-action="true"]').length,
        footerPinned: Boolean(frame && footer && Math.abs(frame.bottom - footer.bottom) <= 14),
        lightActions: [...document.querySelectorAll('.portrait-detail-actions .detail-action-light')].map((node) => {
          const icon = node.querySelector('svg')?.getBoundingClientRect();
          const label = node.querySelector('span')?.getBoundingClientRect();
          const style = getComputedStyle(node);
          return { background: style.backgroundColor, border: style.borderTopWidth, direction: style.flexDirection, iconAboveLabel: Boolean(icon && label && icon.bottom <= label.top + 1) };
        }),
      };
    });
    assert(portraitDetail.layout, '竖屏详情缺少独立布局');
    assert(portraitDetail.hero, '竖屏详情缺少线框主视觉');
    assert(portraitDetail.primaryCount === 1 && portraitDetail.footerPinned, '竖屏详情主按钮未固定或不唯一');
    assert(portraitDetail.lightActions.length === 2 && portraitDetail.lightActions.every(({ background, border, direction, iconAboveLabel }) => background === 'rgba(0, 0, 0, 0)' && border === '0px' && direction === 'column' && iconAboveLabel), `更多/秒玩必须为无背景的上图标下文字：${JSON.stringify(portraitDetail.lightActions)}`);
    ...
    const landscapeDetail = await page.evaluate(() => {
      ...
      return {
        ...
        game: Boolean(document.querySelector('.landscape-detail-game')),
        title: rect('.landscape-detail-copy h1'),
        intro: rect('.landscape-detail-copy > p'),
        tags: rect('.landscape-detail-genre'),
        actions: rect('.landscape-detail-copy .detail-action-set'),
        facts: rect('.landscape-detail-facts'),
        primaryCount: document.querySelectorAll('[data-primary-action="true"]').length,
      };
    });
    assert(landscapeDetail.layout, '横屏详情缺少独立 DOM');
    assert(landscapeDetail.game && landscapeDetail.title && landscapeDetail.intro && landscapeDetail.tags && landscapeDetail.actions, '横屏详情主信息结构不完整');
    assert(
      landscapeDetail.title.bottom <= landscapeDetail.intro.top + 1
        && landscapeDetail.intro.bottom <= landscapeDetail.tags.top + 1
        && landscapeDetail.tags.bottom <= landscapeDetail.actions.top + 1
        && landscapeDetail.actions.bottom <= landscapeDetail.facts.top + 1,
      `横屏详情必须按游戏名→简介→标签→按钮排列且不重叠：${JSON.stringify(landscapeDetail)}`,
    );
    ...
    assert(checkoutFromDetail.screen === 'checkout' && !checkoutFromDetail.panel && checkoutFromDetail.order?.durationLabel === '长期有效' && checkoutFromDetail.order?.amount === 198 && checkoutFromDetail.saleMode === 'entitlement', '首次点击“租号开玩”必须直接进入默认单游戏永久确认订单');
    await page.evaluate(() => window.__appRentalDemo.selectRentalSku('trial'));
    const confirmedRentalDetail = await page.evaluate(() => window.__appRentalDemo.snapshot());
    assert(confirmedRentalDetail.screen === 'checkout' && confirmedRentalDetail.order?.durationLabel === '2小时' && confirmedRentalDetail.order?.amount === 1.9, '确认订单切换首次体验后未按原始金额重建订单');
    ...
    await page.getByRole('button', { name: '租号开玩', exact: true }).click();
    const entitlementOptions = await page.locator('[data-checkout-field="entitlement"] .checkout-option').evaluateAll((nodes) => nodes.map((node) => node.textContent.trim()));
    assert(entitlementOptions.join('|') === '首次体验 · 2小时|单游戏永久|开会员畅玩', `确认订单首期权益不完整：${JSON.stringify(entitlementOptions)}`);
    assert((await page.locator('[data-checkout-field="rental-plan"], [data-hour-shortcut]').count()) === 0, '首期确认订单不应显示小时、日租或周租控件');
    const timeRentalBoundary = await page.evaluate(() => {
      const before = window.__appRentalDemo.snapshot();
      const results = [window.__appRentalDemo.selectRentalPlan('daily'), window.__appRentalDemo.setRentalHours(8)];
      const after = window.__appRentalDemo.snapshot();
      return {
        results,
        beforeOrderId: before.order?.id,
        afterOrderId: after.order?.id,
        beforeSku: before.selectedSku,
        afterSku: after.selectedSku,
      };
    });
    assert(
      timeRentalBoundary.results.every((value) => value === false)
        && timeRentalBoundary.beforeOrderId === timeRentalBoundary.afterOrderId
        && timeRentalBoundary.beforeSku === timeRentalBoundary.afterSku,
      `首期权益商品必须拒绝时租 API 且不得重建草稿：${JSON.stringify(timeRentalBoundary)}`,
    );
    ...
    assert(
      checkoutSelectionMatrix.enhancedRejected === false
        ...
        && new Set([selectionRows[0].id, ...selectionRows.slice(2).map(({ id }) => id)]).size === 3,
      `非标准版应拒绝且权益切换后应重建草稿：${JSON.stringify(checkoutSelectionMatrix)}`,
    );
    assert(
      selectionRows.every(({ editionId }) => editionId === 'standard')
        && selectionRows[0].sku === 'permanent'
        && selectionRows[0].durationLabel === '长期有效'
        && selectionRows[0].amount === 198
        && selectionRows[2].sku === 'trial'
        && selectionRows[2].durationLabel === '2小时'
        && selectionRows[2].amount === 1.9
        && selectionRows[3].sku === 'permanent',
      `确认订单标准版与首期权益快照不一致：${JSON.stringify(checkoutSelectionMatrix)}`,
    );
    const checkoutText = await page.locator('#appRentalDemo').innerText();
    assert(['影之刃零', '标准版', '权益方案', '首次体验 · 2小时', '单游戏永久', '开会员畅玩', '支付方式', '需支付 ¥198.00', '立即购买'].every((value) => checkoutText.includes(value)), '确认订单核心字段不完整');
    assert(['按小时', '日租', '周租', '续租'].every((value) => !checkoutText.includes(value)), '确认订单仍展示非首期时租能力');
    ...
    if (await page.locator('[data-membership-intro]').count()) {
      await page.locator('[data-membership-intro] [data-action="close-membership-intro"]').last().click();
    }
    const membership = await page.evaluate(() => ({
      ...
      recommendationCount: document.querySelectorAll('.plan-recommend').length,
      weeklySelected: document.querySelector('.membership-plan-card[data-plan="weekly"]')?.classList.contains('selected'),
      ...
      dueText: document.querySelector('.membership-checkout-bar strong')?.textContent.trim(),
      text: document.querySelector('#appRentalDemo').innerText,
    ...
    assert(membership.recommendationCount === 0 && membership.weeklySelected && membership.primaryLabel === '立即购买' && membership.dueText === '需支付 ¥39.00', '周卡未作为默认套餐、仍有推荐标签或底部结算信息不一致');
    assert(
      membership.valueHero.includes('一个会员，畅玩本期精选游戏')
        && membership.benefitTitles.join('|') === '会员库内畅玩|游戏持续更新|个人云存档同步'
        && membership.previewCards === 4,
      `会员中心三项权益或游戏库预览不完整：${JSON.stringify(membership)}`,
    );
    ...
    assert((await page.locator('.membership-checkout-bar strong').first().textContent())?.trim() === '需支付 ¥39.00', '切换周卡后底部需支付金额未同步');
    assert(!/自动续费|一次性购买/.test(membership.text), '会员中心出现禁用文案');
    assert(membership.paymentMethods === 2 && !membership.qr && membership.text.includes('支付宝') && membership.text.includes('微信'), 'APP 会员支付方式不完整或仍显示二维码');
    ...
    await page.evaluate(() => window.__appRentalDemo.openMemberLibrary({ remember: true }));
    const portraitMemberLibrary = await page.evaluate(() => {
      const cards = [...document.querySelectorAll('.device.portrait .member-game-card')];
      return {
        screen: window.__appRentalDemo.snapshot().screen,
    ...
    const landscapeMemberLibrary = await page.evaluate(() => {
      const cards = [...document.querySelectorAll('.device.landscape .member-game-card')];
    ...
    const readCheckoutReview = () => page.evaluate(() => {
      const rootNode = document.querySelector('#appRentalDemo');
      const paymentRow = rootNode.querySelector('.checkout-payment-row');
      const title = paymentRow?.querySelector('h2')?.getBoundingClientRect();
      const methods = [...(paymentRow?.querySelectorAll('.payment-method') || [])];
      const methodRects = methods.map((node) => node.getBoundingClientRect());
      const bottomBar = rootNode.querySelector('.checkout-bottom-bar');
      return {
        name: rootNode.querySelector('.checkout-product-name')?.textContent.trim(),
        edition: rootNode.querySelector('.checkout-product-edition')?.textContent.trim(),
        selectedPayment: methods.find((node) => node.classList.contains('selected'))?.dataset.payment,
        payments: methods.map((node) => node.dataset.payment),
        paymentRowAligned: Boolean(title && methodRects.length === 2 && methodRects.every((rect) => Math.abs(((rect.top + rect.bottom) / 2) - ((title.top + title.bottom) / 2)) <= 2)),
        paymentButtonsSameRow: methodRects.length === 2 && Math.abs(methodRects[0].top - methodRects[1].top) <= 1,
        amount: bottomBar?.querySelector('strong')?.textContent.trim(),
        action: bottomBar?.querySelector('[data-primary-action="true"]')?.textContent.trim(),
        removedCount: rootNode.querySelectorAll('.game-payment-qr, .checkout-agreement, .checkout-policy-list, .price-summary, .game-original-row, .checkout-row.total').length,
        forbiddenText: ['当前报价', '租赁信息', '租号服务协议', '支付有效期', '扫码支付'].filter((value) => rootNode.innerText.includes(value)),
      };
    });
    const portraitCheckoutReview = await readCheckoutReview();
    assert(
      portraitCheckoutReview.name === '影之刃零'
        && portraitCheckoutReview.edition === '标准版'
        && portraitCheckoutReview.selectedPayment === 'alipay'
        && portraitCheckoutReview.payments.join('|') === 'alipay|wechat'
        && portraitCheckoutReview.paymentRowAligned
        && portraitCheckoutReview.paymentButtonsSameRow,
      `竖屏商品信息或支付方式同行双按钮错误：${JSON.stringify(portraitCheckoutReview)}`,
    );
    assert(
      portraitCheckoutReview.amount === '需支付 ¥198.00'
        && portraitCheckoutReview.action === '立即购买'
        && portraitCheckoutReview.removedCount === 0
        && portraitCheckoutReview.forbiddenText.length === 0,
      `竖屏底部支付或冗余信息清理错误：${JSON.stringify(portraitCheckoutReview)}`,
    );
    ...
    assert(portraitWechat.selectedPayment === 'wechat', '竖屏微信支付选择未同步');
    assert(
    ...
    assert(
      landscapeCheckoutReview.selectedPayment === 'wechat'
        && landscapeCheckoutReview.payments.join('|') === 'alipay|wechat'
        && landscapeCheckoutReview.paymentRowAligned
        && landscapeCheckoutReview.paymentButtonsSameRow,
      `横屏支付方式未与标题同行展示双按钮：${JSON.stringify(landscapeCheckoutReview)}`,
    );
    assert(
      landscapeCheckoutReview.amount === '需支付 ¥198.00'
        && landscapeCheckoutReview.action === '立即购买'
        && landscapeCheckoutReview.removedCount === 0
        && landscapeCheckoutReview.forbiddenText.length === 0,
      `横屏底部支付或冗余信息清理错误：${JSON.stringify(landscapeCheckoutReview)}`,
    );
    ...
    assert(
      landscapeAlipay.selectedPayment === 'alipay'
    ...
    await page.evaluate(() => window.__appRentalDemo.navigate('membership'));
    const landscapeLibraryEntryHeight = await page.locator('.landscape-membership .membership-preview [data-action="open-member-library"], .landscape-membership .membership-preview [data-screen="member-library"]').first().evaluate((node) => node.getBoundingClientRect().height);
    ...
    assert(
      negativeGuards.orderAfterReturnMutation.status === 'pending'
        && negativeGuards.orderAfterReturnMutation.sku === 'permanent',
    ...
    const uniqueIds = await page.evaluate(() => {
      ...
      try {
        ...
        window.__appRentalDemo.payOrder();
        window.__appRentalDemo.prepareAccountForLaunch({
          orderId: first.id,
          launchRequestId: 'SAME-MILLISECOND-FIRST-TRIAL',
    ...
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForFunction(() => Boolean(window.__appRentalDemo));
    const accountFlagIgnoredForEntitlement = await page.evaluate(() => {
    ...
    assert(successfulTransaction.pending.sku === 'permanent', '订单 SKU 快照错误');
    assert(successfulTransaction.pending.amount === 198, '订单金额快照错误');
    ...
    assert(accountFlagIgnoredForEntitlement === 'active', '首期权益订单不应因支付阶段的账号参数进入分配或退款中间态');
    ...
    await page.locator('.portrait-order-list .order-list-card[data-status="refunding"] [data-order-card-action="progress"]').click();
    const refundingProgress = await page.locator('.refund-progress-dialog').innerText();
    assert(refundingProgress.includes('退款处理中') && refundingProgress.includes('退款金额 ¥1.90') && refundingProgress.includes('支付渠道处理中'), '退款中订单点击退款进度后未展示渠道处理阶段');
    await page.locator('.refund-progress-dialog [data-action="close-refund-progress"]').last().click();
    await page.locator('.portrait-order-list .order-list-card[data-status="refunded"] [data-order-card-action="progress"]').click();
    const refundedProgress = await page.locator('.refund-progress-dialog').innerText();
    assert(refundedProgress.includes('退款已完成') && refundedProgress.includes('已按原支付方式退回') && refundedProgress.includes('退款原路到账'), '已退款订单点击退款进度后未展示完成结果');
    await page.locator('.refund-progress-dialog [data-action="close-refund-progress"]').last().click();
    ...
    await page.locator('.portrait-order-list .order-list-card[data-status="refunding"]').click();
    await page.locator('.order-detail-actions-dock [data-order-card-action="progress"]').click();
    assert(await page.locator('.refund-progress-dialog').isVisible(), '订单详情退款进度按钮点击后无可见反馈');
    await page.locator('.refund-progress-dialog [data-action="close-refund-progress"]').last().click();
    ...
    const oneClickSuccess = await page.evaluate(() => ({
      screen: window.__appRentalDemo.snapshot().screen,
      libraryTab: window.__appRentalDemo.snapshot().libraryTab,
      dialogOpen: Boolean(document.querySelector('.login-method-dialog')),
      toast: document.querySelector('.demo-toast')?.textContent.trim(),
    }));
    assert(oneClickSuccess.screen === 'detail' && !oneClickSuccess.dialogOpen && oneClickSuccess.toast === '登录成功，已进入游戏详情', `一键上号后未成功进入游戏详情：${JSON.stringify(oneClickSuccess)}`);
    ...
    assert(oneClickToastCleared.stateToast === null && oneClickToastCleared.visibleToast === null && oneClickToastCleared.screen === 'detail', `一键上号成功 Toast 未自动消失或离开游戏详情：${JSON.stringify(oneClickToastCleared)}`);
    await page.evaluate(() => window.__appRentalDemo.navigate('orders'));
    await page.locator('.order-list-card[data-status="active"]').click();
    await page.getByRole('button', { name: '登录游戏', exact: true }).click();
    ...
    const landscapeCredential = await page.evaluate(() => ({
      ...
      steamGuardCount: document.querySelectorAll('.credential-panel--landscape .credential-guard').length,
      thirdPartyCodeRatio: (() => {
    ...
    assert(
      landscapeCredential.count === 1
        ...
        && landscapeCredential.steamGuardCount === 0
    ...
    await page.getByRole('button', { name: '登录游戏', exact: true }).click();
    await page.locator('.login-method-dialog [data-action="open-manual-login"]').click();
    const earlyGuardRequest = await page.evaluate(() => ({
      accepted: window.__appRentalDemo.requestGuardCode(),
      toast: document.querySelector('.demo-toast')?.textContent.trim() || '',
      screen: window.__appRentalDemo.snapshot().screen,
    }));
    assert(
      earlyGuardRequest.accepted === false
        && earlyGuardRequest.screen === 'steam-login'
        && earlyGuardRequest.toast === '请在 Steam 登录验证码界面后再尝试点击',
      `Steam 未进入验证码页时未正确拦截：${JSON.stringify(earlyGuardRequest)}`,
    );
    const guardBeforeLogin = await page.evaluate(() => ({
      sectionCount: document.querySelectorAll('.steam-login-form .steam-guard').length,
      requestCount: document.querySelectorAll('.steam-login-form [data-action="request-guard"]').length,
      legacyHint: document.querySelector('.steam-login-form')?.textContent.includes('提交账号密码后获取令牌') || false,
    }));
    assert(
      guardBeforeLogin.sectionCount === 0 && guardBeforeLogin.requestCount === 0 && !guardBeforeLogin.legacyHint,
      `Steam 登录前不应展示令牌提示或禁用按钮：${JSON.stringify(guardBeforeLogin)}`,
    );
    await page.locator('#steam-account').fill('player@example.com');
    await page.locator('#steam-password').fill('not-a-real-password');
    await page.locator('#steam-remember').check();
    await page.getByRole('button', { name: '登录', exact: true }).click();
    const afterSubmit = await page.evaluate(() => ({
      screen: window.__appRentalDemo.snapshot().screen,
      toast: document.querySelector('.demo-toast')?.textContent.trim() || '',
      guardCount: document.querySelectorAll('.steam-login-form .steam-guard [data-action="request-guard"]').length,
      snapshot: JSON.stringify(window.__appRentalDemo.snapshot()),
    }));
    assert(
      afterSubmit.screen === 'steam-login'
        && afterSubmit.toast === '请完成 Steam 验证码校验'
        && afterSubmit.guardCount === 1,
      `Steam 提交账号密码后未进入验证码阶段：${JSON.stringify(afterSubmit)}`,
    );
    ...
    assert(
      afterGuard.screen === 'detail' && afterGuard.toast === '验证码已自动输入，登录成功',
      `Steam 验证码自动输入后未返回游戏详情：${JSON.stringify(afterGuard)}`,
    );
    assert(!afterGuard.snapshot.includes('48291'), '公开 snapshot 泄露 Steam Guard 验证码');
    await page.evaluate(() => {
      window.__appRentalDemo.navigate('orders');
      window.__appRentalDemo.selectOrder('APP-SCENARIO-ACTIVE');
      window.__appRentalDemo.openCredentialPanel();
    });
    const orderCredentialGuard = await page.evaluate(() => ({
      guardCount: document.querySelectorAll('.credential-panel .credential-guard').length,
      thirdPartyTitle: document.querySelector('.credential-panel [data-third-party-platform]')?.textContent.trim() || '',
    }));
    assert(
      orderCredentialGuard.guardCount === 0 && orderCredentialGuard.thirdPartyTitle.includes('Rockstar Games 登录'),
      `订单登录信息应保留三方登录信息且不再展示 Steam Guard：${JSON.stringify(orderCredentialGuard)}`,
    );
    await page.locator('button[data-action="close-credential"]').click();
    await page.evaluate(() => window.__appRentalDemo.navigate('steam-login'));
    await page.locator('#steam-account').fill('player@example.com');
    await page.locator('#steam-password').fill('not-a-real-password');
    await page.locator('#steam-remember').check();
    ...
    const steamHelp = await page.evaluate(() => ({
      ...
      guardCount: document.querySelectorAll('.steam-credential-sheet .credential-guard').length,
    ...
    assert(
      steamHelp.sheet && steamHelp.formVisible && steamHelp.guardCount === 0
      ...
      'Steam 登录信息必须使用完整内容宽度、不展示 Guard，并保留具体第三方平台',
    );
    ...
    await page.evaluate(() => window.__appRentalDemo.setOrientation('portrait'));
    const rotatedLogin = await page.evaluate(() => ({
      account: document.querySelector('#steam-account')?.value,
      remember: document.querySelector('#steam-remember')?.checked,
      guardCount: document.querySelectorAll('.steam-login-form .steam-guard').length,
    }));
    assert(rotatedLogin.account === 'player@example.com' && rotatedLogin.remember && rotatedLogin.guardCount === 0, '旋转后 Steam 表单未连续保留或误显示 Guard');
    await page.evaluate(() => window.__appRentalDemo.openCredentialPanel());
    assert(
      (await page.locator('.credential-panel .credential-guard').count()) === 0
        && (await page.locator('.credential-panel [data-third-party-platform]').count()) === 1,
      '订单登录信息面板误显示 Guard 或缺少三方登录信息',
    );
    ...
    process.stdout.write('STEAM_SECURITY 12/12 PASS\n');
    ...
    await page.evaluate(() => window.__appRentalDemo.openCredentialPanel());
    await page.locator('.credential-panel [data-action="close-credential"]').click();
    await page.locator('.steam-help-trigger').click();
    assertTouchTargets(
      await readTouchTargets('.steam-login-submit, .steam-guard button, .steam-credential-sheet .dialog-close, .steam-credential-sheet .credential-field button, .steam-credential-sheet .third-party-code button'),
      'Steam 与 Guard 操作',
    );
    await page.evaluate(() => {
      window.__appRentalDemo.setOrientation('portrait');
    ...
    assert(
      steamContinuityAfter.snapshot.screen === 'steam-login'
        ...
        && steamContinuityAfter.guardCount === 0,
      '旋转后 Steam 表单、记住我或全宽登录信息丢失，或误显示 Guard',
    );
    assert(
    ...
    const portraitAfterSales = await page.evaluate(() => ({
      ...
      requestTypes: [...document.querySelectorAll('[data-after-sales-request]')].map((node) => node.textContent.trim()),
      reasons: [...document.querySelectorAll('[data-after-sales-reason]')].map((node) => node.textContent.trim()),
      selectedRequest: document.querySelector('[data-after-sales-request].selected')?.dataset.afterSalesRequest,
      screen: window.__appRentalDemo.snapshot().screen,
    }));
    assert(portraitAfterSales.layout === 'portrait-after-sales' && portraitAfterSales.screen === 'after-sales', '竖屏售后独立页不完整');
    assert(JSON.stringify(portraitAfterSales.requestTypes) === JSON.stringify(['申请退款', '申请换号']), '售后诉求必须区分退款与换号');
    assert(JSON.stringify(portraitAfterSales.reasons) === JSON.stringify(['3天无理由', '无法登录', '无法启动', '账号异常', '其他问题']), '退款原因名称或顺序不符合最终口径');
    assert(portraitAfterSales.selectedRequest === 'refund', '售后默认必须选中申请退款');
    ...
    const landscapeAfterSales = await page.evaluate(() => ({
      ...
      requestType: window.__appRentalDemo.snapshot().afterSalesDraft.requestType,
      reason: window.__appRentalDemo.snapshot().afterSalesDraft.reason,
      description: document.querySelector('#after-sales-description')?.value,
    }));
    assert(landscapeAfterSales.layout === 'landscape-after-sales' && landscapeAfterSales.requestType === 'refund' && landscapeAfterSales.reason === 'launch' && landscapeAfterSales.description.includes('持续闪退'), '旋转后售后右侧面板或草稿丢失');
    const afterSalesSubmit = await page.evaluate(() => {
      ...
      return {
        ...
        second,
        snapshot,
        detailAction: document.querySelector('[data-order-card-action="after-sales-detail"]')?.textContent.trim() || '',
        toast: document.querySelector('.demo-toast')?.textContent.trim() || '',
      };
    });
    ...
    assert(afterSalesSubmit.snapshot.screen === 'order-detail' && afterSalesSubmit.detailAction === '售后详情' && afterSalesSubmit.toast === '售后申请已提交', `提交售后后未关闭申请页、提示成功或切换入口：${JSON.stringify(afterSalesSubmit)}`);
    await page.locator('[data-order-card-action="after-sales-detail"]').click();
    const afterSalesProgress = await page.evaluate(() => ({
      dialog: document.querySelector('[aria-label="售后进度"]')?.textContent || '',
      steps: [...document.querySelectorAll('.after-sales-progress-dialog .refund-progress-timeline span')].map((node) => ({ text: node.textContent.trim(), className: node.className })),
      withdraw: document.querySelector('[aria-label="售后进度"] [data-action="withdraw-after-sales"]')?.textContent.trim() || '',
    }));
    assert(afterSalesProgress.dialog.includes('退款处理中') && afterSalesProgress.steps.length === 3 && afterSalesProgress.steps[0].className.includes('done') && afterSalesProgress.steps[1].className.includes('current') && afterSalesProgress.withdraw === '撤销售后', `售后详情未弹出退款进度或状态不完整：${JSON.stringify(afterSalesProgress)}`);
    await page.waitForTimeout(2000);
    await page.locator('.device').screenshot({ path: path.join(root, 'public', 'prd', 'app-rental', '19-after-sales-progress-landscape.png'), animations: 'disabled' });
    await page.evaluate(() => window.__appRentalDemo.setOrientation('portrait'));
    await page.locator('.device').screenshot({ path: path.join(root, 'public', 'prd', 'app-rental', '19-after-sales-progress-portrait.png'), animations: 'disabled' });
    await page.getByRole('button', { name: '撤销售后', exact: true }).click();
    const afterSalesWithdraw = await page.evaluate(() => ({
      screen: window.__appRentalDemo.snapshot().screen,
      ticket: window.__appRentalDemo.snapshot().afterSalesOrder,
      action: document.querySelector('[data-order-card-action="after-sales"]')?.textContent.trim() || '',
      toast: document.querySelector('.demo-toast')?.textContent.trim() || '',
    }));
    assert(afterSalesWithdraw.screen === 'order-detail' && afterSalesWithdraw.ticket === null && afterSalesWithdraw.action === '申请售后' && afterSalesWithdraw.toast === '售后申请已撤销', `撤销售后后未恢复申请入口：${JSON.stringify(afterSalesWithdraw)}`);
    await page.evaluate(() => {
      window.__appRentalDemo.openAfterSales();
      window.__appRentalDemo.setAfterSalesRequestType('replacement');
      window.__appRentalDemo.setAfterSalesReason('account');
      window.__appRentalDemo.setAfterSalesDescription('再次提交售后用于换号回归');
      window.__appRentalDemo.submitAfterSales();
      window.__appRentalDemo.openAfterSales();
    });
    ...
    const replacement = await page.evaluate(() => {
      ...
      const approved = window.__appRentalDemo.snapshot().replacementRequest;
      const launch = window.__appRentalDemo.prepareAccountForLaunch({
        orderId: approved.orderId,
        launchRequestId: 'REPLACEMENT-VERIFY',
        inventoryAvailable: true,
      });
      return { approved, launch, executed: window.__appRentalDemo.snapshot().replacementRequest };
    });
    assert(
      replacement.approved.status === 'approved-pending-launch'
        && replacement.approved.gameId === 'red-dead-2'
        && replacement.approved.version === 'Steam · 标准版'
        && replacement.launch.replacementApplied === true
        && replacement.executed.status === 'executed',
      `换号必须保持同游戏同版本，并在下次启动时生效：${JSON.stringify(replacement)}`,
    );
    assert(afterSalesSubmit.snapshot.guardCode === null && afterSalesSubmit.snapshot.steamForm.password === '', '售后提交后必须清理凭据');
    process.stdout.write('AFTER_SALES 14/14 PASS\n');
    ...
    const refundingRecovery = await page.evaluate(() => {
      const paid = window.__appRentalDemo.simulateTrialPayment(Date.now());
      window.__appRentalDemo.prepareAccountForLaunch({
        orderId: paid.id,
        launchRequestId: 'TRIAL-NO-ACCOUNT-RECOVERY',
        inventoryAvailable: false,
      });
      window.__appRentalDemo.navigate('orders');
    ...
    const networkOrderId = await page.evaluate(() => {
      const order = window.__appRentalDemo.createOrder({ sku: 'permanent', amount: 198, priceVersion: 'network' });
      window.__appRentalDemo.navigate('checkout');
    ...
    const membershipPreviewBadgeCount = await page.locator('.landscape-membership .membership-preview .cloud-save-badge').count();
    checkLayout(membershipPreviewBadgeCount === 0, `横屏会员中心仍显示云存档标签：${membershipPreviewBadgeCount}`);
    await page.evaluate(() => window.__appRentalDemo.openMemberLibrary({ remember: true }));
    const memberLibraryLayout = await page.evaluate(() => ({
      screen: window.__appRentalDemo.snapshot().screen,
      libraryTab: window.__appRentalDemo.snapshot().libraryTab,
    ...
    const afterSalesReasonBoundary = await page.evaluate(() => {
      ...
      window.__appRentalDemo.openAfterSales();
      return {
        requestCount: document.querySelectorAll('[data-after-sales-request]').length,
        reasonCount: document.querySelectorAll('[data-after-sales-reason]').length,
        labels: [...document.querySelectorAll('[data-after-sales-reason]')].map((node) => node.textContent.trim()),
        noReasonDisabled: Boolean(document.querySelector('[data-after-sales-reason="no-reason"]:disabled')),
      };
    });
    checkReviewFix(afterSalesReasonBoundary.requestCount === 2 && afterSalesReasonBoundary.reasonCount === 5 && afterSalesReasonBoundary.labels.includes('3天无理由') && afterSalesReasonBoundary.noReasonDisabled, `售后诉求、原因或无理由资格边界错误：${JSON.stringify(afterSalesReasonBoundary)}`);
    ...
    assertAnnotation(
      /任务页(?:不显示|保持无)全局导航/.test(annotationSource)
        && /旋转后保留|方向切换不丢|旋转不重复展示/.test(annotationSource),
      '标注版未明确任务页壳层或旋转连续性规则',
    );
    ...
    await annotationPage.locator('[data-after-sales-request="replacement"]').click();
    await annotationPage.locator('[data-after-sales-reason="launch"]').click();
    ...
    assertAnnotation(Boolean(annotatedAfterSales.id) && annotatedAfterSales.type === 'launch' && annotatedAfterSales.refundProgress === 0, '标注版退款／换号两级售后提交不可用或错误显示退款进度');
    ...
    const readHeroWireframeAlignment = () => annotationPage.evaluate(() => {
      const media = document.querySelector('.hero-card [data-wireframe-media], .landscape-home-hero [data-wireframe-media]');
      ...
      return {
        ...
        visualFrameWidth: frameRect.width,
        visualFrameHeight: frameRect.height,
        centerErrorX: Math.abs((mediaRect.left + mediaRect.right) / 2 - (frameRect.left + frameRect.right) / 2),
        centerErrorY: Math.abs((mediaRect.top + mediaRect.bottom) / 2 - (frameRect.top + frameRect.bottom) / 2),
        contained: mediaRect.left >= frameRect.left - 1 && mediaRect.right <= frameRect.right + 1 && mediaRect.top >= frameRect.top - 1 && mediaRect.bottom <= frameRect.bottom + 1,
      };
    });
    ...
    await annotationPage.evaluate(() => window.__appRentalDemo.navigate('home'));
    const compactPortraitCrop = await readHeroWireframeAlignment();
    assertAnnotation(
      compactPortraitCrop
        && compactPortraitCrop.centerErrorX <= 1
        && compactPortraitCrop.centerErrorY <= 1
        && compactPortraitCrop.contained,
      `标注版缩放后竖屏探索线框图位置错乱：${JSON.stringify(compactPortraitCrop)}`,
    );
    ...
    await annotationPage.evaluate(() => window.__appRentalDemo.navigate('home'));
    const compactLandscapeCrop = await readHeroWireframeAlignment();
    assertAnnotation(
      compactLandscapeCrop
        && compactLandscapeCrop.centerErrorX <= 1
        && compactLandscapeCrop.centerErrorY <= 1
        && compactLandscapeCrop.contained,
      `标注版缩放后横屏探索线框图位置错乱：${JSON.stringify(compactLandscapeCrop)}`,
    );
