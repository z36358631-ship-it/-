...
<div class="recharge-grid">${amounts.map((a,i)=>`<div class="ra-item${i===1?' selected':''}" data-amount="${a}" onclick="selectRA(this)"><div class="ra-coin">${a>=10000?(a/10000)+'万':a.toLocaleString()}</div><div class="ra-price">¥${(a/100).toFixed(0)}</div></div>`).join('')}</div>
...
function doRecharge(){
  ...
  const selected=parseInt(document.querySelector('.ra-item.selected')?.dataset.amount)||0;
  const amount=custom||selected;
  if(!amount){showToast('请选择充值金额');return}
  wallet.totalBalance+=amount;
  wallet.rechargeBalance+=amount;
  earnRecords.unshift({name:'充值',time:'2026-08-31 10:20',amount:`+${amount.toLocaleString()}`,type:'recharge'});
...
function openCardRedeem(cardId){
  ...
  if(!card||getCardDisabledReason(card))return;
