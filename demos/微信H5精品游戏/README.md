...
- `test=1`：开启自动验收支持。
- `seed`：仅在 `test=1` 时固定关卡随机种子。
- `speed`：仅在 `test=1` 时加速局内时钟，不改变碰撞和胜负规则。
- `mute=1`：仅在 `test=1` 时强制静音运行。

普通入口即使附带 `seed`、`speed` 或 `mute` 也会忽略这些测试参数，公开状态始终为自然速度。
...
测试接口用于自动验收，不承载正式业务能力。普通入口中
`window.__GAME_TEST__ === undefined`，不能通过 URL 或控制台调用推进局时、注入战利品或强制结算。
