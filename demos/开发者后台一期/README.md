...
本目录是依据 2026-09-03 最新 4 份开发者后台 PRD 构建的离线评审 Demo。最新 PRD 是本轮唯一业务基线；此前 Figma 终稿已冻结，本轮没有修改任何 `Figma/开发者后台一期/**` 文件，也不再以旧 Figma 页面数量或 Frame 映射约束 Demo。
...
| 入口文件 | 用途 | 默认路由 |
|---|---|---|
| `开发者后台一期总览demo.html` | 评审总览、模块入口、角色和状态切换 | 全模块索引 |
...
- P01-01 默认展示已有平台账号／邮箱＋密码登录；可在当前卡片切换盖世游戏扫码登录、刷新二维码并返回账号登录。首次盖世授权演示进入 P01-03 开发者注册；账号密码登录演示进入 P01-02 工作台。
...
- 所有操作只写入当前页面 JavaScript 内存；刷新后恢复初始 Fixture，不使用本地或远程持久化。
- Demo 不执行真实登录、建号、上传、Key 生成、SDK 下载、发布、回滚、资源投放或文件导出。
