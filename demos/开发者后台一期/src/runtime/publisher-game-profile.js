...
(function () {
  ...
  const words = {
    ...
    publication: ['发行设置', 'Release settings'], publicationHint: ['统一设置发行状态、发布地区和生效时间。完成资料后，从顶部提交审核。', 'Set a shared release status, target regions, and effective time, then submit from the top of the page.'], releaseStatus: ['发行状态', 'Release status'], releaseStatusHint: ['所有已选地区使用同一发行状态。', 'All selected regions use the same release status.'], releaseStatusRequired: ['请选择发行状态。', 'Select a release status.'], effectiveTime: ['生效时间', 'Effective time'], historicalStatus: ['历史提交保留原地区发行状态。', 'This historical submission retains its original region-specific statuses.'], immediate: ['审核后立即生效', 'Take effect after approval'], scheduled: ['定时生效', 'Schedule after approval'], scheduleTime: ['计划生效时间', 'Scheduled effective time'], scheduleHint: ['请选择未来时间。只有审核通过后才可按计划生效。', 'Choose a future time. The schedule can take effect only after approval.'], timeZone: ['时间按当前浏览器时区：{zone}', 'Times use the current browser time zone: {zone}'], reviewHint: ['本次提交仅进入资料审核，不代表已经通过审核或公开发布。', 'Submitting sends the details for review. It does not approve or publish the game.'],
  ...
  const stateFor = draft => { let state = runtime.get(draft); if (!state) { state = { root: null, urls: new Map(), uploads: new Map(), uploadSequence: 0, focus: null, observer: null, versionRequest: 0, versionViewer: { status: 'list', submissionId: '', record: null } }; runtime.set(draft, state); } return state; };
  ...
  const versionStatusLabel = (lang, value, detailed = false) => {
    const labels = lang === 'en'
      ? { draft: 'Not released', reviewing: 'In review', approved: 'Approved', rejected: 'Changes required', withdrawn: 'Withdrawn', delisted: 'Delisted', expired: 'Expired' }
      : { draft: '未发布', reviewing: '审核中', approved: detailed ? '审核通过' : '已通过', rejected: detailed ? '审核驳回' : '需修改', withdrawn: '已撤销', delisted: '已下架', expired: '已失效' };
  ...
  function renderVersionSnapshot(draft, lang, records, viewer) {
    ...
    const reviewStatus = record.status || summary.status || review?.decision || 'reviewing';
  ...
  function renderVersions(draft, lang, game = {}) {
    ...
    const fixtureStatus = game.reviewStatus === '审核中' ? 'reviewing' : game.reviewStatus === '需修改' ? 'rejected' : game.status === '已下架' ? 'delisted' : ['已通过', '已上线', '先锋测试', '预发布'].includes(game.reviewStatus) || ['已上线', '先锋测试', '预发布'].includes(game.status) ? 'approved' : 'draft';
    const defaultOperations = status => [
      { at: draft.savedAt || draft.submittedAt || '2026-09-07 19:53', description: isEnglish ? 'Version created' : '创建新版本' },
      ...(['reviewing', 'approved', 'rejected', 'withdrawn'].includes(status) ? [{ at: draft.submittedAt || '2026-09-07 20:16', description: isEnglish ? 'Submitted for release review' : '发布游戏，提交审核' }] : []),
      ...(status === 'approved' ? [{ at: draft.reviewResult?.reviewedAt || '2026-09-07 21:05', description: isEnglish ? 'Review approved' : '审核通过' }] : []),
      ...(status === 'rejected' ? [{ at: draft.reviewResult?.reviewedAt || '2026-09-07 21:05', description: isEnglish ? 'Changes requested' : '审核未通过，等待修改' }] : []),
      ...(status === 'withdrawn' ? [{ at: draft.reviewResult?.reviewedAt || '2026-09-07 20:30', description: isEnglish ? 'Review withdrawn' : '撤销审核' }] : []),
    ];
    const fallbackRecords = game.gameKey === 'existing' ? [
      { id: 'DEMO-CURRENT-VERSION', versionName: 'V-20260907-1', status: 'draft', createdAt: '2026-09-07 19:53', packageSummary: '—', isDemo: true, draft: snapshot, operationLogs: [{ at: '2026-09-07 19:53', description: isEnglish ? 'Version created' : '创建新版本' }] },
      { id: 'DEMO-PUBLISHED-VERSION', versionName: 'V-20260907', status: 'approved', displayStatus: isEnglish ? 'Live (assets need improvement)' : '已上线（素材待优化）', createdAt: '2026-09-07 10:56', submittedAt: '2026-09-07 16:49', reviewedAt: '2026-09-07 18:50', packageSummary: '3 个 PC Build', qualificationVersionId: 'QUAL-V1', submitter: isEnglish ? 'Current developer' : '当前开发者', isDemo: true, openByDefault: true, draft: snapshot, reviewResult: { decision: 'approved', reviewer: '平台运营', reviewedAt: '2026-09-07 18:50', reason: isEnglish ? 'Approved. Replace the default icon and improve the promotional images before the next update.' : '审核通过；建议后续替换默认图标，并优化商店宣传图的清晰度与版权风险。' }, operationLogs: [{ at: '2026-09-07 10:56', description: isEnglish ? 'Version created' : '创建新版本' }, { at: '2026-09-07 16:49', description: isEnglish ? 'Submitted for release review' : '发布游戏，提交审核' }, { at: '2026-09-07 18:50', description: isEnglish ? 'Review approved' : '审核通过' }, { at: '2026-09-07 18:50', description: isEnglish ? 'Published with asset improvement notes' : '已上线（素材待优化）' }] },
    ] : [{ id: `DEMO-${draft.gameKey || 'GAME'}-CURRENT`, versionName: draft.versionName || game.version || 'V-20260907', status: fixtureStatus, displayStatus: '', createdAt: draft.savedAt || draft.submittedAt || '2026-09-07 19:53', submittedAt: draft.submittedAt || '', reviewedAt: draft.reviewResult?.reviewedAt || '', packageSummary: Number(game.builds || 0) ? `${game.builds} 个 PC Build` : '—', qualificationVersionId: draft.qualifications?.activeVersion?.id || '', submitter: isEnglish ? 'Current developer' : '当前开发者', isDemo: true, draft: snapshot, reviewResult: draft.reviewResult || null, operationLogs: defaultOperations(fixtureStatus) }];
    const records = storedRecords.length ? storedRecords.map(record => ({ ...record, versionName: record.versionName || draft.versionName, packageSummary: record.packageSummary || (Number(game.builds || 0) ? `${game.builds} 个 PC Build` : '—'), operationLogs: record.operationLogs || defaultOperations(record.status) })) : fallbackRecords;
    ...
    const viewer = profileState.versionViewer || { status: 'list', submissionId: '', record: null };
    ...
    const rows = records.map(item => {
      ...
      const logs = item.operationLogs || defaultOperations(item.status);
      ...
      return `<article class="pgp-version-record${expanded ? ' is-expanded' : ''}" data-version-record="${esc(item.id)}"><div class="pgp-version-record__row"><div class="pgp-version-record__name"><span aria-hidden="true">◇</span><div><strong>${esc(item.versionName || item.id)}</strong><button type="button" data-version-open="${esc(item.id)}">${isEnglish ? 'Version details' : '版本详情'}</button></div></div><time>${esc(versionDate(lang, item.createdAt || item.submittedAt))}</time><b data-version-status="${esc(item.status)}">${esc(item.displayStatus || versionStatusLabel(lang, item.status))}</b><span>${esc(item.packageSummary || '—')}</span><button type="button" class="pgp-version-record__toggle" data-version-toggle="${esc(item.id)}" aria-expanded="${expanded}">${isEnglish ? `${logs.length} actions` : `${logs.length} 条操作记录`}<i aria-hidden="true">⌄</i></button></div><section class="pgp-version-operations"${expanded ? '' : ' hidden'} data-version-operations="${esc(item.id)}">${reminder}<div class="pgp-version-operation-head"><span>${isEnglish ? 'Time' : '操作时间'}</span><span>${isEnglish ? 'Description' : '描述'}</span><span>${isEnglish ? 'Version name' : '版本名称'}</span><span>${isEnglish ? 'Version number' : '版本号'}</span></div>${logs.map(log => `<div class="pgp-version-operation-row"><time>${esc(versionDate(lang, log.at))}</time><p>${esc(log.description)}</p><span>${esc(log.versionName || '—')}</span><span>${esc(log.versionNumber || '—')}</span></div>`).join('')}</section></article>`;
    }).join('');
    return `<section class="pgp-versions" data-profile-versions><section class="pgp-current-version"><header><h3>${isEnglish ? 'Current version status' : '当前版本状态'}</h3><p>${isEnglish ? `The latest record is ${current.versionName}. Expand the list below to see review feedback and the operation log.` : `最新记录为 ${current.versionName}，可在下方列表展开查看审核反馈与操作日志。`}</p></header><div><span>${isEnglish ? 'Current version' : '当前版本'} <strong>${esc(current.versionName || current.id)}</strong></span><span>${isEnglish ? 'Current status' : '当前状态'} <b data-version-status="${esc(current.status)}">${esc(current.displayStatus || versionStatusLabel(lang, current.status))}</b></span></div></section><header class="pgp-version-list-head"><div><h3>${isEnglish ? 'Version list' : '版本列表'} <small>${isEnglish ? `${records.length} records` : `共 ${records.length} 条记录`}</small></h3><p>${isEnglish ? 'Review every version’s created time, review status, PC build and complete operation log.' : '按版本查看创建时间、审核状态、PC 包体与完整操作日志。'}</p></div><div>${refreshed}<button type="button" data-version-refresh aria-label="${isEnglish ? 'Refresh version list' : '刷新版本列表'}">↻</button></div></header><div class="pgp-version-table-scroll" role="region" aria-label="${isEnglish ? 'Version records table' : '版本记录表格'}" tabindex="0"><div class="pgp-version-table-head"><span>${isEnglish ? 'Version' : '版本号'}</span><span>${isEnglish ? 'Created' : '创建时间'}</span><span>${isEnglish ? 'Review status' : '审核状态'}</span><span>${isEnglish ? 'PC build' : 'PC 包体'}</span><span>${isEnglish ? 'Actions' : '操作'}</span></div><div class="pgp-version-list">${rows}</div></div><footer class="pgp-version-pagination"><span>${isEnglish ? `Total ${records.length}` : `共 ${records.length} 条`}</span><button type="button" disabled aria-label="${isEnglish ? 'Previous page' : '上一页'}">‹</button><b>1</b><button type="button" disabled aria-label="${isEnglish ? 'Next page' : '下一页'}">›</button></footer></section>`;
  }
  ...
  function bind(container, { draft, language = 'zh', onChange = () => {}, onInterfaceLanguage = () => {}, onSectionChange = () => {}, onSave = async () => {}, onSubmit = async () => {}, onWithdraw = async () => {}, onQualificationSubmit = async () => {}, onQualificationWithdraw = async () => {}, onVersionOpen = async () => null, onVersionRefresh = async () => null }) {
    ...
    const openVersion = async submissionId => {
      ...
      const inlineRecord = state.displayedVersionRecords?.find(record => record.id === submissionId && record.isDemo);
      if (inlineRecord) {
    ...
    function changed(key) {
      ...
      draft.storeLocales = { enabled: draft.nameLanguages, default: draft.defaultNameLanguage, current: draft.currentNameLanguage };
      Object.assign(draft.gameProfileDraft, { gameNames: draft.gameNames, defaultLocale: draft.defaultNameLanguage, currentLocale: draft.currentNameLanguage, localizedContent: draft.localizedContent, localizedAssets: draft.localizedAssets, genres: draft.genres, platforms: draft.platforms, relationship: draft.relationship, developerName: draft.developerName, website: draft.website, playerGroupName: draft.playerGroupName, playerGroupNumber: draft.playerGroupNumber, listedElsewhere: draft.listedElsewhere });
      Object.assign(draft.releaseConfig, { releaseStatus: draft.releaseStatus, effectiveMode: draft.publication.mode, scheduledAt: draft.publication.scheduledAt });
