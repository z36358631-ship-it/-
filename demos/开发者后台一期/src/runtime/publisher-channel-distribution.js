...
(function registerPublisherChannelDistribution(namespace) {
  ...
  const fixture = Object.freeze({
    ...
    channels: [
      ...
      { id:'CH-240902', name:'ArcadeX 文件渠道', delivery:'file', productIds:['BASE-GLOBAL'], status:'active', credentialStatus:'not_applicable', lastDeliveredAt:'2026-09-12 10:06' },
      { id:'CH-240903', name:'北美线下渠道', delivery:'api', productIds:['BASE-GLOBAL'], status:'paused', credentialStatus:'paused', clientId:'cli_na_20260818', secretLast4:'2K8M', lastDeliveredAt:'2026-09-08 18:40' },
      { id:'CH-240904', name:'PixelMall 待接入', delivery:'api', productIds:['DLC-SEASON-01'], status:'active', credentialStatus:'pending', clientId:'', secretLast4:'', lastDeliveredAt:'—' },
      { id:'CH-240905', name:'CloudPlay 已停止渠道', delivery:'api', productIds:['BASE-GLOBAL'], status:'stopped', credentialStatus:'disabled', clientId:'cli_cp_20260709', secretLast4:'9P1R', lastDeliveredAt:'2026-09-01 09:14', stoppedAt:'2026-09-02 11:00' },
      { id:'CH-240906', name:'旧版合作渠道', delivery:'api', productIds:['BASE-GLOBAL'], status:'stopped', credentialStatus:'disabled', clientId:'cli_legacy_202602', secretLast4:'6N3W', lastDeliveredAt:'2026-07-31 20:18', stoppedAt:'2026-08-01 00:00' },
      { id:'CH-240907', name:'异常监控渠道', delivery:'api', productIds:['BASE-GLOBAL'], status:'risk_paused', credentialStatus:'reset_required', clientId:'cli_risk_202609', secretLast4:'1D8V', lastDeliveredAt:'2026-09-15 21:09' },
    ],
    fileBatches: [
    ...
    keyMetrics: [
      { channelId:'CH-240901', skuId:'BASE-GLOBAL', issued:1400, redeemed:1100, lastDeliveredAt:'2026-09-16 14:22', lastRedeemedAt:'2026-09-16 14:20' },
      { channelId:'CH-240901', skuId:'DLC-SEASON-01', issued:240, redeemed:168, lastDeliveredAt:'2026-09-16 14:18', lastRedeemedAt:'2026-09-16 14:17' },
      { channelId:'CH-240902', skuId:'BASE-GLOBAL', issued:300, redeemed:42, lastDeliveredAt:'2026-09-12 10:06', lastRedeemedAt:'2026-09-16 11:08' },
      { channelId:'CH-240903', skuId:'BASE-GLOBAL', issued:480, redeemed:397, lastDeliveredAt:'2026-09-08 18:40', lastRedeemedAt:'2026-09-15 22:10' },
      { channelId:'CH-240904', skuId:'DLC-SEASON-01', issued:0, redeemed:0, lastDeliveredAt:'', lastRedeemedAt:'' },
      { channelId:'CH-240905', skuId:'BASE-GLOBAL', issued:920, redeemed:851, lastDeliveredAt:'2026-09-01 09:14', lastRedeemedAt:'2026-09-15 09:32' },
      { channelId:'CH-240906', skuId:'BASE-GLOBAL', issued:310, redeemed:291, lastDeliveredAt:'2026-07-31 20:18', lastRedeemedAt:'2026-09-02 16:05' },
      { channelId:'CH-240907', skuId:'BASE-GLOBAL', issued:86, redeemed:65, lastDeliveredAt:'2026-09-15 21:09', lastRedeemedAt:'2026-09-15 21:04' },
    ],
  });
  ...
  const channelStatusMeta = (language, value) => ({ active:[tx(language,'合作中','Active'),'success'], paused:[tx(language,'已暂停','Paused'),'warning'], stopped:[tx(language,'已停止','Stopped'),'neutral'], risk_paused:[tx(language,'风险暂停','Risk paused'),'danger'] }[value] || [e(value || '—'),'neutral']);
  ...
  const renderChannelActions = (language, channel, state) => {
    ...
    const supplyAction = channel.delivery === 'api' ? actionLink(tx(language,'管理接入','Manage access'),'channel-api-access',{channelId:channel.id}) : channel.status === 'active' ? actionLink(tx(language,'创建文件','Create file'),'channel-file-create',{channelId:channel.id}) : '';
  ...
  const renderSupply = ({ language, state, transientSecret }) => {
    ...
    const channelsPanel = `<section class="publisher-channel-card" role="tabpanel"><header><h2>${tx(language,'渠道列表','Channel list')}</h2></header>${table([tx(language,'渠道名称／编号','Channel / ID'),tx(language,'销售项','Products'),tx(language,'供货方式','Supply method'),tx(language,'合作状态','Cooperation'),tx(language,'API 或文件状态','API / file status'),tx(language,'最近供货时间','Last supplied'),tx(language,'操作','Actions')],channelRows,{className:'publisher-channel-table--channels'})}</section>`;
    const batchesPanel = `<section class="publisher-channel-card" role="tabpanel"><header><h2>${tx(language,'文件批次','File batches')}</h2></header>${table([tx(language,'批次编号','Batch ID'),tx(language,'渠道','Channel'),tx(language,'销售项／SKU','Product / SKU'),tx(language,'生成数量','Quantity'),tx(language,'有效期','Valid until'),tx(language,'状态','Status'),tx(language,'创建／下载时间','Created / downloaded'),tx(language,'操作','Action')],batchRows,{className:'publisher-channel-table--batches'})}</section>`;
  ...
  const renderRevenue = ({ language, state, transientSecret }) => {
    const rows = state.keyMetrics.map(item => {
      ...
      return [`<span class="publisher-channel-row-title"><strong>${e(channel.name)}</strong><small>${e(channel.id)}</small></span>`,e(productType),e(productName),`<code>${e(product.id)}</code>`,e(delivery),formatNumber(issued),formatNumber(redeemed),formatNumber(unredeemed),e(rate),e(item.lastRedeemedAt || '—')];
    });
    ...
    return `${head(tx(language,'分销数据','Distribution data'),helpAction)}${metricCards}<section class="publisher-channel-card"><header><h2>${tx(language,'Key 发放与兑换','Key issuance and redemption')}</h2></header>${table([tx(language,'渠道','Channel'),tx(language,'销售项','Product type'),tx(language,'商品名称','Product'),'SKU',tx(language,'供货方式','Supply method'),tx(language,'发放量','Issued'),tx(language,'兑换量','Redeemed'),tx(language,'未兑换量','Unredeemed'),tx(language,'兑换率','Redemption rate'),tx(language,'最近兑换时间','Last redeemed')],rows,{className:'publisher-channel-table--distribution'})}</section>${renderDialog({language,state,transientSecret})}`;
  };
