...
(function registerShell(namespace) {
  ...
  const renderDeveloperDemoStateSwitcher = ({ module, role, redacted, isLogin, language, demoState = {} }) => {
    ...
    if (demoState.channelMode) {
      ...
      return `<section class="developer-demo-state-switcher">${trigger}<aside class="developer-demo-state-panel" id="developer-demo-state-panel" data-demo-state-panel role="dialog" aria-modal="false" aria-labelledby="developer-demo-state-title"><header><div><strong id="developer-demo-state-title">${isEnglish ? 'Batch outcomes' : '批次处理结果'}</strong><small>${isEnglish ? 'Demo only. Switch the result of a batch request.' : '仅用于演示批次申请结果'}</small></div><button type="button" data-portal-action="demo-state-toggle" aria-label="${isEnglish ? 'Close state preview' : '关闭状态预览'}">×</button></header><section class="developer-demo-state-group"><div class="developer-demo-state-options" role="radiogroup" aria-label="${isEnglish ? 'Batch outcome' : '批次处理结果'}">${batchOptions.map(item => option(item,'channel-batch-outcome',demoState.channelBatchOutcome || 'generated')).join('')}</div></section><footer><span>${demoState.active ? (isEnglish ? 'Preview is active' : '当前已启用预览状态') : (isEnglish ? 'Showing the default result' : '当前为默认结果')}</span><button type="button" data-portal-action="demo-state-reset"${demoState.active ? '' : ' disabled'}>${isEnglish ? 'Restore default' : '恢复默认'}</button></footer></aside></section>`;
    }
