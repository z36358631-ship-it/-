...
(function registerShell(namespace) {
  ...
  const renderPortalDemoSwitch = ({ module, portalData, role, redacted, isLogin, language }) => {
    ...
    if (!account) return '';
    const isEnglish = language === 'en';
    const fullLabel = role === 'operations' ? '切换开发者前台' : (isEnglish ? 'Switch to operations' : '切换运营后台');
    const shortLabel = role === 'operations' ? '开发者前台' : (isEnglish ? 'Operations' : '运营后台');
    const assistiveLabel = role === 'operations' ? 'Demo 工具：切换开发者前台' : (isEnglish ? 'Demo tool: switch to operations' : 'Demo 工具：切换运营后台');
  ...
  const renderDeveloperDemoStateSwitcher = ({ module, role, redacted, isLogin, language, demoState = {} }) => {
    ...
    if (demoState.channelMode) {
      ...
      const channelOptions = [['unopened','未开通','Not enabled'],['active','合作中','Active'],['paused','暂停','Paused']];
      const batchOptions = [['generated','自动生成','Auto-generated'],['pending','审核中','In review'],['rejected','已拒绝','Rejected'],['failed','生成失败','Generation failed']];
      ...
      return `<section class="developer-demo-state-switcher">${trigger}<aside class="developer-demo-state-panel" id="developer-demo-state-panel" data-demo-state-panel role="dialog" aria-modal="false" aria-labelledby="developer-demo-state-title"><header><div><strong id="developer-demo-state-title">${isEnglish ? 'Channel scenarios' : '渠道场景'}</strong><small>${isEnglish ? 'Demo only. Switch partnership and batch outcomes.' : '仅用于演示渠道合作和批次处理结果'}</small></div><button type="button" data-portal-action="demo-state-toggle" aria-label="${isEnglish ? 'Close state preview' : '关闭状态预览'}">×</button></header><section class="developer-demo-state-group"><div><strong>${isEnglish ? 'Channel partnership' : '渠道合作'}</strong><small>${isEnglish ? 'Controls authorization and batch availability' : '联动授权状态与批次创建权限'}</small></div><div class="developer-demo-state-options" role="radiogroup" aria-label="${isEnglish ? 'Channel partnership status' : '渠道合作状态'}">${channelOptions.map(item => option(item,'channel-status',demoState.channelStatus || 'active')).join('')}</div></section><section class="developer-demo-state-group"><div><strong>${isEnglish ? 'Batch outcome' : '批次处理结果'}</strong><small>${isEnglish ? 'Opens the selected result in Key batches' : '选择后进入 Key 批次查看结果与恢复操作'}</small></div><div class="developer-demo-state-options" role="radiogroup" aria-label="${isEnglish ? 'Batch outcome' : '批次处理结果'}">${batchOptions.map(item => option(item,'channel-batch-outcome',demoState.channelBatchOutcome || 'generated')).join('')}</div></section><footer><span>${demoState.active ? (isEnglish ? 'Preview is active' : '当前已启用预览状态') : (isEnglish ? 'Showing the default scenario' : '当前为默认场景')}</span><button type="button" data-portal-action="demo-state-reset"${demoState.active ? '' : ' disabled'}>${isEnglish ? 'Restore default' : '恢复默认'}</button></footer></aside></section>`;
    }
    if (demoState.financeMode) {
      const triggerLabel = isEnglish ? 'Demo states' : 'Demo 状态';
      const trigger = `<button class="developer-demo-state-fab" type="button" data-portal-action="demo-state-toggle" aria-expanded="${Boolean(demoState.open)}" aria-controls="developer-demo-state-panel" title="${triggerLabel}"><span class="developer-demo-state-fab__mark" aria-hidden="true">Demo</span><span>${triggerLabel}</span></button>`;
      if (!demoState.open) return `<section class="developer-demo-state-switcher">${trigger}</section>`;
      const scenarios = [['exhaustive','穷举态','Exhaustive'],['empty','缺省态','Empty']];
      return `<section class="developer-demo-state-switcher">${trigger}<aside class="developer-demo-state-panel" id="developer-demo-state-panel" data-demo-state-panel role="dialog" aria-modal="false" aria-labelledby="developer-demo-state-title"><header><div><strong id="developer-demo-state-title">${isEnglish ? 'Finance scenarios' : '财务场景'}</strong><small>${isEnglish ? 'Switch the current finance dataset.' : '切换当前财务数据。'}</small></div><button type="button" data-portal-action="demo-state-toggle" aria-label="${isEnglish ? 'Close state preview' : '关闭状态预览'}">×</button></header><section class="developer-demo-state-group"><div class="developer-demo-state-options" role="radiogroup" aria-label="${isEnglish ? 'Finance scenario' : '财务场景'}">${scenarios.map(([value, zh, en]) => `<button type="button" role="radio" aria-checked="${demoState.financeScenario === value}" class="${demoState.financeScenario === value ? 'is-active' : ''}" data-portal-action="demo-finance-scenario" data-finance-scenario="${value}">${isEnglish ? en : zh}</button>`).join('')}</div></section></aside></section>`;
    }
  ...
  const renderContext = ({ portalData, redacted, route, editorMode = 'edit', language = 'zh' }) => {
    ...
    if (['P15-01','P15-02','P15-03'].includes(route.id)) {
      const isEnglish = language === 'en';
      const end = route.id === 'P15-01' ? (isEnglish ? 'Finance entity' : '财务主体') : route.id === 'P15-02' ? (isEnglish ? 'Reconciliation & settlement' : '对账结算') : (isEnglish ? 'Reconciliation & settlement / Reconciliation flows' : '对账结算 / 对账流水');
  ...
  const renderPageHeader = ({ route, page, state, redacted, editorMode = 'edit', language = 'zh' }) => {
    if (['P15-01','P15-02','P15-03'].includes(route.id)) {
      const EnglishTitles = { 'P15-01':'Finance entity', 'P15-02':'Reconciliation & settlement', 'P15-03':'Reconciliation flows' };
  ...
  const renderBusiness = ({ module, routes, route, page, portalData, role, state, editorMode = 'edit', content, qualification, language, managedContent, registration, demoState }) => {
    ...
    const isFinanceWorkspace = ['P15-01','P15-02','P15-03'].includes(route.id);
