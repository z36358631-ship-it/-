...
(function registerTemplates(namespace) {
  ...
  const renderSupplyList = data => {
    const ledgers = data.supplyLedgers || { gamehub: [] };
    ...
    return `<div class="supply-overview">
      <section class="supply-product-card"><div class="supply-product-card__art">${icon('game')}</div><div><span>当前游戏</span><h2>星海远征</h2><p>GAME-48291 · 盖世 Key 渠道分销</p></div><div class="supply-product-card__status">${c.statusTag('正常', 'success')}<small>企业认证通过后可自行分销</small></div></section>
      <div class="supply-rule-banner">${icon('info')}<div><strong>仅管理盖世 Key</strong><span>开发者创建渠道和批次后，可下载兑换码文件或通过 API 向渠道供码。</span></div></div>
      ...
      ${panel({ title: '渠道供给状态', description: '渠道和批次启用后即可供码', body: `<div class="sale-readiness"><div class="is-done">${icon('check')}<span><strong>企业认证</strong><small>当前主体已通过认证</small></span>${c.statusTag('已通过')}</div><div class="is-done">${icon('check')}<span><strong>渠道管理</strong><small>开发者自行创建和维护渠道</small></span>${c.statusTag('可使用')}</div><div class="is-done">${icon('check')}<span><strong>批次管理</strong><small>支持文件下载和 API 取码</small></span>${c.statusTag('可使用')}</div></div>` })}
    </div>`;
  };
  ...
  const renderKeyBatchPanel = data => `<div class="key-batch-workspace">
    ...
    <div class="cdkey-panel-grid cdkey-panel-grid--batches">
      ...
      </div><div class="source-boundary"><strong>本页仅创建盖世 Key</strong><span>渠道可通过下载文件或 API 获取兑换码。</span></div><div data-key-batch-result></div><footer class="form-footer"><span class="save-state">生成成功后提供兑换码文件下载</span>${c.button({ label: '创建批次', variant: 'primary', action: 'create-key-batch' })}</footer>` })}
  ...
  const renderCdkeyWorkspace = (page, options = {}) => {
    ...
    return `<div class="cdkey-workspace" data-cdkey-workspace>
      ${embedded ? '' : `<section class="cdkey-context-header"><div><span>当前游戏</span><h2>星海远征</h2><p>GAME-48291 · 渠道分销</p></div><div><span>认证主体</span><strong>星海互动</strong><small>${e(data.authorization.scope)}</small></div><div><span>盖世 Key 批次</span><strong class="number">${e(data.keyBatches.length)}</strong><small>文件与 API 批次</small></div><div>${c.statusTag(data.authorization.status, 'success')}<small>企业认证有效</small></div></section>`}
  ...
  const publisherChannelConsoleSections = [
    ['channel-supply', '渠道与供给', 'key', '渠道 供给 API 文件 Key 合作'],
  ...
  const renderPublisherData = (state, language = 'zh', rawAccess, game = null, demoState = {}) => {
    ...
    return `<section class="publisher-platform-page" data-publisher-page="data">
      ...
      <div class="publisher-overview-grid"><section class="publisher-overview-card publisher-overview-card--wide"><header><div><span>游戏接入</span><h2>项目进度</h2></div>${c.statusTag(pendingGames.length ? `${pendingGames.length} 项待处理` : '暂无待处理', pendingGames.length ? 'warning' : 'success')}</header><div class="publisher-progress-table"><div class="is-head"><span>游戏</span><span>审核结果</span><span>SDK</span><span>PC 包体</span><span>发行阶段</span></div>${projectContent}</div></section>
        <section class="publisher-overview-card"><header><div><span>发行供给</span><h2>渠道分销</h2></div></header><dl class="publisher-supply-summary"><div><dt>启用渠道</dt><dd>${hasExistingGame ? '2' : '0'}</dd></div><div><dt>启用批次</dt><dd>${hasExistingGame ? '5' : '0'}</dd></div><div><dt>盖世 Key 已兑换</dt><dd>${hasExistingGame ? '1,707' : '0'}</dd></div></dl>${hasExistingGame ? `<button type="button" data-portal-action="publisher-open-cdkey">进入渠道与供给 ${icon('chevron')}</button>` : '<p class="publisher-overview-empty-copy">添加游戏并创建渠道后展示。</p>'}</section>
  ...
  const renderPublisherVendorSettings = (language = 'zh', rawAccess, qualification = {}, registration = {}, demoState = {}) => {
    ...
    return `<section class="publisher-vendor-settings" data-publisher-page="vendor" data-publisher-vendor-restriction="${e(status)}"><header class="publisher-vendor-title"><h1>${isEnglish ? 'Company settings' : '厂商设置'}</h1></header>${pendingProgress}<div class="publisher-empty-state publisher-vendor-placeholder">${icon(suspended ? 'warning' : 'vendor')}<strong>${e(title)}</strong>${cardAction}</div></section>`;
  };
  ...
  const renderPublisherStoreSection = (section, page, game) => {
    if (section === 'cdkey') {
      const header = `<header class="publisher-content-title"><div><span>CHANNEL DISTRIBUTION</span><h2>渠道分销</h2><p>创建渠道和盖世 Key 批次，并通过文件或 API 向渠道供码。</p></div><div><span class="publisher-context-chip">${e(game.name)} · ${e(game.gameId)}</span></div></header>`;
      if (game.gameKey !== 'existing') return `<section class="publisher-detail-section publisher-detail-section--cdkey">${header}<div class="publisher-empty-state">${icon('key')}<strong>暂未配置渠道分销</strong><p>${e(game.name)}尚未创建渠道或盖世 Key 批次。</p></div></section>`;
  ...
  const renderPublisherAnalyticsSection = (section, game) => {
    ...
    if (section === 'cdkey-data') {
      if (game.gameKey !== 'existing') return `<section class="publisher-detail-section"><header class="publisher-content-title"><div><span>KEY ANALYTICS</span><h2>分销数据</h2><p>展示盖世 Key 的发放与兑换。</p></div></header><div class="publisher-empty-state">${icon('key')}<strong>暂无分销数据</strong><p>${e(game.name)}尚未创建渠道或盖世 Key 批次。</p></div></section>`;
      return `<section class="publisher-detail-section"><header class="publisher-content-title"><div><span>KEY ANALYTICS</span><h2>分销数据</h2><p>按渠道和销售项展示盖世 Key 的发放与兑换。</p></div></header><div class="publisher-comment-metrics">${c.metricCard({ label: '盖世 Key 已发放', value: '2,420', trend: '文件与 API 批次合计' })}${c.metricCard({ label: '盖世 Key 已兑换', value: '1,707', trend: '兑换成功终态' })}${c.metricCard({ label: '未兑换', value: '713', trend: '已发放减已兑换' })}</div>${c.table({ headers: ['渠道', '销售项', '发放量', '兑换量', '更新时间'], rows: [['NovaPlay Store', '星海远征 · 本体', '1,400', '1,100', '2026-09-16 14:22'], ['ArcadeX 文件渠道', '星海远征 · 本体', '300', '42', '2026-09-16 11:08']] })}</section>`;
    }
  ...
  const renderPublisherGameConsole = (page, state, language = 'zh', rawAccess, demoState = {}, transientSecret = '') => {
    const access = publisherAccessFor(rawAccess);
  ...
  const renderPlatformAccountOverview = ({ isEnglish, status, verificationLabel, accessEnabled, activeTab }) => {
    ...
    return `<section class="platform-console-panel" data-platform-console-panel="overview"${activeTab === 'overview' ? '' : ' hidden'}>
      ...
      <div class="platform-dashboard-grid">
        ...
        <section class="platform-task-card"><header><div><span>${isEnglish ? 'TO DO' : '待处理事项'}</span><h2>${isEnglish ? 'Continue setup' : '继续完成以下事项'}</h2></div><strong>${pendingCount}</strong></header><div class="platform-task-list">${accessEnabled ? `<button type="button" data-demo-action="enter-game-review"><span class="task-level is-warning">优先</span><span><strong>修改预发布申请</strong><small>开始时间与测试服务器说明需要补充</small></span>${icon('chevron')}</button><button type="button" data-demo-action="open-cdkey-console"><span class="task-level">分销</span><span><strong>维护渠道与批次</strong><small>按合作安排创建渠道并选择文件或 API 供码</small></span>${icon('chevron')}</button>` : `<button type="button" data-demo-action="${status === 'unsubmitted' ? 'start-company-verification' : 'platform-console-tab'}"${status === 'unsubmitted' ? '' : ' data-platform-console-tab="qualification"'}><span class="task-level is-warning">认证</span><span><strong>${verificationLabel}</strong><small>${verificationCopy}</small></span>${icon('chevron')}</button>`}</div></section>
  ...
  const renderPlatformResources = ({ isEnglish, activeTab }) => {
    ...
    const documents = isEnglish ? [
      ...
      ['cdkey', 'CDKEY & Channel API', 'Create GameHub Key batches and deliver them to channels by file or API.', ['Create a channel', 'Create a GameHub Key batch', 'Choose file download or API delivery', 'Review issuance and redemption data']],
    ] : [
      ...
      ['cdkey', 'CDKEY 与渠道 API', '创建盖世 Key 批次，并通过文件或 API 向渠道供码。', ['创建渠道', '创建盖世 Key 批次', '选择文件下载或 API 供码', '查看发放与兑换数据']],
    ];
    const deliverables = {
  ...
  const renderPlatformDeveloperAccount = ({ registration, qualification, language, demoState }) => {
    const isEnglish = language === 'en';
  ...
  const renderT03 = ({ route, page, qualification, operationsReview, registration, workspaceState, language, access, demoState, transientSecret }) => {
    if (route.id === 'P01-08') {
    ...
    if (route.id === 'P02-01') return renderPublisherWorkspace({ page, workspaceState, language, rawAccess: access, qualification, registration, demoState, transientSecret });
  ...
  const renderT04 = ({ qualification, language, managedContent, registration, demoState }) => {
    ...
    return qualificationFlowOpen
      ...
      : renderPlatformDeveloperAccount({ registration, qualification, language, demoState });
  };
  ...
  const renderT15 = ({ page, route }) => {
    ...
    const configs = {
      'P04-01': {
        metrics: [['支付订单', '3,284', '成功支付终态'], ['成功退款', '126', '退款率 3.84%'], ['盖世 Key 已发放', '2,420', '文件与 API 批次合计'], ['首次成功启动', '2,408', 'uid＋app_id 去重']],
        ...
        definitions: ['支付订单：订单达到支付成功终态后计入。', '盖世 Key 发放：文件下载或 API 成功取码后计入。', '首次成功启动：同一 uid＋app_id 仅记录历史首次成功。', '交易与 Key 数据小时级更新；客户端数据次日更新。'],
        filters: ['全部游戏'],
      },
      'P04-02': {
      ...
      'P04-03': {
        metrics: [['盖世 Key 已发放', '2,420', '文件与 API 批次合计'], ['盖世 Key 已兑换', '1,707', '兑换成功终态'], ['未兑换', '713', '已发放减已兑换'], ['兑换率', '70.5%', '已兑换／已发放']],
        chart: '盖世 Key 发放与兑换趋势', title: '渠道分销数据', unit: 'Key 数量（个）', legend: '兑换成功',
        rows: [['NovaPlay Store', '星海远征 · 本体', '1,400', '1,100'], ['ArcadeX 文件渠道', '星海远征 · 本体', '300', '42']],
        headers: ['渠道', '销售项', '发放量', '兑换量'],
        definitions: ['发放量：文件批次下载数量与 API 成功取码数量之和。', '兑换量：盖世兑换服务返回成功终态后计入。', '未兑换量为已发放减已兑换，不代表渠道销量。', '测试取码和测试兑换全部排除。'],
        filters: ['全部游戏', '全部渠道'],
      },
      'P04-04': {
  ...
  namespace.templates = {
    ...
    render({ route, page, state = 'default', editorMode = 'edit', qualification, language = 'zh', managedContent, contentEditor, operationsReview, registration, authenticated = false, workspaceState, access, demoState, transientSecret = '' }) {
      ...
      return `<section class="page-template template-${route.templateId.toLowerCase()}" data-page-state="default">${renderer({ route, page, editorMode, qualification, language, managedContent, contentEditor, operationsReview, registration, authenticated, workspaceState, access, demoState, transientSecret })}</section>`;
    },
  };
