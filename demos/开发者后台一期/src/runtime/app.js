...
(function startApplication(namespace) {
  ...
  const hasFinancePrimaryEntry = hasFinanceRoutes && financeRouteIds.has('P01-01');
  ...
  if (hasFinancePrimaryEntry && !memory.session.authenticated) {
    memory.session = {
      authenticated: true,
      accountKey: memory.session.accountKey || 'demo:finance-primary',
      vendorId: memory.session.vendorId || String(portalData.context?.vendorId || 'VENDOR-DEMO'),
      activeGameId: memory.session.activeGameId || 'existing',
      expiresAt: Math.max(Number(memory.session.expiresAt || 0), Date.now() + 8 * 60 * 60 * 1000),
    };
    if (!storedQualification) {
      memory.qualification = {
        ...
        submittedAt: memory.qualification.submittedAt || '2026-09-01 09:00',
        reviewedAt: memory.qualification.reviewedAt || '2026-09-02 10:00',
        reviewer: memory.qualification.reviewer || '平台运营 李然',
      };
    }
    memory.registration = {
      ...memory.registration,
      accountTier: memory.qualification.status === 'approved' ? 'enterprise' : 'registered',
      registeredAt: memory.registration.registeredAt || memory.qualification.submittedAt || '2026-09-01 09:00',
    };
  }
