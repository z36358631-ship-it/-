(function developerFinanceSettlementDemo() {
  ...
  const detailSummary = row => `<section class="d15-cdkey-summary d15-detail-summary"><div><span>用户支付金额</span><strong>${esc(cnyWithSymbol(row.userPaidMinor))}</strong></div><div><span>结算比例</span><strong>${esc(percent(settlementRate(row)))}</strong></div><div><span>实际到账金额</span><strong>${esc(cnyWithSymbol(row.platformReceivedMinor))}</strong></div><div><span>结算金额</span><strong>${esc(cnyWithSymbol(row.payableMinor))}</strong></div></section>`;
