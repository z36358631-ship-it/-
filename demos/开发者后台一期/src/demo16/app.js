...
((namespace,model) => {
  ...
  const tierEditor = () => {
    ...
    const cards = draft.tiers.map((tier,index) => {
      ...
      const lower = first
        ? `<span class="fo-tier-boundary is-fixed">0</span>`
        : `<input name="from" type="number" min="0" step="0.01" value="${tier.fromMinor === '' || tier.fromMinor == null ? '' : esc(tier.fromMinor / 100)}" aria-label="第 ${index + 1} 档下限金额">`;
      const upper = last
