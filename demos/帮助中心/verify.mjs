...
try{
 ...
 await p.locator('[data-ha-edit-language="en"]').click();await p.locator('[data-ha-title]').fill('Updated welcome article');await p.locator('[data-ha-edit-language="zh"]').click();assert.equal(await p.locator('[data-ha-title]').inputValue(),'修改中的入门文章');record('语言页签切换保留输入且草稿不泄露');
