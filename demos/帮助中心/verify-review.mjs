...
try{
 ...
 const count=await p.evaluate(()=>HelpStore.draft.docs.length);await p.locator('[data-ha-language]').selectOption('hi');
 ...
 await p.locator('[data-ha-min-version]').fill('6.3.2');await p.locator('[data-ha-max-version]').fill('6.3.1');
 await p.locator('[data-ha-action="back"]').first().click();await p.locator('[data-ha-action="publish"][data-id="HC-OD002"]').click();assert.match(await p.locator('[data-ha-feedback]').innerText(),/最低版本不能/);
 assert.equal(await p.evaluate(()=>HelpStore.published.docs.find(d=>d.id==='HC-OD002').maxVersion),'');checks.push('错误版本区间发布被阻止，原发布快照保留');
 const model=await p.evaluate(()=>{const s=HelpStore,d=s.get('docs','HC-OD001'),out={};d.minVersion='6.3.2';d.maxVersion='6.3.10';out.boundaries=['6.3.1','6.3.2','6.3.9','6.3.10','6.3.11'].map(v=>s.visible(d,'overseas',v));out.cross=s.visible(d,'domestic','6.3.2');out.compare=s.compareVersion('6.10.0','6.9.9');d.ja.title='途中';out.partial=s.publish('docs',d.id);d.ja.title='';out.complete=s.publish('docs',d.id);out.fallback=s.locale(s.published.docs.find(x=>x.id===d.id),'ja').title;out.foreign=s.update('docs',d.id,{navId:'HC-N001'});d.en.title='Draft only';out.order=s.setOrder('docs',d.id,5);out.live=s.published.docs.find(x=>x.id===d.id).en.title;out.weight=s.published.docs.find(x=>x.id===d.id).order;out.bad=s.setOrder('docs',d.id,-1);out.sanitize=s.sanitize('<span style="font-size:20px;color:red" onclick="x()">字</span><span style="font-size:999px">坏</span>');return out;});
 assert.deepEqual(model.boundaries,[false,true,true,true,false]);assert.equal(model.cross,false);assert.equal(model.compare,1);assert.match(model.partial,/同时填写/);assert.equal(model.complete,'');assert.equal(model.fallback,'Welcome to the Help Center');assert.match(model.foreign,/跨包/);assert.equal(model.order,'');assert.equal(model.live,'Welcome to the Help Center');assert.equal(model.weight,5);assert.match(model.bad,/整数/);assert.match(model.sanitize,/font-size:20px/);assert.doesNotMatch(model.sanitize,/onclick|color|999/);checks.push('版本闭区间、数字比较、半译阻止、整篇回退、跨包阻止、排序与内容隔离、字号安全');
 ...
 await p.goto(appUrl+'?capture=1&package=overseas&version=6.3.1&page=help');assert.equal(await p.locator('.help-row').count(),0);assert.match(await p.locator('#device').innerText(),/暂无帮助内容/);checks.push('版本未命中隐藏文章及空导航');
