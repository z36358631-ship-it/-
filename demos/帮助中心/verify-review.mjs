...
try{
 ...
 await p.locator('[data-ha-tab="docs"]').click();await p.locator('[data-ha-action="edit"][data-id="HC-OD002"]').click();
 const count=await p.evaluate(()=>HelpStore.draft.docs.length);await p.locator('[data-ha-edit-language="hi"]').click();
 ...
 await p.waitForFunction(()=>document.querySelector('video')?.readyState>=2);checks.push('国内海外列表隔离；同ID按语言页签切换且不新增对象');
 const model=await p.evaluate(()=>{const s=HelpStore,d=s.get('docs','HC-OD001'),out={};out.cross=s.visible(d,'domestic');s.setLanguages('docs',d.id,[...d.enabledLanguages,'ja'],d.defaultLanguage);d.ja.title='途中';out.partial=s.publish('docs',d.id);d.ja.title='';out.complete=s.publish('docs',d.id);out.fallback=s.locale(s.published.docs.find(x=>x.id===d.id),'ja').title;out.foreign=s.update('docs',d.id,{navId:'HC-N001'});d.en.title='Draft only';out.order=s.setOrder('docs',d.id,5);out.live=s.published.docs.find(x=>x.id===d.id).en.title;out.weight=s.published.docs.find(x=>x.id===d.id).order;out.bad=s.setOrder('docs',d.id,-1);out.sanitize=s.sanitize('<span style="font-size:20px;color:red" onclick="x()">字</span><span style="font-size:999px">坏</span>');return out;});
