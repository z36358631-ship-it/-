(function () {
  ...
  function list() {
    ...
    return `<section class="ha-panel"><div class="ha-panel-head"><div><h2>${docs?'目录子文档':'导航目录'}</h2><p>${docs?'所属导航发布后，子文档可独立发布至 App。':'分别维护导航与子文档，同一条内容集中维护多语言，完成默认语言后逐条发布。'}</p></div>${btn(docs?'＋ 新建子文档':'＋ 新建导航','create','','primary')}</div><div class="ha-filters"><input type="search" data-ha-search aria-label="搜索标题或编号" placeholder="${docs?'输入文档标题或唯一编号':'输入导航名称或唯一编号'}" value="${e(state.search)}">${docs?`<select data-ha-filter aria-label="按导航筛选"><option value="">全部导航</option>${sorted('navs').filter(nav=>nav.package===state.package).map(nav=>`<option value="${nav.id}" ${nav.id===state.filter?'selected':''}>${e(title(nav))}</option>`).join('')}</select>`:''}${btn('查询','search','','primary')}${btn('重置','reset')}</div><div class="ha-table-wrap"><table><thead><tr><th>${docs?'文档标题':'导航名称'} / 唯一编号</th>${docs?'<th>所属导航</th>':'<th>子文档</th>'}<th>多语言</th><th>排序权重</th><th>发布状态</th><th>操作</th></tr></thead><tbody>${rows.map(item=>{
      const status=store.status(state.tab,item.id);
      return `<tr data-ha-row="${item.id}"><td><strong>${e(title(item))}</strong><small>${item.id}</small></td><td>${docs?`${e(title(store.get('navs',item.navId)))}<small>${e(item.minVersion||'不限')} 至 ${e(item.maxVersion||'不限')}</small>`:`${store.draft.docs.filter(doc=>doc.navId===item.id).length} 篇`}</td><td>${textBtn('多语言','languages',item.id,'','orange')}<small>${store.languages.filter(lang=>item[lang.code]?.title?.trim()).length} / 6 种语言</small></td><td><input class="ha-weight" data-ha-order="${item.id}" aria-label="${e(title(item))}排序权重" type="number" min="0" max="999999" step="1" value="${item.order}">${textBtn('保存排序','save-order',item.id)}</td><td><span class="ha-state ${status==='已发布'?'':'draft'}">${status}</span></td><td><div class="ha-actions">${status!=='已发布'?textBtn('发布','publish',item.id,'','orange'):''}${textBtn('编辑','edit',item.id,'','orange')}${textBtn('删除','delete',item.id,'','red')}</div></td></tr>`;
