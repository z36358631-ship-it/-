(function () {
  ...
  function languageSearch(item){return store.languages.map(lang=>store.translation(item,lang.code)?.title||'').join(' ');}
  function save() {
    const item=current(); if(!item||!root) return;
    const titleInput=root.querySelector('[data-ha-title]');
    if(!titleInput)return;
    if(!store.translation(item,state.language))store.setTranslation(item,state.language,{title:'',...(state.tab==='docs'?{html:''}:{})});
    if(titleInput) store.translation(item,state.language).title=titleInput.value;
    if(rich()) store.translation(item,state.language).html=store.sanitize(rich().innerHTML);
  ...
  function list() {
    ...
    return `<section class="ha-panel"><div class="ha-panel-head"><div><h2>${docs?'目录子文档':'导航目录'}</h2><p>${docs?'所属导航发布后，子文档可独立发布至 App。':'分别维护导航与子文档，同一条内容集中维护多语言，完成默认语言后逐条发布。'}</p></div>${btn(docs?'＋ 新建子文档':'＋ 新建导航','create','','primary')}</div><div class="ha-filters"><input type="search" data-ha-search aria-label="搜索标题或编号" placeholder="${docs?'输入文档标题或唯一编号':'输入导航名称或唯一编号'}" value="${e(state.search)}">${docs?`<select data-ha-filter aria-label="按导航筛选"><option value="">全部导航</option>${sorted('navs').filter(nav=>nav.package===state.package).map(nav=>`<option value="${nav.id}" ${nav.id===state.filter?'selected':''}>${e(title(nav))}</option>`).join('')}</select>`:''}${btn('查询','search','','primary')}${btn('重置','reset')}</div><div class="ha-table-wrap"><table><thead><tr><th>${docs?'文档标题':'导航名称'} / 唯一编号</th>${docs?'<th>所属导航</th>':'<th>子文档</th>'}<th>语言配置</th><th>排序权重</th><th>发布状态</th><th>操作</th></tr></thead><tbody>${rows.map(item=>{
      ...
      return `<tr data-ha-row="${item.id}"><td><strong>${e(title(item))}</strong><small>${item.id}</small></td><td>${docs?e(title(store.get('navs',item.navId))):`${store.draft.docs.filter(doc=>doc.navId===item.id).length} 篇`}</td><td><small>${item.enabledLanguages.length} / ${store.languages.length} 种语言</small></td><td><input class="ha-weight" data-ha-order="${item.id}" aria-label="${e(title(item))}排序权重" type="number" min="0" max="999999" step="1" value="${item.order}">${textBtn('保存排序','save-order',item.id)}</td><td><span class="ha-state ${status==='已发布'?'':'draft'}">${status}</span></td><td><div class="ha-actions">${status!=='已发布'?textBtn('发布','publish',item.id,'','orange'):''}${textBtn('编辑','edit',item.id,'','orange')}${textBtn('删除','delete',item.id,'','red')}${textBtn('多语言','languages',item.id,'','orange')}</div></td></tr>`;
  ...
  function editor() {
    ...
    const locale=store.translation(item,state.language)||{title:'',html:''};
    if(!item){state.selected='';return list();}
    return `<section class="ha-panel"><header class="ha-editor-head"><button class="ha-back" data-ha-action="back">← 返回列表</button><div class="ha-editor-meta"><div><h2>${docs?'目录子文档配置':'帮助中心导航配置'}</h2><small>${item.id} · ${e(store.languages.find(lang=>lang.code===state.language)?.label||state.language)}</small></div><span class="ha-state draft">编辑草稿</span></div></header><div class="ha-form"><div class="ha-fields"><label class="ha-field"><span>唯一编号</span><input value="${item.id}" disabled><small>系统生成，创建后不可修改</small></label><label class="ha-field"><span>所属包</span><input data-ha-object-package value="${item.package==='domestic'?'国内包':'海外包'}" disabled><small>创建后固定，不支持跨包迁移</small></label>${docs?`<label class="ha-field"><span>所属导航<em>*</em></span><select data-ha-nav>${sorted('navs',null).filter(nav=>nav.package===item.package).map(nav=>`<option value="${nav.id}" ${nav.id===item.navId?'selected':''}>${e(title(nav))}</option>`).join('')}</select><small>变更后移至目标导航末尾，发布后生效</small></label>`:''}${docs?`<fieldset class="ha-protocols ha-field full" data-ha-protocols><legend>APP协议<em>*</em></legend><div class="ha-protocol-options">${store.protocols.map(protocol=>`<label><input type="checkbox" data-ha-protocol value="${protocol.code}" ${(item.protocols||[]).includes(protocol.code)?'checked':''}><span>${e(protocol.label)}</span></label>`).join('')}</div><small>至少选择一项；与所属包共同决定文章展示范围。</small></fieldset>`:''}<div class="ha-content-languages ha-field full"><header><h3>${docs?'文档内容':'导航名称'}</h3>${btn('管理多语言','manage-languages',`data-id="${item.id}"`)}</header><nav class="ha-edit-language-tabs" role="tablist" aria-label="编辑语言">${item.enabledLanguages.map(code=>`<button type="button" role="tab" data-ha-edit-language="${code}" aria-selected="${state.language===code}" tabindex="${state.language===code?'0':'-1'}">${e(store.languages.find(lang=>lang.code===code)?.label||code)}${code===item.defaultLanguage?'<small>默认</small>':''}</button>`).join('')}</nav></div><label class="ha-field full"><span>${en?'Title':docs?'文档标题':'导航名称'}${(state.language===item.defaultLanguage||store.requiredLanguages(item).includes(state.language))?'<em>*</em>':''}</span><input data-ha-title maxlength="120" value="${e(locale.title)}" dir="${state.language==='ar'?'rtl':'auto'}" placeholder="${en?'Enter title':'请输入标题'}"><span class="ha-title-count"><span data-ha-title-count>${String(locale.title||'').length}</span> / 120</span><small>${(state.language===item.defaultLanguage||store.requiredLanguages(item).includes(state.language))?'默认语言及当前包必需语种，发布前须完整填写。':(docs?'可全部留空；填写时须同时完成标题与正文。':'可留空，展示时回退本条内容的默认语言。')}</small></label></div>${docs?`<div class="ha-field"><span>${en?'Article body':'文档正文'}${(state.language===item.defaultLanguage||store.requiredLanguages(item).includes(state.language))?'<em>*</em>':''}</span><small>支持文字、标题、列表、链接、图片与视频混排。</small></div><div class="ha-rich"><div class="ha-rich-toolbar" role="toolbar" aria-label="正文格式工具"><select data-ha-font-size aria-label="字号"><option value="">字号</option>${[14,16,18,20,24].map(size=>`<option value="${size}">${size}px</option>`).join('')}</select><button type="button" data-ha-command="formatBlock" data-value="H2" title="二级标题">H2</button><button type="button" data-ha-command="formatBlock" data-value="P" title="正文">正文</button><button type="button" data-ha-command="bold" title="加粗"><b>B</b></button><button type="button" data-ha-command="insertUnorderedList" title="无序列表">• 列表</button><button type="button" data-ha-command="insertOrderedList" title="有序列表">1. 列表</button><button type="button" data-ha-action="link">↗ 链接</button><button type="button" data-ha-action="image">▧ 插入图片</button><button type="button" data-ha-action="video">▷ 插入视频</button></div><div class="ha-inline-link" data-ha-link hidden><input type="url" data-ha-url aria-label="链接地址" placeholder="https://example.com">${btn('插入','insert-link')}${btn('取消','close-link')}</div><div class="ha-rich-body" data-ha-body contenteditable="true" role="textbox" aria-multiline="true" aria-label="${en?'Article body':'文档正文'}" dir="${state.language==='ar'?'rtl':'auto'}">${store.sanitize(locale.html||'')}</div></div><input hidden data-ha-image-file type="file" accept="image/png,image/jpeg,image/gif,image/webp"><input hidden data-ha-video-file type="file" accept="video/mp4,video/webm,video/ogg"><p class="ha-note">本地图片：PNG / JPG / GIF / WebP；视频：MP4 / WebM / Ogg。播放器不自动播放。</p>`:''}</div><footer class="ha-editor-footer"><small data-ha-saved>草稿已自动保留 · 发布后才会同步 App</small>${btn('保存草稿并返回','back','','primary')}</footer></section>`;
  }
  function protectedLanguages(){return [...new Set([state.modal.defaultLanguage,...store.requiredLanguages(current())])];}
  function languageModal(){
    const modal=state.modal,protectedCodes=protectedLanguages();
    return `<div class="ha-modal-layer"><section class="ha-language-dialog" data-ha-language-dialog role="dialog" aria-modal="true" aria-labelledby="ha-language-dialog-title"><header class="ha-dialog-head"><h2 id="ha-language-dialog-title">多语言设置</h2></header><div class="ha-dialog-default"><label for="ha-language-default">默认语言</label><select id="ha-language-default" data-ha-default-language>${store.languages.map(lang=>`<option value="${lang.code}" ${modal.defaultLanguage===lang.code?'selected':''}>${e(lang.label)}</option>`).join('')}</select></div><div class="ha-language-picker"><section><header><label><input type="checkbox" data-ha-language-all ${modal.enabled.length===store.languages.length?'checked':''}><span>${store.languages.length} 项</span></label></header><div class="ha-language-options">${store.languages.map(lang=>`<label><input type="checkbox" data-ha-language-option="${lang.code}" value="${lang.code}" ${modal.enabled.includes(lang.code)?'checked':''} ${protectedCodes.includes(lang.code)?'disabled':''}><span>${e(lang.label)}</span></label>`).join('')}</div></section><section><header><strong>已选 <span data-ha-language-selected-count>${modal.enabled.length}</span> 项</strong>${textBtn('清除','language-clear','',modal.enabled.every(code=>protectedCodes.includes(code))?'disabled':'')}</header><div class="ha-language-selected">${store.languages.filter(lang=>modal.enabled.includes(lang.code)).map(lang=>`<div data-ha-selected-language="${lang.code}"><span>${e(lang.label)}${protectedCodes.includes(lang.code)?`<small>${lang.code===modal.defaultLanguage?'默认':'必需'}</small>`:''}</span>${protectedCodes.includes(lang.code)?'':textBtn('×','language-remove',lang.code,`aria-label="移除${e(lang.label)}"`)}</div>`).join('')}</div></section></div><p class="ha-language-hint">默认语言及当前包必需的语种必须保留；移除语种会保留已填写内容。确认后在编辑页的语言页签中填写文案。</p><div class="ha-feedback" data-ha-feedback role="status">${e(modal.error||'')}</div><footer class="ha-language-settings-actions">${btn('取消','language-cancel')}${btn(modal.origin==='languages'?'确认并编辑':'确认','language-confirm','','primary')}</footer></section></div>`;
  }
  function refreshLanguageSettings(focusSelector){
    const old=root.querySelector('.ha-modal-layer');if(old)old.outerHTML=languageModal();
    const all=root.querySelector('[data-ha-language-all]');if(all)all.indeterminate=state.modal.enabled.length>0&&state.modal.enabled.length<store.languages.length;
    if(focusSelector)root.querySelector(focusSelector)?.focus();
  }
  function openLanguages(id,origin){
    save();const item=store.get(state.tab,id);state.modal={id,type:state.tab,origin,enabled:[...item.enabledLanguages],defaultLanguage:item.defaultLanguage,error:''};render();root.querySelector('[data-ha-default-language]')?.focus();
  }
  function closeLanguages(confirm=false){
    const modal=state.modal;if(!modal)return;
    const before=store.get(modal.type,modal.id),added=store.languages.map(lang=>lang.code).filter(code=>modal.enabled.includes(code)&&!before.enabledLanguages.includes(code));
    if(confirm){const error=store.setLanguages(modal.type,modal.id,modal.enabled,modal.defaultLanguage);if(error){modal.error=error;refreshLanguageSettings();return;}}
    const item=store.get(modal.type,modal.id);state.modal=null;
    if(confirm&&(modal.origin==='languages'||added.length)){
      const unfinished=item.enabledLanguages.find(code=>{const value=store.translation(item,code);return !value?.title?.trim()||(modal.type==='docs'&&!String(value?.html||'').trim());});
      state.tab=modal.type;state.selected=modal.id;state.language=added[0]||(modal.origin==='languages'?unfinished:'')||item.defaultLanguage;state.feedback='';state.deleting='';
      render();root.querySelector('.ha-content-languages')?.scrollIntoView({block:'start'});root.querySelector('[data-ha-title]')?.focus({preventScroll:true});return;
    }
    if(state.selected&&!item.enabledLanguages.includes(state.language))state.language=item.defaultLanguage;
    render();root.querySelector(`[data-ha-action="${modal.origin}"][data-id="${modal.id}"]`)?.focus();
  }
  function render() {
    if(!root)return;
    ...
    if(state.modal){root.querySelector('.ha-side').inert=true;root.querySelector('.ha-main').inert=true;root.querySelector('.ha').insertAdjacentHTML('beforeend',languageModal());const all=root.querySelector('[data-ha-language-all]');all.indeterminate=state.modal.enabled.length>0&&state.modal.enabled.length<store.languages.length;}
    state.range=null;
  }
  async function media(file,kind) {
    ...
    try {
      ...
      if(root&&!state.modal&&current()?.id===docId&&state.language===lang&&rich()){insert(html);feedback(`${kind==='image'?'图片':'视频'}已插入，发布后同步 App。`);}
      else {const item=store.get('docs',docId);if(item){store.translation(item,lang).html+=store.sanitize(html);store.notify();}feedback('媒体已保留到原文章草稿。');}
  ...
  function click(event) {
    ...
    if(button.dataset.haEditLanguage){save();state.language=button.dataset.haEditLanguage;state.feedback='';render();root.querySelector(`[data-ha-edit-language="${state.language}"]`)?.focus();return;}
    ...
    if(!action)return;
    if(action==='languages'||action==='manage-languages'){openLanguages(id,action);return;}
    if(action==='language-cancel'){closeLanguages(false);return;}
    if(action==='language-confirm'){closeLanguages(true);return;}
    if(action==='language-clear'){state.modal.enabled=protectedLanguages();refreshLanguageSettings('[data-ha-action="language-clear"]');return;}
    if(action==='language-remove'){if(!protectedLanguages().includes(id))state.modal.enabled=state.modal.enabled.filter(code=>code!==id);refreshLanguageSettings('[data-ha-default-language]');return;}
    ...
    save();state.feedback='';
    if(action==='edit'){state.selected=id;state.language=store.get(state.tab,id).defaultLanguage;state.deleting='';}
  ...
  function change(event) {
    ...
    if(state.modal){
      ...
      if(target.matches('[data-ha-default-language]')){state.modal.defaultLanguage=target.value;if(!state.modal.enabled.includes(target.value))state.modal.enabled.push(target.value);}
      else if(target.matches('[data-ha-language-option]')){if(target.checked&&!state.modal.enabled.includes(target.value))state.modal.enabled.push(target.value);else if(!target.checked&&!protectedLanguages().includes(target.value))state.modal.enabled=state.modal.enabled.filter(code=>code!==target.value);}
      else if(target.matches('[data-ha-language-all]'))state.modal.enabled=target.checked?store.languages.map(lang=>lang.code):protectedLanguages();
      else return;
      const selector=target.matches('[data-ha-default-language]')?'[data-ha-default-language]':target.matches('[data-ha-language-all]')?'[data-ha-language-all]':`[data-ha-language-option="${target.value}"]`;
      refreshLanguageSettings(selector);return;
    }
  ...
  function input(event){if(state.modal)return;if(event.target.matches('[data-ha-title],[data-ha-body]')){save();const count=root.querySelector('[data-ha-title-count]');if(count)count.textContent=root.querySelector('[data-ha-title]').value.length;}}
  ...
  function keydown(event){const tab=event.target.closest('[data-ha-edit-language]');if(tab&&!state.modal&&['ArrowLeft','ArrowRight','Home','End'].includes(event.key)){event.preventDefault();const enabled=current().enabledLanguages,index=enabled.indexOf(state.language);save();state.language=enabled[event.key==='Home'?0:event.key==='End'?enabled.length-1:(index+(event.key==='ArrowRight'?1:-1)+enabled.length)%enabled.length];render();root.querySelector(`[data-ha-edit-language="${state.language}"]`)?.focus();return;}if(state.modal){if(event.key==='Escape'){event.preventDefault();event.stopPropagation();closeLanguages(false);return;}if(event.key==='Tab'){const dialog=root.querySelector('[data-ha-language-dialog]'),nodes=[...dialog.querySelectorAll('button:not(:disabled),input:not(:disabled),select:not(:disabled),[contenteditable="true"]')].filter(node=>!node.hidden&&node.getClientRects().length);const first=nodes[0],last=nodes[nodes.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first?.focus();}}}if(event.key==='Enter'&&event.target.matches('[data-ha-search]')){event.preventDefault();state.search=event.target.value;render();}}
