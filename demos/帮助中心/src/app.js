...
const state={page:params.get('page')||'settings',docId:params.get('doc')||'',orientation:params.get('orientation')==='landscape'?'landscape':'portrait',lang:params.get('lang')||'zh',package:params.get('package')||'domestic',protocol:params.get('protocol')||'android-official',scenario:'normal',surface:params.get('surface')==='admin'?'admin':'app'};
...
const docs=()=>[...HelpStore.published.docs].filter(d=>(!d.package||d.package===state.package)&&(!HelpStore.visible||HelpStore.visible(d,state.package,state.protocol))&&navs().some(n=>n.id===d.navId)).sort((a,b)=>(a.order??0)-(b.order??0)||a.id.localeCompare(b.id));
