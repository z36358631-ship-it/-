...
- ~~原格式：`分享人：XX GPU：XX 下载次数: N`~~
- **新格式：`分享人：XX 处理器：[cpu字段] 下载次数: N`**
- 处理器型号动态读取 `listData` 对应方案的 `cpu` 字段
- 若 `cpu` 字段为空，则不显示「处理器：」部分，直接显示下载次数
- 本地创建方案（`dl === 0`）显示「本地创建方案」，不显示分享人/处理器信息

> 背景：GPU 型号不能代表设备兼容性的核心差异，处理器架构（如 Snapdragon / Dimensity / Kirin）才是影响转译兼容性的关键维度。
