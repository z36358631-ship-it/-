...
- 本期不接入 iOS 广告。
- 本期不同时接入更多广告聚合 SDK。
- 本期不在游戏启动中、运行中、失败、重连、下载、领取、配置、支付和用户游戏库列表投放广告。
- 本期不把预测收入写入财务承诺。
...
从用户点击“立即游戏/秒玩/启动”并创建启动请求开始，到首个可操作画面出现前，广告曝光必须为 0。固件安装、组件安装、资源检查、排队、调度、客户端启动、连接、加载、失败和重连均属于启动链路。
...
| 字段 | 类型/范围 | 必填 | 说明 |
|---|---|---|---|
| `placement_id` | 字符串，创建后不可改 | 是 | H1/P1/G1/O1/D1/S1 |
| `enabled` | 布尔 | 是 | 广告位总开关 |
...
频控按有效 `ad_impression` 计数；请求失败和无填充不扣次数。G1 主动奖励次数独立统计，但受全局容量和奖励次数限制。
...
- `AC-F011-01 [端到端]`：禁投上下文中所有广告位有效曝光数为 0。
...
8. O1 仅在合格老用户真正冷启动且广告已就绪时展示。
9. 所有广告展示和收入可关联到广告位、用户、请求、实验组和策略版本。
10. 后台支持广告位、网络、地区和版本一键熔断，并可审计、可回滚。
11. 游戏启动中、运行中、失败、重连及其他禁投流程广告曝光为 0。
12. 收入、留存、自然内容、游戏启动、Crash 和 ANR 均有实验对照数据。
