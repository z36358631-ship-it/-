...
- **子 Tab**：🟠 综合、官方、求助、攻略（原为”综合、精选”，V2.0改版）。

    - 🟠 **综合**：展示该分区全部帖子
    - 🟠 **官方**：仅展示 `isOfficial=true` 的帖子（带”官方”身份标签的用户发布的）
    - 🟠 **求助**：仅展示 `isHelp=true` 的帖子（用户发帖时选择”发布到：求助”的）
    - 🟠 **攻略**：仅展示 `isGuide=true` 的帖子（用户发帖时选择”发布到：攻略”的）
    - 交互：点击Tab → 帖子列表实时筛选刷新，不影响分区header；切换时列表scrollTo(0,0)；无内容时显示空态”当前分类暂无内容”

- 🟠 **置顶区**（V2.0新增）：

    - **位置**：专区Header和子Tab之间
    - **展示条件**：当前分区有 `isPinned=true` 的帖子时才展示
    - **单条样式**：左侧橙色”置顶”标签（背景 `rgba(255,149,0,0.15)`，文字 `var(--score-color)`，font-size 10px，padding 2px 6px，border-radius 4px）+ 帖子标题（font-size 13px，单行截断）+ 右侧灰色箭头 `>`
    - **交互**：点击整行 → 跳转至该帖子详情页
    - **多条置顶**：支持多条，纵向排列

- 🟠 **帖子标签展示**（V2.0新增）：

    - **求助标签**：背景 `#f5222d`，文字白色，font-size 11px，padding 2px 6px，border-radius 4px。位于帖子标题文字前方
    - **攻略标签**：背景 `#52c41a`，文字白色，样式同上
    - **官方标签**：背景 `var(--primary)`，文字 `#000`，font-size 9px，padding 1px 4px。位于用户昵称右侧
...
- 🟠 **”发布到”功能**（V2.0新增）：
    ...
    - **位置**：与”☐ 包含AI合成内容”**同一行**（左侧AI checkbox，右侧发布到选项）
    - **展示形式**：三个pill并排平铺（非下拉），单选关系
    - **各pill样式**：
        - 全部（默认选中）：金色高亮背景 `rgba(212,180,115,0.15)`，border-color `var(--primary)`
        - 求助：选中时红色高亮背景 `rgba(245,34,45,0.15)`，border-color `#f5222d`，文字色 `#f5222d`
        - 攻略：选中时绿色高亮背景 `rgba(82,196,26,0.15)`，border-color `#52c41a`，文字色 `#52c41a`
    - **数据写入**：
        - 选中”求助” → 发布后帖子 `isHelp=true`，标题前自动带红色”求助”标签
        - 选中”攻略” → 发布后帖子 `isGuide=true`，标题前自动带绿色”攻略”标签
        - 选中”全部” → 帖子无特殊标签
    - **交互**：点击切换，toast提示”将发布到「XX」”
...
|🟠 `sub\_tab\_switch`|三级sub-tab切换|点击综合/官方/求助/攻略|`game\_id`, `sub\_tab\_name`|
|🟠 `pin\_post\_click`|置顶帖点击|点击置顶区帖子标题|`post\_id`, `game\_id`|
|🟠 `publish\_to\_select`|发布到选择|发帖时切换”发布到”选项|`publish\_target`(全部/求助/攻略)|
