...
|2026\.06\.08|V2\.2|郑群超|分区管理重构+帖子管理改版+管理权限新增（帖子移动/官方内容发布）|竞品对标+运营管理提效|
...
- **B端运营（李四）：** 李四在后台看到用户举报张三的帖子里有违规评论。他通过【举报管理】查实后，一键对该评论执行了“屏蔽处理（仅作者可见）”。为了激励优质内容，李四给张三的账号下发了“硬核高玩”的专属身份标签，并把该帖子【置顶】到分区。
...
- **B端“生态治理”链路：**
新增/编辑分区 \-\&gt; 身份标签赋权 \-\&gt; 接收前端违规举报工单 \-\&gt; 帖子判定（删除/屏蔽/置顶/加精） \-\&gt; 奖品库存维护与物流单号录入。
...
- **【B端 \- 社区中台管理模块】**
    ...
    - 内容治理：举报工单流转、动态风控审核（通过/屏蔽/违规/删除）。
...
|<font color=”#4e73ff”>**黑名单管理（新增）**</font>|（见demo）|- <font color=”#4e73ff”>**新增模块**</font>，国内/海外后台均可用<br>- **模块结构**：三个平铺Tab（IP封禁 / 关键词过滤 / 处罚记录）<br>- **IP封禁**：<br>    - 支持封禁单个IP或IP段（如10.0.0.0/24）<br>    - 处罚对象（平铺单选）：针对动态 / 针对人<br>    - 针对动态：删帖 / 屏蔽（平铺单选）<br>    - 针对人：禁止发帖，时长平铺选择（1天/3天/7天/30天/永久/自定义天数）<br>    - 列表字段：ID、IP地址/段、封禁原因、处罚对象、触发动作、封禁时间、状态、操作人、操作<br>- **关键词过滤**：<br>    - 支持精确匹配/模糊匹配/正则匹配（平铺单选）<br>    - 处罚对象（平铺单选）：针对动态 / 针对人<br>    - 针对动态：删帖 / 屏蔽（平铺单选）<br>    - 针对人：禁止发帖，时长平铺选择（1天/3天/7天/30天/永久/自定义天数）<br>    - 列表字段：ID、关键词、匹配模式、处罚对象、触发动作、触发次数、添加时间、状态、操作<br>- **处罚记录**：<br>    - 手动处罚入口，处罚对象（平铺单选）：针对人 / 针对动态<br>    - 针对人：输入UID，选择禁止发帖(周期/永久)，周期可选天数或自定义<br>    - 针对动态：输入动态ID，选择删帖/屏蔽<br>    - 列表字段：ID、用户、处罚类型、处罚原因、触发规则、开始时间、结束时间、状态、操作<br>- **交互规范**：所有选项均为平铺radio样式（带边框高亮选中态），不使用下拉<br>- **触发动作仅保留**：删帖、屏蔽（移除拦截和标记审核）//2026.5.27修改|
...
|状态干预<br>\(屏蔽/删除/恢复\)|动态ID, 干预动作|点击操作项时绑定的具体动作类型枚举（如 Silence, Delete, Restore）以及该行数据的动态ID。|将动态\[D88204\] 的状态修改为 \[屏蔽\]|
|**解除干预**|动态ID, 干预动作|点击“解除屏蔽”或“恢复”时获取。|解除了动态 \[D88204\] 的屏蔽状态，恢复为正常|
|排序干预 \(置顶/取消\)|动态ID, 干预动作|触发“置顶”或“取消置顶”时绑定的动态ID。|将动态 \[D88201\] 设置为全网置顶|
|**批量操作**|动态ID数组, 执行动作, 生效总数|读取当前列表勾选框（Checkbox）状态为 checked 的对应动态ID集合，并计算 Array\.length。|批量屏蔽了 5 条动态，包含动态ID:\[D88201, D88202, \.\.\.\]|
|**导出动态数据**|筛选条件集合 JSON|点击“导出数据”按钮时，抓取当前顶部搜索栏（分区/时间/状态）所有生效的 Value 值。|导出了动态数据，当前条件：分区\[全部\]、语言\[中文\]、状态\[已屏蔽\]|
|**新增热门话题**|话题内容, 话题ID|表单提交输入的话题内容（带\#号），以及后端入库生成的话题ID。|新建了热门话题 \[\#发现好游戏\#\]，话题ID: TOPIC\_1001|
|**动态状态变更**|动态ID, 操作动作|点击操作列/批量操作时获取的对应动态ID数组，及触发的具体按钮名称（屏蔽、删除、审核通过等）。|将动态 \[D88201\] 的状态由疑似待审核变更为 \[审核通过\]|
...
|------|------|--------|------------|------|
| 2026.05.25 | V2.0 | 郑群超 | 导航改版+游戏圈重构+求助/攻略分类+置顶区 | 基于CTO会议结论 |
| 2026.06.08 | V2.2 | 郑群超 | 分区管理重构+帖子管理改版+管理权限新增 | 竞品对标+运营管理提效 |

功能demo：社区2.0demo.html

---

### V2.2 新增/变更模块（2026.06.08更新）

> 以下为V2.2改版新增内容，修改内容以<font color="#4e73ff">蓝色加粗</font>标识。

#### C端：分区管理页重构

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**分区管理页重构**</font>|- **入口**：游戏圈二级Tab"全部"右侧偏好管理icon<br>- **页面结构变更**：<br>    - 顶部nav：左侧返回箭头 + 中间"分区管理"标题 + 右侧"完成"按钮<br>    - 已置顶栏：显示当前已置顶分区药丸标签（带×移除），右侧显示 n/5 计数<br>    - 搜索框：支持按分区名称实时过滤<br>    - 双Tab切换："全部分区" / "我加入的"<br>    - 列表区：每项显示分区icon + 名称 + 简介，右侧"置顶/已置顶"按钮<br>- **操作变更**：操作从"关注"改为"置顶"，置顶上限5个<br>- **点击icon行为**：点击分区icon/名称进入独立分区详情子页面（非首页tab切换）|
|<font color="#4e73ff">**分区详情子页面（新增）**</font>|- **独立overlay页面**（z-index: 410）<br>- **头部**：分区icon + 名称 + 描述（与首页分区header描述一致，通过getPartitionDesc()取同一份数据） + 加入/已加入按钮<br>- **子Tab**：综合 / 官方 / 求助 / 攻略（与首页分区header一致）<br>- **帖子列表**：展示该分区下帖子，点击可跳转帖子详情<br>- **横屏布局**：强制单列显示<br>- **规则**：官方帖子不可被屏蔽/删除/移动板块|

#### C端：管理权限测试

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**管理权限测试（新增）**</font>|- **入口**：设备容器外右上角toggle开关（仅demo测试用）<br>- **开启后影响**：<br>    - 帖子`···`更多菜单追加管理操作项<br>    - 帖子详情页操作面板追加管理操作区<br>- **管理操作项**：<br>    - 置顶 / 取消置顶<br>    - 屏蔽 / 取消屏蔽（官方帖子不显示）<br>    - 删除（官方帖子不显示，非官方帖子需二次确认弹窗）<br>    - 禁言X天（X=后台身份配置的天数，直接外显，点击后二次确认即执行）<br>    - 板块调整（官方帖子不显示）：可移到综合/求助/攻略，不可移到官方<br>- **禁言交互细节**：<br>    - 按钮文案直接显示"禁言X天"（天数从后台身份配置读取，前端不可修改）<br>    - 点击后弹出二次确认弹窗，确认后执行禁言<br>    - 禁言成功toast显示具体到期时间："已禁言「用户名」至 YYYY-MM-DD HH:mm"<br>    - 已被禁言的用户：按钮置灰显示"禁言中"（不可点击），点击toast提示"该用户已被禁言至 YYYY-MM-DD HH:mm"<br>- **规则总结**：<br>    - 官方帖子：仅可置顶和禁言，不可屏蔽/删除/移动<br>    - 非官方帖子：不可移到官方板块<br>    - 禁言天数由后台身份管理配置，前端只读展示|

#### B端：帖子管理改版

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**"动态管理"改名为"帖子管理"**</font>|- 侧边栏菜单、面包屑、操作记录模块名全部替换为"帖子管理"|
|<font color="#4e73ff">**帖子分类Tab（新增）**</font>|- **位置**：筛选条件下方、批量操作行上方<br>- **Tab列表**：全部（默认） / 综合 / 官方 / 求助 / 攻略<br>- **交互**：点击Tab实时过滤帖子列表<br>- **样式**：选中态品牌色+底部下划线|
|<font color="#4e73ff">**分类列（新增）**</font>|- 表格新增"分类"列，位于"所属分区"列右侧<br>- 显示对应分类标签（综合=灰色，官方=蓝色，求助=红色，攻略=绿色）|
|<font color="#4e73ff">**帖子移动 - 行内下拉**</font>|- 每条帖子操作列新增下拉选择器"移动到..."<br>- 选项：综合 / 官方 / 求助 / 攻略<br>- 选择即生效，无需二次确认<br>- 已删除帖子不显示移动操作|
|<font color="#4e73ff">**批量移动（新增）**</font>|- 批量操作区新增"批量移动到..."下拉<br>- 勾选帖子后激活，选择目标分类后一次移动多条<br>- 与批量屏蔽/批量删除并列|

#### B端：身份权限新增

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**权限checkbox新增**</font>|- 身份编辑弹窗"权限内容"区域新增三项：<br>    - **帖子移动**：拥有此权限的用户可在前端将帖子在综合/求助/攻略板块间移动<br>    - **官方内容发布**：拥有此权限的用户发帖时，内容自动归类到官方Tab下<br>    - **禁言**：勾选后展开"禁言天数上限"输入框（1-999天），拥有此权限的管理员可对用户执行禁言|
|<font color="#4e73ff">**权限说明区域（新增）**</font>|- 位置：身份编辑弹窗"功能描述"字段下方<br>- 展示形式：只读灰色背景卡片，一行一条说明全部权限含义<br>- 内容：<br>    - 身份标识 — 用户昵称旁展示对应身份标签<br>    - 置顶 — 可将帖子置顶到分区顶部<br>    - 删动态 — 可删除其他用户的帖子<br>    - 屏蔽动态 — 可将帖子设为屏蔽(仅发布者可见)<br>    - 帖子移动 — 可将帖子在综合/求助/攻略板块间移动<br>    - 官方内容发布 — 该身份用户发帖时，内容自动归类到官方Tab下<br>    - 禁言 — 可对用户执行禁言处罚，自定义1-999天|
|<font color="#4e73ff">**默认权限配置**</font>|- 官方认证：拥有全部7项权限（禁言上限默认30天）<br>- 知名创作者：身份标识 + 帖子移动|
