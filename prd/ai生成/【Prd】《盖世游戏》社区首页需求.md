...
|<font color="#4e73ff">**分区详情子页面（新增）**</font>|- **独立overlay页面**（z-index: 410）<br>- **头部**：分区icon + 名称 + 描述（与首页分区header描述一致，通过getPartitionDesc()取同一份数据） + 加入/已加入按钮<br>- **子Tab**：综合 / 官方 / 求助 / 攻略（与首页分区header一致）<br>- **帖子列表**：展示该分区下帖子，点击可跳转帖子详情<br>- **横屏布局**：强制单列显示<br>- **规则**：官方帖子不可被静默/删除/移动板块|

#### C端：管理权限测试

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**管理权限测试（新增）**</font>|- **入口**：设备容器外右上角toggle开关（仅demo测试用）<br>- **开启后影响**：<br>    - 帖子`···`更多菜单追加管理操作项<br>    - 帖子详情页操作面板追加管理操作区<br>- **管理操作项**：<br>    - 置顶 / 取消置顶<br>    - 静默 / 取消静默（官方帖子不显示）<br>    - 删除（官方帖子不显示，非官方帖子需二次确认弹窗）<br>    - 板块调整（官方帖子不显示）：可移到综合/求助/攻略，不可移到官方<br>- **规则总结**：<br>    - 官方帖子：仅可置顶，不可静默/删除/移动<br>    - 非官方帖子：不可移到官方板块|
...
|<font color="#4e73ff">**批量移动（新增）**</font>|- 批量操作区新增"批量移动到..."下拉<br>- 勾选帖子后激活，选择目标分类后一次移动多条<br>- 与批量静默/批量删除并列|

#### B端：身份权限新增

|**模块名称**|**展示&交互说明**|
|---|---|
|<font color="#4e73ff">**权限checkbox新增**</font>|- 身份编辑弹窗"权限内容"区域新增两项：<br>    - **帖子移动**：拥有此权限的用户可在前端将帖子在综合/求助/攻略板块间移动<br>    - **官方内容发布**：拥有此权限的用户发帖时，内容自动归类到官方Tab下|
|<font color="#4e73ff">**权限说明区域（新增）**</font>|- 位置：身份编辑弹窗"功能描述"字段下方<br>- 展示形式：只读灰色背景卡片，一行一条说明全部权限含义<br>- 内容：<br>    - 身份标识 — 用户昵称旁展示对应身份标签<br>    - 置顶 — 可将帖子置顶到分区顶部<br>    - 删动态 — 可删除其他用户的帖子<br>    - 静默动态 — 可将帖子设为静默(仅发布者可见)<br>    - 帖子移动 — 可将帖子在综合/求助/攻略板块间移动<br>    - 官方内容发布 — 该身份用户发帖时，内容自动归类到官方Tab下|
|<font color="#4e73ff">**默认权限配置**</font>|- 官方认证：拥有全部6项权限<br>- 知名创作者：身份标识 + 帖子移动|
