...
| 2026/9/2 | 恢复 P02-01 四任务 Tab、Key 来源枚举与已确认的 CDKEY 接口路径 | V1.5 | 郑群超 |
...
| 要素 | 内容说明 |
|---|---|
| 功能简介 | 在同一 Game 上下文内通过“商品与供给”“Key 批次”“渠道 API”“接口说明”四个任务 Tab，查看两条 Key 链的商品、授权、库存摘要和可操作入口。 |
...
| 需求描述 | **图示：**<br>![开发者供给状态](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@f25bc4a2427cdc0061d6919c61dc13781166adc6/public/prd/genuine-game-distribution-phase1/03-cdkey-supply/03-developer-supply-status.png)<br>*图 3.2.1-1：V1.2 旧图仅用于布局参考；正式视觉以四任务 Tab 和两套独立账本摘要为准。*<br>**界面文字与布局：** 顶部固定展示当前 Game、APPID、Product、SKU、授权状态、剩余 Key 配额、可用渠道、发行暂停原因和线下商业结果摘要；下方固定为“商品与供给”“Key 批次”“渠道 API”“接口说明”四个任务 Tab。“商品与供给”内分别展示外部 Key 入站与盖世 Key 出站摘要，不展示跨账本合计库存。<br>**详细说明：** 四个 Tab 共用 P02-01 路由与当前 Game 上下文；页面只统一任务入口，不合并两套库存。外部链只读，盖世链只开放计划授权内自助操作。<br>**展示说明：**<br>1. “商品与供给”展示 Product／SKU、Key 来源、供给状态、可售门禁、异常和更新时间；外部来源展示 `external_key_batch_id`，盖世来源展示 `gamehub_key_program_id`。<br>2. “Key 批次”展示剩余配额、批次状态、用途、渠道、数量、有效期和一次性下载状态；“渠道 API”展示凭据末四位、授权范围、有效期和状态。<br>3. “接口说明”展示鉴权、分配、查询、确认、幂等、错误码和恢复规则；示例不得包含生产 Secret、真实 Key 或真实用户数据。<br>4. Key 明文、完整 `client_secret`、外部供应商凭据和用户个人数据不在总览或再次进入页面时展示。<br>**交互说明：**<br>1. 切换四个任务 Tab 保留当前 Game，刷新后保留所选 Tab；进入任一自助动作时重新校验 SKU 的 Key 链，类型不匹配时阻止进入。<br>2. 外部异常进入 3.2.2；盖世批次、渠道凭据和接口说明复用 3.2.5 的规则。发行方无商品启停、外部入库和对账处置权限。<br>3. 任一 Tab 加载失败只显示本 Tab 失败态，不用另一账本数据兜底；越权时不返回对象是否存在。 |
| 输出／后置条件 | 发行方获得两条供给链的独立状态，并进入有权限的异常详情或盖世 Key 自助页面。 |
| 补充说明 | 同一 SKU 只能归属一条 Key 链。四个任务 Tab 不代表两账本可合并；“商品与供给”内任何总计都必须标注 Key 链。 |
...
| 需求描述 | **图示：**<br>![供应来源关联](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@f25bc4a2427cdc0061d6919c61dc13781166adc6/public/prd/genuine-game-distribution-phase1/03-cdkey-supply/03-ops-supply-link.png)<br>*图 3.2.5-1：V1.2 图片不包含盖世 Key 计划／渠道 API，V1.4 须重新出图。*<br>**界面文字与布局：** 本功能复用 P02-01 的“商品与供给”“Key 批次”“渠道 API”“接口说明”四个任务 Tab；顶部始终显示五级 ID、`gamehub_key_program_id`、总配额、剩余配额、有效期和状态。<br>**详细说明：** 发行方只在预授权计划内自助；平台控制范围和启停，盖世兑换服务负责 Key 生成、渠道交付状态和权益授予。Key 来源枚举固定为 `gamehub_generated`＝盖世兑换服务生成的盖世平台 Key，`external_imported`＝外部平台或供应商提供并受控导入的 Key；盖世不得生成 Steam、Epic、GOG 等外部平台 Key。<br>**展示说明：**<br>1. 批次字段包含 `gamehub_key_batch_id`、名称、用途、`channel_id`、区域、数量、有效期、批次状态、可用／分配待确认／已交付／已兑换数量和受控下载状态。<br>2. 凭据列表显示名称、`channel_id`、`client_id`、Secret 末四位、Scope、配额、有效期、最近调用和状态；完整 `client_secret` 只在创建／轮换成功时展示一次。<br>3. 接口说明展示正式版本、请求／响应、HMAC 签名、时间戳与随机数、防重放、`request_id` 幂等、限流、分配／查询／确认、错误码和响应丢失恢复；示例不得包含生产 Secret、真实 Key 或真实用户数据。<br>**交互说明：**<br>1. 发行方创建批次时提交名称、用途、渠道、区域、数量和有效期；服务端按计划原子校验并扣减可生成配额。生成失败不产生半批次、不扣配额。<br>2. 受控批次下载完成后，文件内 Key 直接进入“已交付”且不可再次展示；下载响应未完成时可用同一 `action_id` 查询并恢复原下载任务，系统不得重新生成一批 Key。成功后遗失只能作废未兑换 Key 并新建批次。<br>3. 渠道凭据支持创建、轮换、暂停、恢复和撤销；Scope 不得超出计划。暂停拒绝新分配；撤销不可恢复，但不反向改写历史分配与兑换。<br>4. 渠道按 `channel_id + request_id` 调用分配接口；内容一致时在幂等保留期内返回原 Key 与状态，内容冲突拒绝。首次成功原子扣减可用 Key 并进入“分配待确认”；确认后进入“已交付”，确认超时只告警、不自动回库。<br>5. 用户可兑换“分配待确认”或“已交付”且仍有效的 Key；成功后 Key、兑换记录和 `app_id + product_id + sku_id` 权益原子写入。若兑换早于渠道确认，交付记录同时记为“已确认／兑换佐证”；服务异常先查询原兑换结果再幂等重试。 |
...
接口说明固定展示 `POST /openapi/v1/cdkeys/allocate`、`GET /openapi/v1/cdkeys/allocations/{request_id}`、`POST /openapi/v1/cdkeys/allocations/{request_id}/confirm` 三个一期草案方法；正式域名、签名时间窗和联调环境仍以研发与安全评审后的同一版本为准。

| 方法 | 路径 | 用途 | 幂等与安全结果 |
|---|---|---|---|
| POST | `/openapi/v1/cdkeys/allocate` | 按渠道订单分配一个盖世 Key | `channel_id + request_id` 内容一致返回原 Key 与原状态；首次成功原子扣减一个可用 Key 并进入“分配待确认” |
| GET | `/openapi/v1/cdkeys/allocations/{request_id}` | 恢复原分配结果并查询交付状态 | 仅同一渠道下具备原 Scope 的当前有效凭据可查；Key 恢复窗口内返回原 Key，窗口结束后只返回掩码和状态，不产生再次扣减 |
| POST | `/openapi/v1/cdkeys/allocations/{request_id}/confirm` | 确认渠道已安全收到原分配结果 | 首次成功将交付记录改为“已确认”、Key 改为“已交付”；重复确认返回原结果，不改变 Key 或配额 |
