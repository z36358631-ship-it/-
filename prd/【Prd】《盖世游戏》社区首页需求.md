...
> 以下为V2.0改版新增内容，标记🟠为本次新增/变更项。

|**模块名称**|**图示**|**展示\&交互说明**|
|---|---|---|
|🟠 **顶部主Tab改版**|（见demo）|- 🟠 **Tab结构变更**：原"游戏/社区"改为 **关注 / 游戏圈**（移除"探索"Tab）<br>- **关注Tab**：展示已关注用户/游戏圈的动态流<br>    - 空态：居中展示"还没有关注内容"+ 金色文字"去游戏圈看看 →"（点击切换至游戏圈Tab）<br>- **游戏圈Tab**（默认选中）：展示游戏圈二级tab + 帖子列表<br>- **交互**：点击Tab切换，不刷新页面，Tab下内容区域切换|
|🟠 **游戏圈二级Tab（icon+文字）**|（见demo）|- 🟠 **展示形式变更**：每个Tab改为 emoji icon + 分区名称（如 🎮 Steam、🚗 GTA 5、⚙️ 配置分享）<br>- **默认Tab**："全部"（右侧带 ⚙️ 偏好管理icon，点击拉起分区偏好管理弹窗）<br>- **Tab来源**：由用户pin的分区决定（偏好管理配置），后台配置icon<br>- **溢出处理**：Tab数量超出屏幕宽度时，横向可滚动<br>- **交互**：点击某个Tab → 帖子列表按该分区筛选刷新；若该分区有专区header则在帖子列表上方展示|
|🟠 **置顶区**|（见demo）|- 🟠 **新增模块**<br>- **位置**：分区header（封面+描述+加入按钮）和 三级sub-tab（综合/官方/求助/攻略）**之间**<br>- **展示条件**：当前分区有 `isPinned=true` 的帖子时才展示；"全部"Tab下不展示<br>- **单条样式**：<br>    - 左侧：橙色"置顶"标签（背景 `rgba(255,149,0,0.15)`，文字 `var(--score-color)`，font-size 10px，padding 2px 6px，border-radius 4px）<br>    - 中间：帖子标题（font-size 13px，单行截断 `text-overflow:ellipsis`）<br>    - 右侧：灰色右箭头 `>`<br>- **交互**：点击整行 → 跳转至该帖子详情页<br>- **多条置顶**：支持多条，纵向排列|
|🟠 **三级Sub-Tab（综合/官方/求助/攻略）**|（见demo）|- 🟠 **新增模块**<br>- **位置**：分区header（含置顶区）下方<br>- **Tab列表**：综合（默认选中） / 官方 / 求助 / 攻略<br>- **筛选逻辑**：<br>    - 综合：展示该分区全部帖子（不筛选）<br>    - 官方：仅展示 `isOfficial=true` 的帖子（带"官方"身份标签的用户发布的）<br>    - 求助：仅展示 `isHelp=true` 的帖子（用户发帖时选择"发布到：求助"的）<br>    - 攻略：仅展示 `isGuide=true` 的帖子（用户发帖时选择"发布到：攻略"的）<br>- **交互**：<br>    - 点击Tab → 帖子列表实时筛选刷新，不影响分区header<br>    - 切换时帖子列表 `scrollTo(0,0)`<br>    - 无内容时显示空态"当前分类暂无内容"|
|🟠 **帖子标签展示**|（见demo）|- 🟠 **新增标签类型**<br>- **求助标签**：背景 `#f5222d`，文字白色，font-size 11px，padding 2px 6px，border-radius 4px。展示位置：帖子标题文字前方，margin-right 6px。触发条件：帖子 `isHelp=true`<br>- **攻略标签**：背景 `#52c41a`，文字白色，样式同上。触发条件：帖子 `isGuide=true`<br>- **官方标签**：背景 `var(--primary)`，文字 `#000`，font-size 9px，padding 1px 4px。展示位置：用户昵称右侧。触发条件：用户 `isOfficial=true`<br>- **置顶标签**：仅在置顶区显示（见上方置顶区模块），帖子列表内不重复展示|
|🟠 **发帖页 - "发布到"功能**|（见demo）|- 🟠 **新增功能**<br>- **位置**：底部工具栏正上方，与"☐ 包含AI合成内容"**同一行**（左侧AI checkbox，右侧发布到选项）<br>- **展示形式**：三个pill并排平铺（非下拉），单选关系<br>- **各pill样式**：<br>    - 全部（默认选中）：金色高亮背景 `rgba(212,180,115,0.15)`<br>    - 求助：选中时红色高亮背景 `rgba(245,34,45,0.15)`，border-color `#f5222d`<br>    - 攻略：选中时绿色高亮背景 `rgba(82,196,26,0.15)`，border-color `#52c41a`<br>- **数据写入**：<br>    - 选中"求助" → 发布后帖子 `isHelp=true`，标题前自动带红色"求助"标签<br>    - 选中"攻略" → 发布后帖子 `isGuide=true`，标题前自动带绿色"攻略"标签<br>    - 选中"全部" → 帖子无特殊标签<br>- **交互**：点击切换，toast提示"将发布到「XX」"|
|🟠 **横屏切换按钮位置**|（见demo）|- 🟠 **位置变更**<br>- **原位置**：tabs-toolbar右侧（"横屏模式"文字按钮）<br>- **新位置**：顶部搜索栏右侧，消息通知🔔 icon **左侧**<br>- **icon样式**：📱（fas fa-tablet-alt），28×28圆形背景 `rgba(255,255,255,0.08)`<br>- **交互**：点击 → 切换横竖屏模式|
|🟠 **横屏模式布局**|（见demo）|- 🟠 **布局变更**<br>- **帖子列表**：一排**1个帖子**（单列 `flex-direction: column`，原为双列grid）<br>- **底部导航**：隐藏<br>- **顶部header**：padding-top缩小为12px<br>- **分区header**：加圆角边框 `border-radius:12px; border:1px solid var(--border-color)`|
|🟠 **底部导航栏（5Tab）**|（见demo）|- 🟠 **结构变更**<br>- **原结构**：3个Tab（探索 / 手柄 / 我的）<br>- **新结构**：5个Tab → 首页（合并原游戏库） / 社区（默认选中） / 玩游戏 / 排行榜 / 我的<br>- **选中态**：icon+文字高亮为主色 `var(--primary)`<br>- **未选中态**：灰色 `var(--text-sub)`<br>- **横屏时**：底部导航隐藏|
