...
| tab | 说明 |
|-----|------|
| 关注 | 已关注用户/游戏圈的动态流；空态显示"还没有关注内容，去游戏圈看看→" |
| 游戏圈 | 默认选中。游戏圈分区列表+帖子流 |
...
二级tab改为 **icon+文字** 形式（原纯文字）：

| tab | icon | 说明 |
|-----|------|------|
| 全部 | — | 默认选中，显示所有分区帖子。右侧保留⚙️偏好管理入口 |
| 🎮 Steam | emoji | 具体游戏圈 |
| 🚗 GTA 5 | emoji | 具体游戏圈 |
| ⚙️ 配置分享 | emoji | 具体游戏圈 |
| 🏪 Epic | emoji | 具体游戏圈 |
...
icon来源：后台配置分区时上传icon图片或选择emoji。

## 7.3 三级sub-tab（进入具体分区后）

进入具体游戏圈分区后，分区header下方显示三级sub-tab：

| tab | 筛选逻辑 | 说明 |
|-----|---------|------|
| 综合 | 该分区全部帖子 | 默认选中 |
| 官方 | 仅显示`isOfficial=true`的帖子 | 带"官方"身份标签的用户发布的帖子 |
| 求助 | 仅显示`isHelp=true`的帖子 | 用户发帖时选择"发布到：求助"的帖子 |
| 攻略 | 仅显示`isGuide=true`的帖子 | 用户发帖时选择"发布到：攻略"的帖子 |

交互说明：
- 点击sub-tab实时筛选帖子列表，不刷新分区header
- 切换sub-tab时帖子列表滚动回顶部
- 无内容时显示空态"当前分类暂无内容"
...
在分区header（游戏名+描述+加入按钮）和三级sub-tab（综合/官方/求助/攻略）**之间**。

### 展示规则

| 条件 | 展示 |
|------|------|
| 当前分区有置顶帖（`isPinned=true`） | 显示置顶区 |
| 当前分区无置顶帖 | 不显示置顶区 |
| 在"全部"tab下 | 不显示置顶区 |
...
- "置顶"标签：橙色背景(rgba(255,149,0,0.15))，橙色文字，font-size:10px，padding:2px 6px，border-radius:4px
- 标题：font-size:13px，单行截断(text-overflow:ellipsis)
- 右侧箭头：fas fa-chevron-right，灰色
- 点击整行：跳转至帖子详情页
...
发帖页底部toolbar上方，与"包含AI合成内容"checkbox同一行：

```
┌─────────────────────────────────────────────────┐
│ ☐ 包含AI合成内容        发布到：[全部][求助][攻略] │
├─────────────────────────────────────────────────┤
│ 📷  😀  #          选择分区  关联游戏  配置方案   │
└─────────────────────────────────────────────────┘
```
...
| 元素 | 说明 |
|------|------|
| "全部" pill | 默认选中，金色高亮背景(rgba(212,180,115,0.15)) |
| "求助" pill | 选中时红色高亮背景(rgba(245,34,45,0.15))，边框红色 |
| "攻略" pill | 选中时绿色高亮背景(rgba(82,196,26,0.15))，边框绿色 |

- 三个pill为单选关系，点击切换
- 选中"求助"→发布后帖子标题前自动带红色"求助"标签，帖子`isHelp=true`
- 选中"攻略"→发布后帖子标题前自动带绿色"攻略"标签，帖子`isGuide=true`
- 选中"全部"→帖子无特殊标签
...
| 标签 | 背景色 | 文字色 | 触发条件 |
|------|--------|--------|---------|
| 求助 | #f5222d | white | 帖子`isHelp=true` |
| 攻略 | #52c41a | white | 帖子`isGuide=true` |
| 置顶 | rgba(255,149,0,0.15) | var(--score-color) | 帖子`isPinned=true`（仅在列表标题前显示） |
| 官方 | var(--primary) | #000 | 用户`isOfficial=true`（显示在昵称后） |

标签位置：帖子标题文字前方，font-size:11px，padding:2px 6px，border-radius:4px，margin-right:6px。

## 7.7 横屏模式改版

### 切换按钮位置

从原tabs-toolbar右侧移至**顶部搜索栏右侧、消息通知bell icon左侧**：
...
| 元素 | 横屏表现 |
|------|---------|
| 帖子列表 | **一排1个帖子**（单列flex布局，非grid双列） |
| 底部导航 | 隐藏 |
| 顶部header | padding-top缩小为12px |
| 分区header | 加圆角边框 |
...
| 事件ID | 事件名称 | 触发时机 | 关键参数 |
|--------|---------|---------|---------|
| sub_tab_switch | 三级sub-tab切换 | 点击综合/官方/求助/攻略 | `partition`, `sub_tab_name` |
| pin_post_click | 置顶帖点击 | 点击置顶区帖子标题 | `post_id`, `partition` |
| publish_to_select | 发布到选择 | 发帖时切换"发布到"选项 | `publish_target`(全部/求助/攻略) |
| orientation_toggle | 横竖屏切换 | 点击横竖屏切换按钮 | `from_mode`, `to_mode` |
