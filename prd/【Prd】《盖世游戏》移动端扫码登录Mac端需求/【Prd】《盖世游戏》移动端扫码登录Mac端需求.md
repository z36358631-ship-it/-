...
|Mac端等待确认|![image\.png](图片和附件/image%204.png)<br>|**1\. 二维码遮罩：** 二维码容器覆盖完整的**黑色半透明遮罩**。<br>**2\. 等待文案：** 遮罩内部展示\*\*“二维码已识别”**和**“请尽快在手机上确认”**。**<br>**3\. 有效期： 待确认阶段继续受二维码生成起**2 分钟有效期**限制。**<br><del>**4\. 超时状态：** 超过有效期后展示“二维码已失效/点击刷新”。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>超过有效期后，二维码遮罩内展示“二维码已过期”和唯一“刷新二维码”按钮。</span><br>**5\. 登录生效：** Mac 登录态在收到被绑定账号和设备的有效确认后生效。|
|Mac端登录已取消|![image\.png](图片和附件/image%203.png)|<del>**1\. 取消结果：** 移动端取消确认后，Mac 端展示“本次登录已取消，请刷新二维码”。<br>**2\. 重新登录：** 点击二维码区域或“刷新二维码”按钮生成新二维码。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>标题保持“扫码登录”；二维码遮罩内展示“登录已取消”和唯一“刷新二维码”按钮。App 返回“我的”页并提示一次 2 秒 Toast“登录已取消，请重新扫码”。</span><br>|
|移动端登录结果|![image\.png](图片和附件/image%208.png)<br>![image 10\.png](图片和附件/image%2010.png)<br>![image 12\.png](图片和附件/image%2012.png)|<span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>成功、授权失败、取消、过期均返回“我的”页，每个请求只显示一次 2 秒 Toast。文案依次为“Mac端登录成功”“登录未完成，请重新扫码”“登录已取消，请重新扫码”“二维码已过期，请重新扫码”。`ready_to_claim` 不提示成功。</span><br>|
|Mac端登录成功|![image\.png](图片和附件/image%206.png)<br>|<del>**1\. 提示形式：** Mac 端完成登录后显示 Toast“登录成功”。<br>**2\. 展示时长：** Toast 展示 2 秒后自动消失。<br>**3\. 状态结果：** 停留在登录前页面。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>Mac 持久化并 ACK、服务端进入 `used` 后，关闭扫码登录界面，直接恢复扫码前 `returnTo` 页面并刷新登录态和用户数据；无成功页、Toast、弹窗。返回目标无效或无权限时进入默认登录后页面。</span><br>|
...
|登录成功|![image 8\.png](图片和附件/image%208.png)<br>![image 6\.png](图片和附件/image%206.png)|<del>移动端显示成功并列出继承平台；Mac 展示成功弹窗。</del><br><strong style="color:#3370FF">V1\.2：App 回“我的”页，仅提示一次 2 秒 Toast“Mac端登录成功”；Mac 在 `used` 后直接恢复 `returnTo` 页面并刷新登录态，无成功页、Toast 或弹窗。</strong>|
|授权失败|![image 10\.png](图片和附件/image%2010.png)|<del>移动端和 Mac 端均显示“登录未完成”及说明正文。</del><br><strong style="color:#3370FF">V1\.2：App 回“我的”页，仅提示一次 2 秒 Toast“登录未完成，请重新扫码”；Mac 标题保持“扫码登录”，结果文案和唯一刷新按钮均在二维码遮罩内。</strong>|
...
|---|---|---|---|
|App|“我的”页－成功|![image 8\.png](图片和附件/image%208.png)|显示一次 2 秒 Toast“Mac端登录成功”；无结果页、弹窗和平台列表|
|App|“我的”页－授权失败|![image 10\.png](图片和附件/image%2010.png)|显示一次 2 秒 Toast“登录未完成，请重新扫码”|
|App|“我的”页－取消|与 Mac 取消态同链路，见 image 3\.png|显示一次 2 秒 Toast“登录已取消，请重新扫码”|
|App|“我的”页－过期|![image 12\.png](图片和附件/image%2012.png)|显示一次 2 秒 Toast“二维码已过期，请重新扫码”|
|Mac|扫码登录－授权失败|![image 10\.png](图片和附件/image%2010.png)|二维码遮罩内展示“登录未完成”和唯一“刷新二维码”按钮|
|Mac|扫码登录－取消|![image 3\.png](图片和附件/image%203.png)|二维码遮罩内展示“登录已取消”和唯一“刷新二维码”按钮|
|Mac|扫码登录－过期|![image 12\.png](图片和附件/image%2012.png)|二维码遮罩内展示“二维码已过期”和唯一“刷新二维码”按钮|
|Mac|扫码前业务页－成功返回|![image 6\.png](图片和附件/image%206.png)|直接恢复经过校验的 `returnTo` 页面并刷新登录态；Demo 以“游戏库”为例|
...
- 本 PRD 当前引用 13 张本地页面图和 1 张本地流程图；尚未发布为固定 Git SHA 的公开 HTTPS 地址，因此当前版本为本地评审稿，不能声明可直接导入飞书。
