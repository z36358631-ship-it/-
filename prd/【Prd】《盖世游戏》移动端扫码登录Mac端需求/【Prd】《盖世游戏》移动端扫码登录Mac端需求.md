...
|2026\.08\.06|V1\.3|郑群超 / Codex|发布扫码登录 Demo 与 14 张 PRD 图，全部图示替换为固定提交 SHA 的公开 HTTPS 地址|GUANWANGGAID-1|
...
- <del>二维码过期用户：用户未在有效期内扫码，Mac 端展示二维码已过期，用户点击二维码区域或“刷新二维码”按钮生成新二维码后重新扫码。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>请求过期后，App 返回“我的”页并提示一次“二维码已过期，请重新扫码”；Mac 仅在二维码遮罩内展示“二维码已过期”和“刷新二维码”按钮。用户点击按钮生成新二维码，二维码容器本身不可点击、不可聚焦。</span>
...
9. <del>用户点击“取消授权”后，待确认请求结束，当前二维码立即失效，移动端返回“我的”页，Mac 端展示“本次登录已取消，请刷新二维码”。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>用户点击“取消授权”后，请求进入 `cancelled`，App 返回“我的”页并提示一次“登录已取消，请重新扫码”；Mac 仅在二维码遮罩内展示“登录已取消”和“刷新二维码”按钮。</span>

10. 用户点击“确认登录”后，服务端复核二维码状态、扫码账号、移动端设备、登录 session 和 Mac 登录请求。账号或登录态变化时，移动端提示“登录状态已变化，请重新扫码”。

11. <del>Mac 登录成功时读取当前盖世账号的 Steam/Epic 绑定关系，并展示包含盖世账号昵称的成功弹窗。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>Mac 持久化会话并 ACK、服务端进入 `used` 后，App 返回“我的”页并提示一次“Mac端登录成功”；Mac 直接恢复扫码前 `returnTo` 页面并刷新登录态。</span>

12. <del>用户关闭登录成功弹窗后，Mac 端保持当前登录状态。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>Mac 不展示成功页、Toast 或弹窗；`returnTo` 缺失、失效或无权限时进入默认登录后页面。</span>
...
- <del>移动端扫码确认页展示设备名称、按 IP 粗定位至市级的登录位置；当前盖世账号存在 Steam/Epic 移动端本地绑定关系时展示“已绑定账号”分区；Mac 端登录成功弹窗展示盖世账号昵称。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>移动端扫码确认页展示设备和服务端返回的实际绑定平台；最终结果均回“我的”页并按请求 ID 提示一次 2 秒 Toast。Mac 成功后直接进入扫码前页面，失败、取消、过期仅在二维码遮罩内反馈。</span>

## 四、概要设计

### 4\.1 模块设计

- <del>Mac 端登录页：左侧展示二维码及扫码状态，右侧展示账号密码登录；二维码过期后支持点击二维码区域或“刷新二维码”按钮生成新二维码。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>Mac 登录页左侧展示二维码及扫码状态，右侧展示账号密码登录；失败、取消、过期时，状态文案与唯一“刷新二维码”按钮均位于二维码遮罩内，二维码容器不可点击、不可聚焦。</span>
...
- <del>Mac 端登录结果：移动端确认后进入登录态，并展示盖世账号昵称。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>`used` 后直接恢复扫码前 `returnTo` 页面并刷新登录态；Mac 不展示成功页、Toast、弹窗或平台继承结果。</span>

- 异常处理：覆盖二维码过期/失效、确认取消、多手机竞争、移动端登录态变化和各阶段网络异常。

#### 业务流程图

![扫码登录功能流程图\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/14-qr-login-flow.png)

### 4\.2 详细设计（C端）

功能 demo：

[扫码登录Mac端交互标注版demo\.html](https://htmlpreview.github.io/?https://raw.githubusercontent.com/z36358631-ship-it/-/3af87cfa41fc7236c240c230d54d56713256379a/demos/mac-qr-login-result-states.html)

#### 4\.2\.1 扫码登录全链路页面流程

|模块名称|图示|展示\&交互说明|
|---|---|---|
|Mac端登录页<br>|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/08-mac-login-page.png)<br>|**1\. 页面布局：** 登录卡片**左侧为扫码登录，右侧为账号密码登录**，两种登录方式在同一页面并列呈现。<br>**2\. 扫码区组成：** 扫码区由一次性二维码、扫码状态和安全说明组成。<br>**3\. 二维码有效态：** 二维码保持可扫描状态。<br><del>**4\. 二维码过期态：** 页面标题与状态行展示“二维码已过期”，二维码遮罩中央展示“点击刷新”，二维码下方展示“刷新二维码”按钮。<br>**5\. 刷新操作：** 点击二维码区域或“刷新二维码”按钮生成新二维码。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2 失败/取消/过期态：</strong>页面标题保持“扫码登录”；结果文案和唯一“刷新二维码”按钮均位于二维码遮罩内。点击按钮生成全新 challenge；二维码容器不可点击、不可聚焦。</span><br>**6\. 安全说明：** “二维码仅用于本次登录，使用后立即失效”位于状态行下方。<br>**7\. App下载入口：** 二维码下方展示“下载盖世游戏APP”，点击后跳转盖世游戏官网首页。<br>|
|移动端我的页扫码入口|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/01-profile-scan-entry.png)<br>|**1\. 展示条件：** 扫一扫入口仅对**已登录盖世账号**的用户可见。<br>**2\. 入口位置：** “我的”页右上角依次展示扫一扫、下载、设置三个按钮，**扫一扫位于下载按钮左侧**。<br>**3\. 已授权状态：** 相机权限已授权时，点击扫一扫直接进入扫码页。<br>**4\. 首次拒绝授权：** 首次未授权时触发系统相机权限弹窗；用户拒绝后返回“我的”页并显示 Toast **“摄像头权限未获取，请重新授权”**。<br>|
|移动端扫码权限引导弹窗<br>|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/06-camera-permission-guide.png)|**1\. 触发条件：** 用户选择不再询问相机权限后，再次点击扫一扫时展示权限引导弹窗。<br>**2\. 弹窗操作：** 主按钮为\*\*“去设置”**，次按钮为**“取消”\*\*。<br>**3\. 授权结果：** 用户从系统设置完成授权后，再次点击扫一扫进入扫码页。|
|移动端扫码页|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/02-mobile-scanner.png)|**1\. 页面组成：** 页面由扫码取景区域和返回入口组成。<br>**2\. 二维码识别：** 用户对准 Mac 端二维码后，系统识别二维码。<br>**3\. Mac端联动：** 移动端进入摄像头扫码页及识别阶段时，Mac 端保持左侧扫码、右侧账号密码登录的同屏布局，扫码区继续显示\*\*“等待手机扫一扫”\*\*。<br>**4\. 首次有效扫码：** 第一个有效扫码账号和设备进入确认环节。<br>**5\. 多设备扫码：** 其他手机扫描同一二维码时显示 Toast **“二维码已被扫描”**，并停留在扫码页。<br>**6\. 无效或过期：** 无效二维码显示 Toast **“二维码无效”**；过期二维码显示 Toast **“二维码已过期”**。<br>**7\. 网络异常：** 扫码校验网络异常时显示 Toast **“网络异常，请重试”**，移动端停留扫码页。|
|移动端扫码确认|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/03-mobile-confirm.png)|**1\. 确认弹窗：** 二维码校验通过后，移动端展示\*\*“登录至MacOS设备”**弹窗，明确本次操作正在登录 Mac 端。**<br>**2\. 设备信息： 弹窗展示设备名称和按 IP 粗定位至市级的登录位置；登录时间在能力支持时展示。**<br>**3\. 已绑定账号： 当前盖世账号已绑定平台时，“已绑定账号”分区展示服务端平台昵称、**“Steam 已登录”**或**“Epic 已登录”**状态及**“可用”**标签。**<br>**4\. 安全提示： 授权操作上方展示**“若非本人操作，请点击取消授权”\*\*。<br>**5\. 取消授权：** 点击“取消授权”后，移动端返回“我的”页，当前二维码立即失效。<br>**6\. 确认校验：** 点击“确认登录”时复核当前登录态、uid、移动端设备、session 和请求状态。<br>**7\. 登录态异常：** 登录态或账号变化时显示 Toast **“登录状态已变化，请重新扫码”**。<br>**8\. 请求失效：** 请求过期、已使用或失效时显示 Toast **“登录请求已失效，请重新扫码”**。<br>**9\. 网络异常：** 确认提交网络异常时显示 Toast **“网络异常，请重试”**，移动端停留确认弹窗。|
|Mac端等待确认|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/05-mac-pending-confirm.png)<br>|**1\. 二维码遮罩：** 二维码容器覆盖完整的**黑色半透明遮罩**。<br>**2\. 等待文案：** 遮罩内部展示\*\*“二维码已识别”**和**“请尽快在手机上确认”**。**<br>**3\. 有效期： 待确认阶段继续受二维码生成起**2 分钟有效期**限制。**<br><del>**4\. 超时状态：** 超过有效期后展示“二维码已失效/点击刷新”。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>超过有效期后，二维码遮罩内展示“二维码已过期”和唯一“刷新二维码”按钮。</span><br>**5\. 登录生效：** Mac 登录态在收到被绑定账号和设备的有效确认后生效。|
|Mac端登录已取消|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/04-mac-cancelled.png)|<del>**1\. 取消结果：** 移动端取消确认后，Mac 端展示“本次登录已取消，请刷新二维码”。<br>**2\. 重新登录：** 点击二维码区域或“刷新二维码”按钮生成新二维码。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>标题保持“扫码登录”；二维码遮罩内展示“登录已取消”和唯一“刷新二维码”按钮。App 返回“我的”页并提示一次 2 秒 Toast“登录已取消，请重新扫码”。</span><br>|
|移动端登录结果|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/09-mobile-success-toast.png)<br>![image 10\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/11-authorization-failed.png)<br>![image 12\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/13-expired.png)|<span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>成功、授权失败、取消、过期均返回“我的”页，每个请求只显示一次 2 秒 Toast。文案依次为“Mac端登录成功”“登录未完成，请重新扫码”“登录已取消，请重新扫码”“二维码已过期，请重新扫码”。`ready_to_claim` 不提示成功。</span><br>|
|Mac端登录成功|![image\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/07-mac-return-game-library.png)<br>|<del>**1\. 提示形式：** Mac 端完成登录后显示 Toast“登录成功”。<br>**2\. 展示时长：** Toast 展示 2 秒后自动消失。<br>**3\. 状态结果：** 停留在登录前页面。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>Mac 持久化并 ACK、服务端进入 `used` 后，关闭扫码登录界面，直接恢复扫码前 `returnTo` 页面并刷新登录态和用户数据；无成功页、Toast、弹窗。返回目标无效或无权限时进入默认登录后页面。</span><br>|
...
|已过期 `expired`|显示“二维码已过期”|显示“登录请求已失效，请重新扫码”|<del>二维码已失效，点击二维码区域或刷新按钮生成新二维码</del><br><strong style="color:#3370FF">V1\.2：二维码遮罩内显示“二维码已过期”和唯一“刷新二维码”按钮</strong>|是|否|
...
|待确认超时|<del>超过有效期后，Mac 端展示“二维码已过期/点击刷新”；移动端确认弹窗停留时，点击“确认登录”后 Toast“登录请求已失效，请重新扫码”</del><br><strong style="color:#3370FF">V1\.2：请求进入 `expired`；App 回“我的”页提示一次“二维码已过期，请重新扫码”；Mac 二维码遮罩内展示过期文案和唯一刷新按钮</strong>|
...
|Mac 未登录|确认通过后登录扫码账号|
|用户关闭登录成功弹窗|<del>点击弹窗右上角关闭按钮或弹窗外空白区域均可关闭；仅收起弹窗，Mac 端保持已登录状态</del><br><strong style="color:#3370FF">V1\.2：不再展示成功弹窗；`used` 后直接恢复 `returnTo` 页面</strong>|
...
|`ready_to_claim`|Mac 领取并 ACK|同一设备凭证有效，且会话包已持久化|`used`|<del>移动端与 Mac 显示成功</del><br><strong style="color:#3370FF">V1\.2：此状态不显示成功；进入 `used` 后 App 提示一次，Mac 直接恢复 `returnTo` 页面。</strong>|响应丢失时同一 Mac 在 30 秒内幂等领取同一会话包；超时未 ACK 则拒绝鉴权并回收|
...
|授权中|![image 9\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/10-authorizing.png)|点击“确认登录”后按钮进入不可重复点击状态，页面显示“正在登录 Mac 端”；Mac 端显示“正在同步账号”，最长等待 15 秒。平台数量和名称按该账号实际绑定关系动态展示。|
|待领取|![image 11\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/12-ready-to-claim.png)|全部会话原子激活后进入 <strong style="color:#3370FF">`ready_to_claim`</strong>。移动端继续显示“正在登录 Mac 端”；Mac 使用私有领取凭证获取同一加密会话包，持久化并 ACK 前不展示登录成功。领取响应丢失时，30 秒内幂等返回同一会话包。|
|登录成功|![image 8\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/09-mobile-success-toast.png)<br>![image 6\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/07-mac-return-game-library.png)|<del>移动端显示成功并列出继承平台；Mac 展示成功弹窗。</del><br><strong style="color:#3370FF">V1\.2：App 回“我的”页，仅提示一次 2 秒 Toast“Mac端登录成功”；Mac 在 `used` 后直接恢复 `returnTo` 页面并刷新登录态，无成功页、Toast 或弹窗。</strong>|
|授权失败|![image 10\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/11-authorization-failed.png)|<del>移动端和 Mac 端均显示“登录未完成”及说明正文。</del><br><strong style="color:#3370FF">V1\.2：App 回“我的”页，仅提示一次 2 秒 Toast“登录未完成，请重新扫码”；Mac 标题保持“扫码登录”，结果文案和唯一刷新按钮均在二维码遮罩内。</strong>|
...
本节为 <strong style="color:#3370FF">V1\.2 当前规则</strong>。与 V1\.0、V1\.1 的结果展示、文案和刷新交互冲突时，以本节为准；服务端状态码、日志和指标口径不变。

#### 4\.5\.1 主要问题与修改点

|主要问题|用户影响|V1\.2 修改点|
|---|---|---|
|Mac 失败、取消、过期同时在标题、状态行、二维码和外部按钮重复提示|信息层级混乱，用户不确定该点哪里|标题保持“扫码登录”；状态文案和唯一“刷新二维码”按钮收进二维码遮罩，二维码容器不可点击、不可聚焦|
|App 结果页、说明文字和操作按钮重复表达最终结果|用户完成操作后仍需处理多余页面|最终结果统一回“我的”页，每个请求仅显示一次 2 秒 Toast|
|Mac 成功后展示成功弹窗、平台列表或 Toast|成功后多一步关闭操作，打断原任务|`used` 后直接恢复扫码前 `returnTo` 页面并刷新登录态，无成功页、Toast、弹窗|
|`ready_to_claim` 与真正成功展示边界不清|可能在 Mac 尚未持久化时误报成功|只有 Mac 持久化并 ACK、服务端进入 `used` 后才提示成功|

#### 4\.5\.2 页面清单

|端|页面/状态|图示|V1\.2 展示与交互|
|---|---|---|---|
|App|“我的”页－成功|![image 8\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/09-mobile-success-toast.png)|显示一次 2 秒 Toast“Mac端登录成功”；无结果页、弹窗和平台列表|
|App|“我的”页－授权失败|![image 10\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/11-authorization-failed.png)|显示一次 2 秒 Toast“登录未完成，请重新扫码”|
|App|“我的”页－取消|与 Mac 取消态同链路，见 image 3\.png|显示一次 2 秒 Toast“登录已取消，请重新扫码”|
|App|“我的”页－过期|![image 12\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/13-expired.png)|显示一次 2 秒 Toast“二维码已过期，请重新扫码”|
|Mac|扫码登录－授权失败|![image 10\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/11-authorization-failed.png)|二维码遮罩内展示“登录未完成”和唯一“刷新二维码”按钮|
|Mac|扫码登录－取消|![image 3\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/04-mac-cancelled.png)|二维码遮罩内展示“登录已取消”和唯一“刷新二维码”按钮|
|Mac|扫码登录－过期|![image 12\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/13-expired.png)|二维码遮罩内展示“二维码已过期”和唯一“刷新二维码”按钮|
|Mac|扫码前业务页－成功返回|![image 6\.png](https://cdn.jsdelivr.net/gh/z36358631-ship-it/-@3af87cfa41fc7236c240c230d54d56713256379a/public/prd/mac-qr-login/07-mac-return-game-library.png)|直接恢复经过校验的 `returnTo` 页面并刷新登录态；Demo 以“游戏库”为例|
...
|场景|处理方式|
|---|---|
|App 最终结果通知|Toast 持续 2 秒，按请求 ID 去重；同一请求重复轮询、重复 ACK 或页面恢复不得重复提示|
|`ready_to_claim`|继续显示授权中/等待态，不显示成功 Toast，不进入 Mac 业务页|
|Mac 成功返回|使用扫码前保存且经过校验的 `returnTo`；刷新登录态、用户信息及当前页面依赖的登录数据|
|`returnTo` 缺失、失效或无权限|进入默认登录后页面，不停留扫码登录界面，不增加兜底弹窗|
|失败、取消、过期|二维码外不重复展示结果标题、正文或按钮；二维码容器不可点击、不可聚焦|
|刷新二维码|仅遮罩内“刷新二维码”按钮可触发；鼠标点击、Enter、Space 均有效；每次生成全新 challenge，旧 challenge 保持终态且不可复用|
|重复操作|按钮触发生成期间防重复提交；最终以最新成功生成的 challenge 为准|
|弱网|刷新失败时保留当前终态并在二维码遮罩内反馈重试；不得恢复旧二维码或创建多个可用 challenge|
|国内与海外|结果逻辑一致；海外包产品名使用 GameHub，Toast 按现有多语言资源翻译，不新增区服差异|
...
|单平台继承|账号只绑定 Steam|扫码并确认|<del>确认页和成功页只展示 Steam</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">V1\.2：</strong>确认页只展示 Steam；成功后不展示结果页或平台列表；只创建盖世账号与 Steam Mac 会话</span>|P0|
...
|`ready_to_claim` 不提前成功|服务端已进入 `ready_to_claim`，Mac 尚未完成持久化和 ACK|保持页面并重复查询结果|App 不显示成功 Toast；Mac 不进入 `returnTo` 页面|P0|
|成功结果映射|Mac 已持久化会话并 ACK，服务端进入 `used`|客户端收到最终结果|App 回“我的”页，仅显示一次 2 秒 Toast“Mac端登录成功”；Mac 直接恢复 `returnTo` 页面并刷新登录态，无成功页、Toast、弹窗|P0|
|授权失败结果映射|服务端进入 `authorization_failed`|客户端收到最终结果|App 回“我的”页，仅显示一次 2 秒 Toast“登录未完成，请重新扫码”；Mac 标题保持“扫码登录”，二维码遮罩内显示“登录未完成”和唯一刷新按钮|P0|
|取消结果映射|服务端进入 `cancelled`|客户端收到最终结果|App 回“我的”页，仅显示一次 2 秒 Toast“登录已取消，请重新扫码”；Mac 二维码遮罩内显示“登录已取消”和唯一刷新按钮|P0|
|过期结果映射|服务端进入 `expired`|客户端收到最终结果|App 回“我的”页，仅显示一次 2 秒 Toast“二维码已过期，请重新扫码”；Mac 二维码遮罩内显示“二维码已过期”和唯一刷新按钮|P0|
|Toast 去重|同一请求重复轮询、重复 ACK 或 App 页面恢复|多次接收同一最终结果|同一请求只提示一次；新 challenge 对应的新请求可正常提示|P0|
|唯一刷新入口|Mac 处于失败、取消或过期态|检查页面并点击二维码外区域|仅二维码遮罩内存在一个“刷新二维码”按钮；二维码容器不可点击、不可聚焦；外部无重复状态块和刷新按钮|P0|
|键盘刷新|Mac 处于失败、取消或过期态|聚焦刷新按钮后按 Enter 或 Space|生成全新 challenge 并返回 `waiting_scan`；旧 challenge 不可复用|P0|
|无效 `returnTo`|扫码前页面缺失、已下线或当前账号无权限|完成登录进入 `used`|进入默认登录后页面并刷新登录态；无兜底弹窗，不停留扫码登录界面|P0|

### 9\.2 待确认项

#### 必须确认（阻塞开发）

无。平台继承范围、会话签发方式、成功条件、授权时限、最终结果文案、Toast 时长、Mac 成功返回规则和唯一刷新入口已确认。

#### 建议确认（影响完整度）

- [假设] 指标目标先采用“扫码登录成功率不低于 95%、授权失败率低于 1%”。默认按此口径配置灰度告警；影响上线判断，引用第七章。

- <del>[假设] 授权失败提示统一为“平台登录状态未能全部同步，本次授权已撤销，请重新扫码”。</del><br><span style="background-color:#FFF2CC"><strong style="color:#3370FF">已确认：</strong>V1\.2 的 App 授权失败 Toast 为“登录未完成，请重新扫码”；Mac 二维码遮罩文案为“登录未完成”，不再显示长说明。引用 4\.5。</span>
...
- 已补齐 V1\.2 的四类最终结果映射、Toast 去重、`ready_to_claim` 与 `used` 边界、唯一刷新入口、键盘操作、无效 `returnTo`、弱网和重复提交处理。
- 国内与海外结果逻辑一致；海外产品名使用 GameHub，文案沿用现有多语言资源，不改变登录入口差异。
- 本 PRD 共引用 13 张页面图和 1 张流程图，均已发布到 Git 提交 `3af87cfa41fc7236c240c230d54d56713256379a`；24 处 Markdown 图示引用和 1 个 Demo 链接已替换为固定 SHA 的公开 HTTPS 地址。14 张唯一图片均通过 HTTP 200 与 `image/png` 校验；Demo 固定 SHA 预览页通过 HTTP 200 与 `text/html` 校验，可直接打开并导入飞书。
...
|角色|结论|发现的问题|
|---|---|---|
|前端开发|✓ 已修复|旧规则同时保留二维码点击刷新和外部按钮，且成功弹窗与 `returnTo` 冲突。已在 4\.2\.1、4\.4\.3、4\.4\.4、4\.5 中明确唯一按钮、不可交互二维码容器和成功直返。|
|测试工程师|✓ 已修复|`ready_to_claim` 可能误报成功，Toast 重复和无效 `returnTo` 缺少验收。已在 4\.5\.4、9\.1 增加状态边界、去重和兜底用例。|
|运营/业务方|✓ 通过|本轮不新增运营配置；国内与海外仅复用现有多语言资源，服务端状态码、日志和指标不变。|

**已自动补充的硬伤：** 唯一刷新入口、键盘可操作、二维码不可聚焦、四类结果映射、`ready_to_claim` 不提前成功、Toast 按请求去重、无效 `returnTo` 兜底。

**待用户确认的建议：** 无。结果文案与交互均已确认。
